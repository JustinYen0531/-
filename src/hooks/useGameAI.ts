...
const AI_MAX_ACTIONS_PER_UNIT = 99;
const AI_FOLLOW_UP_SCORE_THRESHOLD = 2;
const AI_MOVE_FOLLOW_UP_EXTRA_THRESHOLD = 4.5;
const AI_MAX_NON_URGENT_MOVE_CHAIN = 2;
const AI_FOLLOW_UP_DELAY_BASE = 280;
const AI_FOLLOW_UP_DELAY_JITTER = 220;
const AI_NON_MOVE_REPEAT_LIMIT = 1;
...
const getActionSignature = (action: AICandidateAction) => {
    const targetSig = action.target?.kind === 'cell'
        ? `${action.target.r},${action.target.c}`
        : action.target?.kind === 'unit'
            ? action.target.unit.id
            : action.target?.kind === 'mineType'
                ? action.target.mineType
                : '-';
    const sourceSig = action.sourceCell ? `${action.sourceCell.r},${action.sourceCell.c}` : '-';
    const mineTypeSig = action.mineType ?? '-';
    return `${action.type}|${targetSig}|${sourceSig}|${mineTypeSig}`;
};

const dedupeActionsBySignature = (actions: AICandidateAction[]) => {
...
const isActionTooRepetitive = (
    action: AICandidateAction,
    completedActions: AICandidateAction[]
) => {
    if (completedActions.length === 0) return false;
    const signature = getActionSignature(action);
    if (completedActions.some(done => getActionSignature(done) === signature)) {
        return true;
    }
    if (action.type === 'move') return false;
    const sameTypeCount = completedActions.filter(done => done.type === action.type).length;
    return sameTypeCount >= AI_NON_MOVE_REPEAT_LIMIT;
};
...
const isBacktrackMoveAction = (
    state: GameState,
    unitId: string,
    action: AICandidateAction
) => {
    if (action.type !== 'move' || action.target?.kind !== 'cell') return false;
    const unitMoves = state.movements.filter(m => m.unitId === unitId);
    if (unitMoves.length === 0) return false;
    const lastMove = unitMoves[unitMoves.length - 1];
    return action.target.r === lastMove.from.r && action.target.c === lastMove.from.c;
};

const isRevisitLoopMoveAction = (
    state: GameState,
    unitId: string,
    action: AICandidateAction
) => {
    const target = action.target;
    if (action.type !== 'move' || !target || target.kind !== 'cell') return false;
    const unitMoves = state.movements.filter(m => m.unitId === unitId);
    if (unitMoves.length === 0) return false;
    const visited = [unitMoves[0].from, ...unitMoves.map(m => m.to)];
    return visited.some(cell => cell.r === target.r && cell.c === target.c);
};
...
export const useGameAI = ({
    ...
    useEffect(() => {
        ...
        const performAITurn = () => {
            ...
            const uniqueSortedActions = dedupeActionsBySignature(sortedActions);
            const finalTopCandidates = summarizeTopCandidates(uniqueSortedActions);
            ...
            if (debug) {
                const picks = uniqueSortedActions.slice(0, AI_MAX_LOGGED_ACTIONS);
            ...
            const primaryAction = uniqueSortedActions[0];
            const candidatePressure = Math.min(6, Math.max(0, uniqueSortedActions.length - 1));
            ...
            let completedActions: AICandidateAction[] = [];
            ...
            const tryFollowUp = () => {
                ...
                const freshUnique = dedupeActionsBySignature(freshConstrained)
                    .filter(candidate => !isActionTooRepetitive(candidate, completedActions));
                const followUps = freshUnique.filter(a => a.type !== 'end_turn');
                const endScore = freshUnique.find(a => a.type === 'end_turn')?.score ?? 0;
                const unitMoveCount = freshState.movements.filter(m => m.unitId === selectedUnit.id).length;
                const urgentMoveMode =
                    freshUnit.hasFlag ||
                    freshCtx.intent === 'hunt_flag_carrier' ||
                    freshCtx.endgame.mode === 'race' ||
                    (freshCtx.endgame.mode === 'defense' && freshCtx.endgame.urgency >= 2);
                const viableFollowUps = followUps.filter(candidate => {
                    if (candidate.type !== 'move') {
                        return candidate.score > endScore + AI_FOLLOW_UP_SCORE_THRESHOLD;
                    }
                    if (isBacktrackMoveAction(freshState, selectedUnit.id, candidate)) {
                        return false;
                    }
                    if (!urgentMoveMode && isRevisitLoopMoveAction(freshState, selectedUnit.id, candidate)) {
                        return false;
                    }
                    if (!urgentMoveMode && unitMoveCount >= AI_MAX_NON_URGENT_MOVE_CHAIN) {
                        return false;
                    }
                    const moveThreshold = AI_FOLLOW_UP_SCORE_THRESHOLD
                        + (urgentMoveMode ? 1.5 : AI_MOVE_FOLLOW_UP_EXTRA_THRESHOLD)
                        + Math.max(0, unitMoveCount - 1) * 1.2;
                    return candidate.score > endScore + moveThreshold;
                });
                if (viableFollowUps.length === 0) {
                    actionsRef.current.handleActionComplete(selectedUnit.id);
                    return;
                }
                const followUp = viableFollowUps[0];
                if (debug) {
                    console.log(
                ...
                if (onDecisionRef.current) {
                    ...
                    onDecisionRef.current({
                        ...
                        finalTopCandidates: summarizeTopCandidates(freshUnique),
                ...
                setTimeout(() => {
                    ...
                    completedActions = [...completedActions, followUp];
            ...
            const tryAction = (index: number) => {
                ...
                if (index >= uniqueSortedActions.length || index >= AI_MAX_ACTION_RETRIES) {
                    actionsRef.current.handleActionComplete(selectedUnit.id);
                    return;
                }

                const action = uniqueSortedActions[index];
                if (onDecisionRef.current) {
                    onDecisionRef.current({
