import { GRID_COLS, GRID_ROWS, UNIT_STATS, ORE_REWARDS } from '../constants';
...
const getUnitMoveTrail = (state: GameState, unitId: string) =>
    state.movements.filter(m => m.unitId === unitId);

const getNearestOreDistance = (state: GameState, fromR: number, fromC: number) => {
    let best = Number.POSITIVE_INFINITY;
    for (let r = 0; r < GRID_ROWS; r += 1) {
        for (let c = 0; c < GRID_COLS; c += 1) {
            const cell = state.cells[r][c];
            if (!cell.hasEnergyOre || !cell.oreSize) continue;
            const dist = manhattan(fromR, fromC, r, c);
...
export const evaluateUnitPriorityWithContext = (
    ...
    if (context.intent === 'push_flag') {
        if (unit.hasFlag) total += 12;
        const anyoneHasFlag = state.players[unit.owner].units.some(u => !u.isDead && u.hasFlag);
        if (!anyoneHasFlag) {
            // No one has the flag yet — prioritize flag-capable units to go pick it up
            const genLevelB = state.players[unit.owner].evolutionLevels[UnitType.GENERAL].b;
            const canCarry = unit.type === UnitType.GENERAL || genLevelB >= 3;
            if (canCarry) {
                const ownFlagPos = state.players[unit.owner].flagPosition;
                const distToFlag = manhattan(unit.r, unit.c, ownFlagPos.r, ownFlagPos.c);
                // Closer to flag = higher priority, at flag = massive priority
                total += distToFlag === 0 ? 18 : Math.max(0, 12 - distToFlag * 1.5);
            }
        }
        if (unit.type === UnitType.GENERAL || unit.type === UnitType.RANGER) total += 3;
...
export const evaluateActionCandidate = (
    ...
    if (actionType === 'move' && target?.kind === 'cell') {
        ...
        const flagBonus = unit.hasFlag ? 8 : 3;
        flag = Math.max(0, currentDist - nextDist) * flagBonus;
        const moveTrail = getUnitMoveTrail(state, unit.id);
        const moveCount = moveTrail.length;
        const lastMove = moveCount > 0 ? moveTrail[moveCount - 1] : null;
        const laneAnchorR = moveCount > 0 ? moveTrail[0].to.r : unit.r;
        const visitedCells = moveCount > 0
            ? [{ r: moveTrail[0].from.r, c: moveTrail[0].from.c }, ...moveTrail.map(m => ({ r: m.to.r, c: m.to.c }))]
            : [];
        const revisitCount = visitedCells.filter(cell => cell.r === target.r && cell.c === target.c).length;
        const recentlyVisited = visitedCells.slice(-4).some(cell => cell.r === target.r && cell.c === target.c);
        const urgentReposition =
            unit.hasFlag ||
            context?.intent === 'hunt_flag_carrier' ||
            context?.endgame.mode === 'race' ||
            (context?.endgame.mode === 'defense' && context?.endgame.urgency >= 2);

        // Flag pickup incentive: if no one carries the flag, incentivize flag-capable units
        // to move toward their own flag to pick it up
        const genLevelB = state.players[unit.owner].evolutionLevels[UnitType.GENERAL].b;
        const canCarryFlag = unit.type === UnitType.GENERAL || genLevelB >= 3;
        const anyoneHasFlag = state.players[unit.owner].units.some(u => !u.isDead && u.hasFlag);
        if (canCarryFlag && !anyoneHasFlag) {
            const ownFlagPos = state.players[unit.owner].flagPosition;
            const curDistToOwnFlag = manhattan(unit.r, unit.c, ownFlagPos.r, ownFlagPos.c);
            const nextDistToOwnFlag = manhattan(target.r, target.c, ownFlagPos.r, ownFlagPos.c);
            const approachBonus = Math.max(0, curDistToOwnFlag - nextDistToOwnFlag) * 5;
            // Extra bonus when arriving at the flag
            const arrivalBonus = nextDistToOwnFlag === 0 ? 12 : 0;
            flag += approachBonus + arrivalBonus;
        }

        // Energy ore incentive: reward moving onto or toward ore cells
        const targetCell = state.cells[target.r]?.[target.c];
        if (targetCell?.hasEnergyOre && targetCell.oreSize) {
            const oreValue = ORE_REWARDS[targetCell.oreSize] ?? 0;
            // Direct capture: strong utility bonus scaled by ore value
            utility += oreValue * 2.6 + 2;
            energy += oreValue * 1.2;
        } else {
            // Proximity to nearby ores: mild incentive to move toward them
            let nearestOreDist = 999;
                ...
                for (let dc = -3; dc <= 3; dc++) {
                    ...
                    if (nr >= 0 && nr < GRID_ROWS && nc >= 0 && nc < GRID_COLS) {
                        ...
                        if (cell.hasEnergyOre && cell.oreSize) {
                            ...
                            if (dist < nearestOreDist) nearestOreDist = dist;
                        }
                    }
                }
            }
            if (nearestOreDist < 999) {
                utility += Math.max(0, 4 - nearestOreDist) * 1.2;
            }
        }

        const nearestOreBefore = getNearestOreDistance(state, unit.r, unit.c);
        const nearestOreAfter = getNearestOreDistance(state, target.r, target.c);
        if (nearestOreBefore !== null && nearestOreAfter !== null) {
            const oreGain = nearestOreBefore - nearestOreAfter;
            if (oreGain > 0) {
                utility += oreGain * 2.2;
            } else if (oreGain < 0 && !urgentReposition) {
                utility -= Math.abs(oreGain) * 2.6;
            }
        }

        const forwardStep = unit.owner === PlayerID.P2 ? -1 : 1;
        const frontR = unit.r;
        const frontC = unit.c + forwardStep;
        if (frontC >= 0 && frontC < GRID_COLS) {
            const frontCell = state.cells[frontR][frontC];
            const frontOccupied = isCellOccupied(state, frontR, frontC);
            const frontOreFree = !!frontCell?.hasEnergyOre && !!frontCell?.oreSize && !frontCell.isObstacle && !frontOccupied;
            if (frontOreFree) {
                if (target.r === frontR && target.c === frontC) {
                    utility += 10;
                    energy += 3.5;
                } else if (!urgentReposition) {
                    utility -= 8;
                }
            }
        }
        ...
        if (lastMove && target.r === lastMove.from.r && target.c === lastMove.from.c) {
            // Strongly discourage oscillation (A -> B -> A) unless the board state is urgent.
            utility -= urgentReposition ? 8 : 16;
            energy = Math.max(0, energy - (urgentReposition ? 1.5 : 3));
        }

        if (revisitCount > 0) {
            const revisitPenalty = urgentReposition ? (2.8 + revisitCount * 1.6) : (9 + revisitCount * 3.8);
            utility -= revisitPenalty;
            if (recentlyVisited && !urgentReposition) {
                utility -= 7;
            }
        }

        if (moveCount > 0) {
            // Keep the unit close to the first chosen lane in this action sequence.
            const currentLaneDrift = Math.abs(unit.r - laneAnchorR);
            const nextLaneDrift = Math.abs(target.r - laneAnchorR);
            if (nextLaneDrift > currentLaneDrift) {
                utility -= (urgentReposition ? 1.4 : 3.2) * (nextLaneDrift - currentLaneDrift);
            } else if (nextLaneDrift < currentLaneDrift) {
                utility += Math.min(2.4, (currentLaneDrift - nextLaneDrift) * 1.2);
            }

            // Each extra move in the same unit turn gets lower marginal value.
            const fatiguePenalty = Math.min(9, moveCount * (urgentReposition ? 1.1 : 2.1));
            utility -= fatiguePenalty;

            if (!urgentReposition && !unit.hasFlag && (currentDist - nextDist) <= 0) {
                utility -= 2 + moveCount * 0.8;
            }
        }
        ...
        const pickableMines = state.mines.filter(m =>
            m.r === target.r &&
            m.c === target.c &&
            (m.owner === unit.owner || m.revealedTo.includes(unit.owner))
        );
        const preferredMine = pickableMines.find(m => m.owner === unit.owner) ?? pickableMines[0];
        const ownMineBonus = preferredMine?.owner === unit.owner ? 8 : 2.5;
        const mineTypeBonus = preferredMine
            ? (preferredMine.type === MineType.NUKE ? 4.5
                : preferredMine.type === MineType.CHAIN ? 3.2
                    : preferredMine.type === MineType.SMOKE ? 2.1
                        : preferredMine.type === MineType.SLOW ? 1.8
                            : 1.2)
            : 0;
        utility = 6.5 + nearEnemyFlag + ownMineBonus + mineTypeBonus;
        safety = 6;
