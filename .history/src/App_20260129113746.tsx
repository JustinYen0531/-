...
export default function App() {
    ...
    // Use ref for isProcessing to avoid re-renders when toggling
    const isProcessingRef = useRef(false);
    const [, forceRender] = useState(0); // Minimal state to trigger re-render when needed
    ...
    */
    ...
    const handleCellClick = useCallback((r: number, c: number) => {
        ...
        // We set the visual state BEFORE any logic to ensure the UI updates in the very first frame.
        ...
        if (isActionPotentiallyFreezing) {
            // Use flushSync to force immediate DOM update before executing logic
        ...
        // Execute logic synchronously - no more artificial delay
        try {
            executeCellClickLogic(r, c);
        } finally {
            // Reset processing state after a brief overlap period
            // This ensures smooth visual transition to isTimeFrozen
            if (isActionPotentiallyFreezing) {
                setTimeout(() => {
                            ...
                            <div className="flex-1 flex flex-col overflow-visible relative">
                                ...
                                <div className="w-full px-8 py-3 flex items-center justify-center gap-6 relative z-20">
                                    ...
                                    <div className="flex items-center gap-2 justify-end min-w-[80px]">
                                        <div className={`transition-opacity duration-75 ease-in-out ${gameState.isTimeFrozen || isProcessingRef.current ? 'opacity-100' : 'opacity-0'}`}>
                                            <Snowflake size={18} className="text-cyan-300 animate-spin-slow" />
                                        </div>
                                        <span className={`font-bold text-lg text-right transition-colors duration-75 ${gameState.isTimeFrozen || isProcessingRef.current ? 'text-cyan-300' : 'text-white'}`}>
