...
export default function App() {
    ...
    const handleUnitClick = (unit: Unit) => {
        ...
        if (state.phase === 'thinking') {
            ...
            setGameState(prev => {
                ...
                // Find the unit currently at the top of the squad panel (ready to act)
                const firstAliveId = p.unitDisplayOrder.find(id => {
                    const u = p.units.find(un => un.id === id);
                    return u && !u.isDead;
                });

                let newOrder = [...p.unitDisplayOrder];
                if (firstAliveId && firstAliveId !== unit.id) {
                    const idx1 = newOrder.indexOf(firstAliveId);
                    const idx2 = newOrder.indexOf(unit.id);
                    if (idx1 !== -1 && idx2 !== -1) {
                        // Swap with the unit at the top so the clicked one becomes next to act
                        [newOrder[idx1], newOrder[idx2]] = [newOrder[idx2], newOrder[idx1]];
                        addLog('log_squad_swap', 'info', { unit: getUnitName(unit.type) }, state.currentPlayer);
                    }
                }
                ...
                return {
                    ...
                    players: {
                        ...prev.players,
                        [prev.currentPlayer]: { ...p, unitDisplayOrder: newOrder }
                    }
                };
            });
            setTargetMode(null);
    ...
    const handleDetonateTowerAction = (unit: Unit) => {
        ...
        setGameState(prev => {
            ...
            // Detonate enemy mines only in 3x3 range of any tower
            const minesToRemove = prev.mines.filter(m =>
                m.owner !== unit.owner && // Only ENEMY mines
              ...
              `}</style>
                    ...
                    <ControlPanel
                        ...
                        actions={{
                            ...
                            handleSkipTurn: playerActions.handleSkipTurn,
