...
import {
    ...
    Bomb, Eye, Shield, Footprints, Crown, Flag, CheckCircle, Play, LogOut, Cpu, Users, ArrowDownToLine,
    Dna, Unlock, X, Globe, Star, Hand, ChevronLeft, ChevronRight, FlaskConical, Radio, Ghost, Volume2, VolumeX
} from './icons';
...
export function getUnitTypeAbbr(type: UnitType): string {
    switch (type) {
        case UnitType.GENERAL: return 'GEN';
        case UnitType.MINESWEEPER: return 'SWE';
        case UnitType.RANGER: return 'RAN';
        case UnitType.MAKER: return 'MAK';
        case UnitType.DEFUSER: return 'DEF';
        default: return 'UNT';
    }
}

export function getMineBaseCost(type: MineType): number {
    switch (type) {
        case MineType.NORMAL: return 10;
        case MineType.SLOW: return 20;
        case MineType.SMOKE: return 15;
        case MineType.NUKE: return 25;
        case MineType.CHAIN: return 30;
        default: return 10;
    }
}

export function getUnitIcon(type: UnitType, size: number = 18, tier: number = 0) {
    const commonProps = { size, className: "transition-all duration-300" };
    switch (type) {
        case UnitType.GENERAL: return <Crown {...commonProps} className={`${commonProps.className} ${tier === 3 ? 'text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.6)]' : ''}`} />;
        case UnitType.MINESWEEPER: return <Eye {...commonProps} className={`${commonProps.className} ${tier === 3 ? 'text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.6)]' : ''}`} />;
        case UnitType.RANGER: return <Footprints {...commonProps} className={`${commonProps.className} ${tier === 3 ? 'text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.6)]' : ''}`} />;
        case UnitType.MAKER: return <Bomb {...commonProps} className={`${commonProps.className} ${tier === 3 ? 'text-rose-400 drop-shadow-[0_0_8px_rgba(251,113,133,0.6)]' : ''}`} />;
        case UnitType.DEFUSER: return <Shield {...commonProps} className={`${commonProps.className} ${tier === 3 ? 'text-indigo-400 drop-shadow-[0_0_8px_rgba(129,140,248,0.6)]' : ''}`} />;
...
export function getUnitName(type: UnitType): string {
    switch (type) {
        case UnitType.GENERAL: return '帥位 (GENERAL)';
        case UnitType.MINESWEEPER: return '工兵 (SWEEPER)';
        case UnitType.RANGER: return '遊騎 (RANGER)';
        case UnitType.MAKER: return '佈雷 (MAKER)';
        case UnitType.DEFUSER: return '排雷 (DEFUSER)';
        default: return type;
    }
}
              ...
              `}</style>
                    ...
                    {showEvolutionTree && (
                        <EvolutionTree
                            ...
                            getUnitTypeAbbr={getUnitTypeAbbr}
                        />
                    )}
