...
export default function App() {
    ...
    */
    ...
    const handleCellClick = useCallback((r: number, c: number) => {
        // === IMMEDIATE TIME FREEZE with CHRONAL OVERLOAD SYSTEM === 
        const state = gameStateRef.current;
        if (state.gameOver || state.isPaused) return;
        ...
        if (isActionPotentiallyFreezing) {
            const now = Date.now();
            const MAX_CHRONAL_CHARGES = 10;

            // Check if we're in overload cooldown
            if (state.isChronalOverload) {
                // During overload, time does NOT freeze - just execute the action
                executeCellClickLogic(r, c);
                return;
            }

            // Increment chronal charges
            const newCharges = state.chronalCharges + 1;

            if (newCharges > MAX_CHRONAL_CHARGES) {
                // === CHRONAL OVERLOAD TRIGGERED! ===
                // 1 second penalty where time cannot be frozen
                flushSync(() => {
                    setGameState(prev => ({
                        ...prev,
                        isTimeFrozen: false,
                        isChronalOverload: true,
                        chronalOverloadEndTime: now + 1000, // 1 second penalty
                        chronalCharges: 0, // Reset charges
                        lastActionTime: now
                    }));
                });
                // Execute action without freeze
                executeCellClickLogic(r, c);
                return;
            }

            // Normal freeze: Increment charges and freeze time
            flushSync(() => {
                setGameState(prev => ({
                    ...prev,
                    isTimeFrozen: true,
                    lastActionTime: now,
                    chronalCharges: newCharges
                }));
            });
        }
                            ...
                            <div className="flex-1 flex flex-col overflow-visible relative">
                                ...
                                <div className="w-full px-8 py-3 flex items-center justify-center gap-6 relative z-20">
                                    ...
                                    <div className="flex items-center gap-2 justify-end min-w-[120px]">
                                        {/* Chronal Overload Indicator */}
                                        {gameState.isChronalOverload && (
                                            <div className="absolute inset-0 pointer-events-none animate-pulse">
                                                <div className="absolute inset-0 bg-red-500/20 animate-shake" />
                                            </div>
                                        )}
                                        {gameState.isChronalOverload ? (
                                            /* OVERLOAD STATE */
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Snowflake size={18} className="text-red-400 opacity-30" />
                                            </div>
                                        ) : (
                                            /* NORMAL STATE */
                                            <>
                                                {/* Chronal Charge Counter - shows as snowflake opacity */}
                                                <div className={`transition-opacity duration-75 ease-in-out relative ${gameState.isTimeFrozen ? 'opacity-100' : 'opacity-0'}`}>
                                                    <Snowflake
                                                        size={18}
                                                        className="text-cyan-300 animate-spin-slow"
                                                        style={{
                                                            opacity: Math.max(0.3, 1 - (gameState.chronalCharges / 10) * 0.7)
                                                        }}
                                                    />
                                                    {/* Charge counter badge */}
                                                    {gameState.chronalCharges > 0 && (
                                                        <span className="absolute -top-1 -right-1 text-[9px] font-bold text-cyan-200 bg-cyan-900/80 rounded-full w-3.5 h-3.5 flex items-center justify-center">
                                                            {10 - gameState.chronalCharges}
                                                        </span>
                                                    )}
                                                </div>
                                            </>
                                        )}
                                        <span className={`font-bold text-lg text-right transition-colors duration-75 ${gameState.isChronalOverload ? 'text-red-400' :
                                                gameState.isTimeFrozen ? 'text-cyan-300' : 'text-white'
                                            }`}>
