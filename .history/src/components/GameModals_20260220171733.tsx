    ...
    for (let i = 0; i < headerIndices.length; i++) {
        ...
        if (headerMatch) {
            ...
            logs.push({
                ...
                content: content,
                preview: normalizeDeveloperLogPreview(rawArticle)
            });
        }
    }
    ...
    if (view === 'lobby') {
        return (
            <div className="flex flex-col items-center justify-center h-full bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 p-4 relative w-full absolute inset-0 z-50 overflow-hidden">
                ...
                {!roomId ? (
                    <div className="relative z-10 mt-12 flex w-full max-w-5xl flex-col items-center gap-8">
                        ...
                        {latestDeveloperLog && (
                            <button
                                ...
                                className="w-full max-w-2xl rounded-2xl border-2 border-cyan-400/60 bg-gradient-to-r from-slate-900/95 via-cyan-950/70 to-slate-900/95 px-6 py-5 text-center shadow-2xl shadow-cyan-500/25 transition-all hover:border-cyan-300 hover:scale-[1.02] hover:shadow-cyan-500/40"
                            >
                                <div className="flex flex-col items-center gap-3">
                                    <div className="flex items-center gap-2 text-cyan-200/90 bg-cyan-500/10 px-3 py-1 rounded-full border border-cyan-500/30">
                                        <Info size={14} />
                                        <span className="text-[10px] font-black tracking-[0.15em] uppercase">{uiText.latestArticle}</span>
                                    </div>
                                    <div className="space-y-1">
                                        <h3 className="text-xl font-black text-white">{latestDeveloperLog.title}</h3>
                                        <div className="text-xs font-mono text-cyan-400/80">{latestDeveloperLog.dateLabel}</div>
                                    </div>
                                    <div className="w-12 h-0.5 bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent" />
                                    <p className="text-sm text-slate-300 leading-relaxed max-w-lg mx-auto line-clamp-2">
