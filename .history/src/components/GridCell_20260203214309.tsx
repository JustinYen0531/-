...
const GridCell: React.FC<GridCellProps> = ({
  ...
  if (selectedUnit && targetMode) {
    ...
    if (targetMode === 'move' && manhattanDist === 1) {
      ...
      isInActionRange = true;
      actionRangeColor = 'border-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.6)]';
    } else if (targetMode === 'pickup_mine_select' && chebyshevDist <= 1) {
      isInActionRange = true;
      ...
      const isInFactoryRange = factories.some(f =>
        Math.max(Math.abs(f.r - cell.r), Math.abs(f.c - cell.c)) <= (f.level >= 2 ? 2 : 1)
      );
  ...
  const hubBoundaryBuilding = buildings.find(b =>
    b.type === 'hub' &&
  ...
  if (isInsideTowerRange && !cell.isObstacle) {
    bgColor = `${bgColor} border-2 border-dotted ${towerBuilding!.owner === PlayerID.P1 ? 'border-cyan-500/50' : 'border-red-500/50'}`;
  }
  ...
  const renderBuilding = () => {
    ...
    if (building.type === 'tower') {
      ...
      return (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 overflow-visible">
          {/* Hub Boundary: Dashed Line (3x3 area) */}
  ...
  return (
    ...
    >
      ...
      {
        mine && isMineVisible && (
          <div className={`absolute opacity-95 drop-shadow-2xl animate-pulse ${unit ? 'bottom-0.5 left-0.5 z-10' : 'inset-0 flex items-center justify-center'} ${mine?.owner === PlayerID.P1 ? 'text-cyan-400' : 'text-red-400'}`}>
      ...
      {/* Minimalist Persistent Smoke Effect */}
      {
        (isSmoked || isHub31Smoke) && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 opacity-30">
            <Cloud
              size={28}
              className={`animate-[gentleBreathe_8s_infinite_ease-in-out] ${(isHub31Smoke ? hubBoundaryBuilding?.owner : smokeOwner) === PlayerID.P1 ? 'text-cyan-200' : 'text-red-200'
                }`}
            />
          </div>
        )
      ...
      {/* Unit */}
      {
        unit && !unit.isDead && (
          ...
          >
            ...
            {/* Breathing Wrapper for Little Person Character */}
            <div className="w-full h-full relative flex items-center justify-center animate-unit-breathe font-bold">

              {/* Limbs - Hands (Small Nubs) */}
              <div className={`little-person-limb limb-hand-left -left-0.5 top-[18px] ${unit?.owner === PlayerID.P1 ? 'bg-cyan-600' : 'bg-red-600'}`} />
              <div className={`little-person-limb limb-hand-right -right-0.5 top-[18px] ${unit?.owner === PlayerID.P1 ? 'bg-cyan-600' : 'bg-red-600'}`} />

              {/* Limbs - Feet (Small Nubs) */}
              <div className={`little-person-limb limb-foot-left left-[10px] bottom-[2px] ${unit?.owner === PlayerID.P1 ? 'bg-cyan-600' : 'bg-red-600'}`} />
              <div className={`little-person-limb limb-foot-right right-[10px] bottom-[2px] ${unit?.owner === PlayerID.P1 ? 'bg-cyan-600' : 'bg-red-600'}`} />
      ...
      {(isSmoked || isHub31Smoke) && (
        <div className="absolute inset-0 z-40 pointer-events-none overflow-hidden rounded-sm">
          <div className={`absolute inset-0 ${(isHub31Smoke ? hubBoundaryBuilding!.owner : smokeOwner) === PlayerID.P1 ? 'bg-cyan-900/40' : 'bg-red-900/40'} mix-blend-overlay`} />
