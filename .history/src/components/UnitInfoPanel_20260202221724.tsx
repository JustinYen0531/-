    ...
    return (
        <div className="flex-[7] flex flex-col border-l-2 border-white/30 px-4 items-center h-full justify-center bg-slate-800/30">
            ...
            <div className="flex gap-4 justify-center w-full">
                {/* Use unitDisplayOrder to render units in custom order */}
                {player.unitDisplayOrder.map((unitId, index) => {
                    const u = player.units.find(unit => unit.id === unitId);
                    if (!u) return null;
                    ...
                    // Calculate current action position (X-1 to X-5)
                    // During action phase, highlight the current turn position
                    const actedCount = player.units.filter(unit => unit.hasActedThisRound).length;
                    const currentActionIndex = actedCount; // 0-indexed: if 0 have acted, current is position 0
                    const isCurrentTurn = !isPlacement && gameState.phase === 'action' && index === currentActionIndex;

                    // Waiting units: not current turn, not acted yet, not dead (slightly dimmed with different border)
                    const isWaiting = !isPlacement && gameState.phase === 'action' && index !== currentActionIndex && !u.hasActedThisRound && !u.isDead;

                    return (
                        <div key={u.id} className="flex flex-col items-center gap-1">
                            <button
                                disabled={u.isDead || u.hasActedThisRound || isWaiting}
                                ...
                                className={`
                                    ...
                                    ${isCurrentTurn && !isDragging && !isDragOver ? 'ring-2 ring-emerald-400 ring-offset-2 ring-offset-slate-900 shadow-[0_0_20px_rgba(16,185,129,0.5)]' : ''}
                                    ${isWaiting && !isDragging && !isDragOver ? 'brightness-90 border-dashed border-amber-600/70' : ''}
                                    ${u.isDead ? 'opacity-30 grayscale cursor-not-allowed bg-red-950/50 border-red-600' : ''}
                                    ${u.hasActedThisRound ? `opacity-60 cursor-not-allowed border-slate-500 ${u.owner === PlayerID.P1 ? 'bg-slate-800/60' : 'bg-slate-800/60'}` : ''}
                                    ${!isDragging && !isDragOver && !isWaiting && canSwap
                                        ? 'bg-emerald-500/20 border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.3)]'
                                        : !isDragging && !isDragOver && !isWaiting && gameState.selectedUnitId === u.id
                                            ? 'bg-cyan-900/80 border-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.6)] scale-110 z-10'
                                            : !isDragging && !isDragOver && !isWaiting && u.owner === PlayerID.P1
                                                ? 'bg-cyan-900/40 border-slate-600 hover:bg-cyan-900/60 hover:border-cyan-500'
                                                : !isDragging && !isDragOver && !isWaiting ? 'bg-red-900/40 border-slate-600 hover:bg-red-900/60 hover:border-red-500' : ''
                                    }
                                `}
                            >
