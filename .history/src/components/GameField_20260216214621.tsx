            ...
            <div ref={boardRef} className="grid gap-0 border-4 border-white bg-slate-900 rounded-lg overflow-visible relative z-10 shadow-[0_0_10px_rgba(255,255,255,0.3)]"
                ...
                {gameState.cells.map((row, r) => row.map((cell, c) => {
                    ...
                    const cellSensorResults = gameState.sensorResults?.filter(sr =>
                        sr.r === r && sr.c === c && sr.owner === gameState.currentPlayer
                    ) || [];
                    ...
                    return (
                        <div key={`${r}-${c}`} className="relative">
                            <GridCell
                                ...
                                onClick={() => {
                                    // Dismiss MISS marker IMMEDIATELY on click (standalone call, no routing).
                                    if (markResult?.success === false && onDismissMiss) {
                                        onDismissMiss(r, c);
                                        return;
                                    }
