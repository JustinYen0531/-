                ...
                {gameState.cells.map((row, r) => row.map((cell, c) => {
                    ...
                    return (
                        <div key={`${r}-${c}`} className="relative">
                            ...
                            />
                            {sResult && (
                                <div className="absolute inset-0 pointer-events-none z-[60] flex flex-col items-center justify-center translate-x-1 -translate-y-1">
                                    {/* Map Pin Floating Container */}
                                    <div className="relative mb-6 animate-float-pin flex flex-col items-center">
                                        {/* Pin Body (SVG) */}
                                        <div className="relative drop-shadow-[0_4px_6px_rgba(0,0,0,0.5)]">
                                            <svg width="32" height="40" viewBox="0 0 384 512" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M172.268 501.67C26.97 291.031 0 269.413 0 192C0 85.961 85.961 0 192 0C298.039 0 384 85.961 384 192C384 269.413 357.03 291.031 211.732 501.67C191.95 530.41 152.48 530.41 172.268 501.67Z" fill="#22d3ee" />
                                                <circle cx="192" cy="192" r="120" fill="white" />
                                            </svg>
                                            {/* Number inside the pin */}
                                            <div className="absolute top-0 left-0 w-full h-[30px] flex items-center justify-center text-cyan-700 font-extrabold text-lg">
                                                {sResult.count}
                                            </div>
                                        </div>
                                    </div>
                                    {/* Ground Shadow */}
                                    <div className="absolute bottom-2 w-4 h-1.5 bg-black/40 rounded-[100%] blur-[1px] animate-shadow-pulse"></div>
                                </div>
                            )}
                        </div>
                    );
