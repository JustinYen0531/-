                                                    ...
                                                    if (canTeleport) {
                                                        buttons.push(
                                                            <div key="teleport" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => { handleTeleportToHubAction(unit); setEndTurnConfirm(false); }} className={`w-[82px] h-[74px] rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'teleport' && !endTurnConfirm ? 'bg-indigo-600 border-indigo-400 scale-105 shadow-lg shadow-indigo-500/50 text-white' : 'bg-indigo-900/40 hover:bg-indigo-800/60 border-indigo-800/50 text-indigo-100/70'}`}>
                                                                    <div className="absolute top-0.5 left-1.5 text-sm font-black text-white/90">{helpers.getActionButtonIndex('teleport', unit)}</div>
                                                                    <Zap size={28} /> <span className="text-xs">傳送歸還</span>
                                                                </button>
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white">⚡ {helpers.getDisplayCost(unit, (rangerLevelA === 3 && rangerVariantA === 2) ? 4 : (rangerLevelA >= 2 ? 0 : 6), gameState, 'teleport')}</div>
                                                            </div>
                                                        );
                                                    }
                                                    return buttons;
