    ...
    >
      ...
      {/* Persistent Smoke Effect (from Smoke Mines or Hub 3-1) */}
      {
        (isSmoked || isHub31Smoke) && (
          <div className="absolute inset-0 pointer-events-none z-10 opacity-60 overflow-hidden">
            <div className="relative w-full h-full">
              {/* Cloud A - Top Left drift */}
              <Cloud
                size={22}
                className={`absolute animate-[smoke_4s_infinite_ease-out] ${(isHub31Smoke ? hubBoundaryBuilding?.owner : smokeOwner) === PlayerID.P1 ? 'text-cyan-200' : 'text-red-200'
                  }`}
              ...
              <Cloud
                size={26}
                className={`absolute animate-[smoke_5s_infinite_0.8s_ease-out] opacity-80 ${(isHub31Smoke ? hubBoundaryBuilding?.owner : smokeOwner) === PlayerID.P1 ? 'text-cyan-100' : 'text-red-100'
                  }`}
