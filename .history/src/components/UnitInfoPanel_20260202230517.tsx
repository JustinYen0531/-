...
const UnitInfoPanel: React.FC<UnitInfoPanelProps> = ({ gameState, t, onUnitClick, language, onSwapUnits }) => {
    ...
    return (
        <div className="flex-[7] flex flex-col border-l-2 border-white/30 px-4 items-center h-full justify-center bg-slate-800/30">
            ...
            <div className="flex gap-4 justify-center w-full">
                {(() => {
                    // Sort units for visual representation:
                    // 1. Acted units (Left) - Fixed
                    // 2. Active unit (Green Box) - Current Turn
                    // 3. Waiting units (Orange Dashed Box -> Colored Dashed Box)
                    // 4. Dead units (End)
                    const actedUnits = player.units.filter(u => u.hasActedThisRound && !u.isDead);
                    const deadUnits = player.units.filter(u => u.isDead);

                    // Identify the "Current Turn" unit index based on how many have acted
                    // In the original logic, index === actedCount determines the active slot
                    // Since we are reordering, the 'Active' unit is the one that SHOULD act next
                    // However, 'isCurrentTurn' in the original code relied on rendering index
                    // Let's deduce the "Active" unit as the one that triggers 'isCurrentTurn' logic conceptually
                    // If we simply sort, the unit at the 'active' position (after all acted units) is the active one.

                    // Filter remaining units (not acted, not dead)
                    const remainingUnits = player.units.filter(u => !u.hasActedThisRound && !u.isDead);

                    // Stable sort for remaining units to keep Q,W,E,R,T order relative to each other if possible?
                    // Actually, let's just stick to the text-based hotkey (Q=General) so order doesn't confuse selection
                    const unitOrder = [UnitType.GENERAL, UnitType.MINESWEEPER, UnitType.RANGER, UnitType.MAKER, UnitType.DEFUSER];
                    remainingUnits.sort((a, b) => unitOrder.indexOf(a.type) - unitOrder.indexOf(b.type));
                    actedUnits.sort((a, b) => unitOrder.indexOf(a.type) - unitOrder.indexOf(b.type));

                    // If we are in action phase, the FIRST remaining unit is the "Current Active" one
                    let activeUnit: Unit | null = null;
                    let waitingUnits: Unit[] = [];

                    if (remainingUnits.length > 0) {
                        activeUnit = remainingUnits[0]; // The first available unit is 'Active' (Green)
                        waitingUnits = remainingUnits.slice(1);
                    }

                    // Combine into display list
                    // Note: If phase is placement, we might want original order? 
                    // User said "Finished unit is forever on the left", implies permanently. 
                    // But in placement, no one has acted. So it's just order.

                    let displayList: Unit[] = [];
                    if (isPlacement) {
                        // Keep original order in placement for dragging?
                        // Or sort by type? Let's use player.unitDisplayOrder map for placement to allow swapping
                        displayList = player.unitDisplayOrder.map(id => player.units.find(u => u.id === id)).filter((u): u is Unit => !!u);
                    } else {
                    ...
                    return displayList.map((u, index) => {
                        ...
                        return (
                            <div key={u.id} className="flex flex-col items-center gap-1">
                                <button
                                    disabled={u.isDead || isActed || isWaiting}
                                    onClick={() => onUnitClick(u)}
                                    draggable={!u.isDead && (isPlacement || (gameState.phase === 'action' && !gameState.activeUnitId))}
                                    ...
                                    className={`
                                        ...
                                        ${!u.isDead && (isPlacement || (gameState.phase === 'action' && !gameState.activeUnitId)) ? 'cursor-grab active:cursor-grabbing' : ''}
                                        ...
                                        ${isActive && !isDragging && !isDragOver ? 'ring-2 ring-emerald-400 ring-offset-2 ring-offset-slate-900 shadow-[0_0_20px_rgba(16,185,129,0.5)]' : ''}
                                        ${isWaiting && !isDragging && !isDragOver ? `brightness-90 border-dashed ${playerColorBorder} ${playerColorBg}` : ''}
