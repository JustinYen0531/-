...
const GridCell: React.FC<GridCellProps> = ({
  ...
  return (
    ...
    >
      ...
      {scanMarkSuccess === false && (
        <div className="absolute top-1 left-1/2 -translate-x-1/2 z-20 group/miss">
          <div className="relative px-1.5 py-0.5 rounded text-[10px] font-black border bg-red-600/90 text-red-50 border-red-200 pointer-events-auto cursor-default">
            MISS
            {/* Close button - appears on hover at top-right */}
            {onDismissMiss && (
              <button
                onClick={(e) => { e.stopPropagation(); onDismissMiss(); }}
                className="absolute -top-2 -right-2 w-4 h-4 rounded-full bg-slate-800 border border-red-300 text-red-200 flex items-center justify-center text-[8px] font-bold leading-none opacity-0 group-hover/miss:opacity-100 transition-opacity duration-150 cursor-pointer hover:bg-red-700 hover:border-white hover:text-white shadow-md"
                title="關閉"
              >
                ✕
              </button>
