                                            ...
                                            <div className="flex gap-2 justify-center flex-wrap">
                                                        ...
                                                        buttons.push(
                                                            <div key="place_tower" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => handlePlaceTowerAction(unit, unit.r, unit.c)} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 text-slate-300">
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('place_tower', unit)}</div>
                                                        ...
                                                        buttons.push(
                                                            <div key="place_factory" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => handlePlaceFactoryAction(unit, unit.r, unit.c)} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 text-slate-300">
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('place_factory', unit)}</div>
                                                        ...
                                                        buttons.push(
                                                            <div key="place_hub" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => handlePlaceHubAction(unit, unit.r, unit.c)} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 text-slate-300">
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('place_hub', unit)}</div>
                                                    ...
                                                    if (unit.type === UnitType.GENERAL) {
                                                            ...
                                                            buttons.push(
                                                                <div key="attack" className="flex flex-col items-center gap-1">
                                                                    <button
                                                                        ...
                                                                        className={`w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'attack' ? 'bg-red-600 shadow-lg shadow-red-500/50 scale-105 border-red-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-red-500 text-slate-300'}`}
                                                                    >
                                                        ...
                                                        buttons.push(
                                                            <div key="scan" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => setTargetMode('scan')} className={`w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'scan' ? 'bg-cyan-600 border-cyan-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'}`}>
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('scan', unit)}</div>
                                                            ...
                                                            <div key="place_mine_group" className="flex flex-col items-center gap-1 relative">
                                                                {targetMode === 'place_mine' && (
                                                                    <div className="absolute bottom-full mb-2 bg-slate-900 border-2 border-purple-500 rounded-lg p-1.5 flex gap-1.5 shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-2">
                                                                        {[MineType.NORMAL, MineType.SMOKE, MineType.CHAIN, MineType.SLOW, MineType.NUKE].map(type => {
                                                                            ...
                                                                            return (
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    {type === MineType.SMOKE && <Cloud size={16} className="text-gray-400" />}
                                                                                    {type === MineType.SLOW && <Snowflake size={16} className="text-blue-400" />}
                                                                                    {type === MineType.CHAIN && <Share2 size={16} className="text-yellow-400" />}
                                                                                    {type === MineType.NUKE && <Radiation size={16} className="text-red-500" />}
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'place_mine' ? 'bg-purple-600 border-white ring-2 ring-purple-400 ring-offset-2 ring-offset-slate-900' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'}`}
                                                                >
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('place_mine', unit)}</div>
                                                                    <Bomb size={20} /> <span className="text-[10px]">{t('place_mine')}</span>
                                                                </button>
                                                                <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                    <span className="text-yellow-400">⚡</span>
                                                        ...
                                                        if (!unit.carriedMine && mineInRange) {
                                                            buttons.push(
                                                                <div key="pickup_mine" className="flex flex-col items-center gap-1">
                                                                    <button onClick={() => handleRangerAction('pickup')} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 text-slate-300">
                                                                        <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('pickup_mine', unit)}</div>
                                                                        <Bomb size={20} /> <span className="text-[10px]">拾取地雷</span>
                                                                    </button>
                                                        ...
                                                        if (unit.carriedMine) {
                                                            buttons.push(
                                                                <div key="throw_mine" className="flex flex-col items-center gap-1">
                                                                    <button onClick={() => setTargetMode('throw_mine')} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 text-slate-300">
                                                                        <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('throw_mine', unit)}</div>
                                                                        <Bomb size={20} /> <span className="text-[10px]">投擲地雷</span>
                                                                    </button>
                                                                    <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">⚡ {helpers.getDisplayCost(unit, 3)}</div>
                                                                </div>
                                                            );
                                                            buttons.push(
                                                                <div key="drop_mine" className="flex flex-col items-center gap-1">
                                                                    <button onClick={() => handleRangerAction('drop')} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-yellow-600/70 hover:bg-yellow-600 border-yellow-500 text-white">
                                                                        <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('drop_mine', unit)}</div>
                                                        ...
                                                        buttons.push(
                                                            <div key="disarm" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => setTargetMode('disarm')} className={`w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'disarm' ? 'bg-cyan-600 border-cyan-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'}`}>
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{helpers.getActionButtonIndex('disarm', unit)}</div>
                                                        ...
                                                        if (!unit.hasFlag && isAtFlag) {
                                                            buttons.push(
                                                                <div key="pickup_flag" className="flex flex-col items-center gap-1">
                                                                    <button onClick={actions.handlePickupFlag} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative bg-yellow-600/70 hover:bg-yellow-600 border-2 border-yellow-500 cursor-pointer text-white">
                                                        ...
                                                        if (unit.hasFlag) {
                                                            buttons.push(
                                                                <div key="drop_flag" className="flex flex-col items-center gap-1">
                                                                    <button onClick={actions.handleDropFlag} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative bg-yellow-600/70 hover:bg-yellow-600 border-2 border-yellow-500 shadow-lg shadow-yellow-500/50 text-white">
                                                    ...
                                                    if (canEvolveA) {
                                                        evolveButtons.push(
                                                            <div key="evolve_a" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => actions.handleEvolve(unit.type, 'a')} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 bg-blue-600 hover:bg-blue-500 font-bold border-2 border-blue-400 shadow-lg text-white">
                                                    ...
                                                    if (canEvolveB) {
                                                        evolveButtons.push(
                                                            <div key="evolve_b" className="flex flex-col items-center gap-1">
                                                                <button onClick={() => actions.handleEvolve(unit.type, 'b')} className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 bg-orange-600 hover:bg-orange-500 font-bold border-2 border-orange-400 shadow-lg text-white">
                                                ...
                                                <div className="flex flex-col items-center gap-1">
                                                    <button
                                                        ...
                                                        className="w-[74px] h-[74px] p-1 rounded flex flex-col items-center justify-center gap-1 transition-all relative bg-slate-600 hover:bg-slate-500 font-bold border-2 border-slate-500 text-slate-200"
                                                    >
