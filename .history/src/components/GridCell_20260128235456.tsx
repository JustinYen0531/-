      ...
      {/* Persistent Smoke Effect (from Smoke Mines or Hub 3-1) */}
      {
        (isSmoked || isHub31Smoke) && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 opacity-70">
            <Cloud
              size={36}
              className={`drop-shadow-lg animate-[gentleBreathe_4s_infinite_ease-in-out] ${(isHub31Smoke ? hubBoundaryBuilding?.owner : smokeOwner) === PlayerID.P1 ? 'text-cyan-200' : 'text-red-200'
                }`}
            />
          </div>
        )
