...
const ControlPanel: React.FC<ControlPanelProps> = ({
    ...
    const selectedUnit = helpers.getUnit(gameState.selectedUnitId);
                                ...
                                <div className="flex gap-2 justify-center items-start w-full px-4 py-2 overflow-visible">
                                    ...
                                    <div className="flex flex-col items-center gap-1">
                                        <button
                                            ...
                                            className={`w-[82px] h-[74px] rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'move' && !endTurnConfirm
                                    ...
                                    {(() => {
                                        ...
                                        if (unit.type !== UnitType.DEFUSER && gameState.buildings.some(b => b.r === unit.r && b.c === unit.c && b.owner !== unit.owner)) {
                                            buttons.push(
                                                <div key="custom_dismantle" className="flex flex-col items-center gap-1">
                                                    <button onClick={() => handleDisarmAction(unit, unit.r, unit.c)} className="w-[82px] h-[74px] rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-indigo-500 text-slate-300">
                                        ...
                                        if (unit.type === UnitType.GENERAL) {
                                            ...
                                            // Also allow Minesweeper to place basic mines
                                            buttons.push(
                                                <div key="place_mine_group_swp" className="flex flex-col items-center gap-1">
                                                    <button
                                                        onClick={() => {
                                                        ...
                                                        className={`w-[82px] h-[74px] rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'place_mine' && !endTurnConfirm
                                                    ...
                                                    <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white">⚡ {helpers.getDisplayCost(unit, getMineBaseCost(MineType.NORMAL))}</div>
                                                </div>
                                            );
                                        } else if (unit.type === UnitType.MAKER) {
                                            // Place Mine button (Popup centered relative to this container)
                                            buttons.push(
                                                <div key="place_mine_group" className="flex flex-col items-center gap-1 relative overflow-visible">
                                                    {targetMode === 'place_mine' && (
                                                        <div className="absolute bottom-[95px] left-1/2 -translate-x-1/2 bg-slate-900 border-2 border-purple-500 rounded-lg p-1.5 flex gap-1.5 shadow-[0_0_20px_rgba(168,85,247,0.6)] z-[60] min-w-max animate-in fade-in slide-in-from-bottom-2">
                                                            {[MineType.NORMAL, MineType.SLOW, MineType.SMOKE, MineType.CHAIN, MineType.NUKE].map(type => {
                                                                ...
                                                                return (
                                                                    <button
                                                                        ...
                                                                        className={`p-1.5 rounded border-2 transition-all flex flex-col items-center gap-0.5 min-w-[44px] ${selectedMineType === type ? 'bg-purple-600 border-purple-300 scale-105 shadow-[0_0_8px_rgba(168,85,247,0.4)]' : 'bg-slate-800 border-slate-700 hover:border-purple-400'}`}
                                                                    >
                                                                        {type === MineType.NORMAL && <Bomb size={18} className="text-white" />}
                                                                        {type === MineType.SLOW && <Snowflake size={18} className="text-blue-200" />}
                                                                        {type === MineType.SMOKE && <Cloud size={18} className="text-slate-300" />}
                                                                        {type === MineType.CHAIN && <Share2 size={18} className="text-purple-400" />}
                                                                        {type === MineType.NUKE && <Radiation size={18} className="text-emerald-400" />}
