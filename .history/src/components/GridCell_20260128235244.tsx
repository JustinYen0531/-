...
import { Cell, Unit, Mine, PlayerID, UnitType, Building, MineType } from '../types';
...
const GridCell: React.FC<GridCellProps> = ({
    ...
    if (building.type === 'tower') {
      return (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 overflow-visible">
          ...
          <div className="building-range-indicator" style={{ borderColor: `${accentColor}20` }} />
          ...
          <div className={`relative w-8 h-8 flex items-center justify-center`}>
            ...
            {(building.duration ?? 0) > 0 && (
              <div className={`absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-slate-800 border border-${building.owner === PlayerID.P1 ? 'cyan' : 'red'}-400 flex items-center justify-center text-[8px] font-black text-white shadow-lg z-30`}>
                {building.duration}
              </div>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      {/* Persistent Smoke Effect (from Smoke Mines or Hub 3-1) */}
      {
        (isSmoked || isHub31Smoke) && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 opacity-70 overflow-visible">
            <div className="relative w-full h-full flex items-center justify-center">
              <Cloud size={32} className={`drop-shadow-lg animate-[smoke_3s_infinite_ease-out] ${(isHub31Smoke ? hubBoundaryBuilding?.owner : smokeOwner) === PlayerID.P1 ? 'text-cyan-200' : 'text-red-200'
                }`} />
              <Cloud size={24} className={`absolute drop-shadow-md animate-[smoke_4s_infinite_0.5s_ease-out] opacity-60 ${(isHub31Smoke ? hubBoundaryBuilding?.owner : smokeOwner) === PlayerID.P1 ? 'text-cyan-100' : 'text-red-100'
