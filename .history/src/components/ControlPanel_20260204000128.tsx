...
import {
    ...
    Cpu, Cloud, Snowflake, Share2, Radiation, Ghost, Scan, Heart, Hand
} from 'lucide-react';
import {
                                                    ...
                                                    if (canPlaceTower) {
                                                        buttons.push(
                                                            <div key="place_tower" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, 8, gameState, 'place_tower')}</div>
                                                            </div>
                                                        );
                                                    }
                                                    if (canPlaceFactory) {
                                                        ...
                                                        buttons.push(
                                                            <div key="place_factory" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, (mkrB === 3 && player.evolutionLevels[UnitType.MAKER].bVariant === 2) ? 4 : 6, gameState, 'place_factory')}</div>
                                                            </div>
                                                        );
                                                    }
                                                    if (canPlaceHub) {
                                                        buttons.push(
                                                            <div key="place_hub" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, 8, gameState, 'place_hub')}</div>
                                                            </div>
                                                        );
                                                    }
                                                    ...
                                                    if (unit.type === UnitType.GENERAL) {
                                                        ...
                                                        buttons.push(
                                                            <div key="scan" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, 3, gameState, 'scan')}</div>
                                                            </div>
                                                        );
                                                        ...
                                                        if (swpLevelB >= 1) {
                                                            ...
                                                            buttons.push(
                                                                <div key="sensor_scan" className="flex flex-col items-center gap-1">
                                                                    ...
                                                                    <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, finalCost, gameState, 'sensor_scan')}</div>
                                                                </div>
                                                            );
                                                        }
                                                            ...
                                                            if (rngLevelB === 3 && rngVariantB === 2) {
                                                                buttons.push(
                                                                    <div key="throw_mine" className="flex flex-col items-center gap-1">
                                                                        ...
                                                                        <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, 4, gameState, 'throw_mine')}</div>
                                                                    </div>
                                                                );
                                                            }
                                                            buttons.push(
                                                        ...
                                                        buttons.push(
                                                            <div key="disarm" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, 3, gameState, 'disarm')}</div>
                                                            </div>
                                                        );
                                                        ...
                                                        if (defLevelB >= 2) {
                                                            ...
                                                            buttons.push(
                                                                <div key="move_mine" className="flex flex-col items-center gap-1">
                                                                    <button onClick={() => handleSetTargetMode('move_mine_start')} className={`w-[82px] h-[74px] rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${(targetMode === 'move_mine_start' || targetMode === 'move_mine_end') && !endTurnConfirm ? 'bg-fuchsia-600 shadow-lg shadow-fuchsia-500/50 scale-105 border-fuchsia-400 text-white font-bold' : 'bg-fuchsia-900/40 hover:bg-fuchsia-800/60 border-fuchsia-800/50 text-fuchsia-100/70'}`}>
                                                                        <div className="absolute top-0.5 left-1.5 text-sm font-black text-white/90">{helpers.getActionButtonIndex('move_mine', unit)}</div>
                                                                        <Hand size={28} /> <span className="text-xs">{isDamageMode ? '苦痛轉嫁' : '空間置換'}</span>
                                                                    </button>
                                                        ...
                                                        if (defLevelB === 3 && defVariantB === 1) {
                                                            buttons.push(
                                                                <div key="convert_mine" className="flex flex-col items-center gap-1">
                                                                    <button onClick={() => handleSetTargetMode('convert_mine')} className={`w-[82px] h-[74px] rounded flex flex-col items-center justify-center gap-1 transition-all relative font-bold border-2 ${targetMode === 'convert_mine' && !endTurnConfirm ? 'bg-indigo-600 shadow-lg shadow-indigo-500/50 scale-105 border-indigo-400 text-white font-bold' : 'bg-indigo-900/40 hover:bg-indigo-800/60 border-indigo-800/50 text-indigo-100/70'}`}>
                                                                        ...
                                                                        <Hand size={28} className="rotate-180" /> <span className="text-xs">意志侵奪</span>
                                                                    </button>
                                                    ...
                                                    if (canTeleport) {
                                                        buttons.push(
                                                            <div key="teleport" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-2 py-1 flex items-center gap-1 text-xs font-bold text-white"><Zap size={12} className="text-yellow-400" /> {helpers.getDisplayCost(unit, unit.type === UnitType.RANGER ? ((rangerLevelA === 3 && rangerVariantA === 2) ? 4 : 0) : 6, gameState, 'teleport')}</div>
                                                            </div>
                                                        );
                                                    }
                                                    return buttons;
