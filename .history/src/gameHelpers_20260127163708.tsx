...
export const getDisplayCost = (unit: Unit | null, baseCost: number, state: GameState, actionType: string = 'move'): number => {
    ...
    export const getActionButtonIndex = (actionType: string, unit: Unit | null | undefined, state: GameState): number => {
        ...
        if (actionType.startsWith('evolve_')) {
            ...
            // if evolve_a was present, increment
            if (unit.type) {
                const levelA = player.evolutionLevels[unit.type].a;
                // ... this logic is a bit complex to duplicate perfectly without just checking the specific actionType
                // For now, let's just say if it's an evolution button, it uses the current index.
                // But wait, there can be TWO evolution buttons (A and B).
            }
            // Actually, the simplest way is to let the caller handle it or just return the base index for A and index+1 for B.
        }
