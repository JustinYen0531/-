...
const isReadyPayload = (payload: any): payload is ReadyPayload => {
    return payload && typeof payload.playerId === 'string' && typeof payload.phase === 'string';
};
...
export default function App() {
    ...
    const finishPlacementPhase = () => {
        ...
        const localPlayer = (state.gameMode === 'pvp' && roomId && isNetworkConnected)
        ...
        if (state.gameMode === 'pvp' && roomId && isNetworkConnected) {
            sendActionPacket({
                ...
                payload: { playerId: localPlayer, phase: 'placement' } as ReadyPayload
            });
        }
    ...
    const startActionPhase = () => {
        const state = gameStateRef.current;
        const localPlayer = (roomId && isNetworkConnected)
            ? (isHost ? PlayerID.P1 : PlayerID.P2)
            : state.currentPlayer;
        ...
        if (state.gameMode === 'pvp' && roomId && isNetworkConnected) {
            sendActionPacket({
                ...
                matchId: roomId,
                turn: state.turnCount,
                payload: { playerId: localPlayer, phase: 'thinking' } as ReadyPayload
            });
        }
    ...
    const handleUnitClick = (unit: Unit) => {
        ...
        if (state.phase === 'placement') {
            const actingPlayer = (state.gameMode === 'pvp' && roomId && isNetworkConnected)
                ? (isHost ? PlayerID.P1 : PlayerID.P2)
                : state.currentPlayer;
    ...
    */
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (type === 'PLAYER_READY') {
                ...
                setGameState(prev => {
                    const readyPlayer = payload.playerId as PlayerID;
                    const newReadyState = {
                        [PlayerID.P1]: prev.pvpReadyState?.[PlayerID.P1] ?? false,
                        [PlayerID.P2]: prev.pvpReadyState?.[PlayerID.P2] ?? false,
                        [readyPlayer]: true
                    ...
                    console.log(`Received READY from ${readyPlayer}. State:`, newReadyState, 'Phase:', prev.phase);

                    // Check if phase transition is needed
                    if (bothReady) {
                        if (prev.phase === 'placement' && payload.phase === 'placement') {
                            return {
                                ...prev,
                                phase: 'thinking',
                                timeLeft: THINKING_TIMER,
                        ...
                        } else if (prev.phase === 'thinking' && payload.phase === 'thinking') {
                            const updatedMines = applyRadarScans(prev);
                            return {
                                ...prev,
                                phase: 'action',
    ...
    const executeCellClickLogic = (r: number, c: number) => {
        ...
        if (state.phase === 'placement') {
            ...
            const actingPlayerId = (state.gameMode === 'pvp' && roomId && isNetworkConnected)
                ? (isHost ? PlayerID.P1 : PlayerID.P2)
                : state.currentPlayer;
            ...
            if (targetMode === 'place_setup_mine') {
                ...
                setGameState(prev => {
                    ...
                    return {
                        ...prev,
                        mines: [...prev.mines, { id: `pm-${Date.now()}`, owner: actingPlayerId, type: MineType.NORMAL, r, c, revealedTo: [] }],
                        players: {
                            ...prev.players,
            ...
            {view === 'game' && (
              ...
              `}</style>
                    ...
                    <ControlPanel
                        ...
                        localPlayerId={localNetworkPlayer ?? PlayerID.P1}
                        t={t}
