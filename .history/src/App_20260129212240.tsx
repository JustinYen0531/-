...
export default function App() {
    ...
    const handleStealthAction = (unitId: string) => {
        const unit = getUnit(unitId);
        if (!unit) return;
        const _player = gameState.players[unit.owner];
        ...
        setGameState(prev => {
            ...
            return {
                ...
                players: {
                    ...prev.players,
                    [unit.owner]: { ...p, units: updatedUnits } // No energy deduction
                },
                lastActionTime: Date.now(),
                isTimeFrozen: true
            };
    ...
    const handleThrowMineAction = (unit: Unit, r: number, c: number) => {
        ...
        setGameState(prev => {
            ...
            // Handle special mine types (e.g. Smoke)
            const mType = unit.carriedMine!.type;
            if (mType === MineType.SMOKE) {
                const newSmoke = {
                    id: `smoke-${Date.now()}`,
                    r, c,
                    owner: unit.owner,
                ...
                newState.smokes = [...(prev.smokes || []), newSmoke];
            }
        ...
        const mType = unit.carriedMine!.type;
        if (mType === MineType.SMOKE) addVFX('smoke', r, c);
        else if (mType === MineType.NUKE) addVFX('nuke', r, c);
        else addVFX('explosion', r, c);
    ...
    const handleDisarmAction = (unit: Unit, r: number, c: number) => {
        ...
        // 2. Range & Cost Check
        const dist = Math.max(Math.abs(unit.r - r), Math.abs(unit.c - c));
        let baseCost = 3;
        let requiredRange = 0;

        if (actionType === 'mine') {
            const stats = UNIT_STATS[unit.type] as any;
            baseCost = stats.disarmCost || 3;
            // Defuser ranges
            requiredRange = (unit.type === UnitType.DEFUSER) ? (defLevelB >= 1 ? 2 : 1) : 0;
        } else {
            // Building Dismantle
            baseCost = 2; // Fixed cost for dismantling buildings
            requiredRange = 0; // Must be standing on it
        }
    ...
    const getActionButtonIndex = (actionType: string, unit: Unit | null | undefined): number => {
        ...
        let actions: string[] = ['move'];
        const player = gameState.players[unit.owner];

        // Placement (Minesweeper/Maker/Ranger)
        if (unit.type === UnitType.MINESWEEPER && player.evolutionLevels[UnitType.MINESWEEPER].a >= 1) actions.push('place_tower');
        if (unit.type === UnitType.MAKER && player.evolutionLevels[UnitType.MAKER].b >= 1) actions.push('place_factory');
        if (unit.type === UnitType.RANGER && player.evolutionLevels[UnitType.RANGER].a >= 1) actions.push('place_hub');

        // Custom Dismantle
        const isOnEnemyBuilding = gameState.buildings.some(b => b.r === unit.r && b.c === unit.c && b.owner !== unit.owner);
        if (isOnEnemyBuilding) actions.push('custom_dismantle');
        ...
        if (unit.type === UnitType.GENERAL) {
            ...
            // Attack Button Condition: Not holding flag OR (Level 3-1) OR GodMode
            const canAttack = !unit.hasFlag || (genLevelA >= 3 && genVariantA === 1) || (gameState as any).isGodMode;
            if (canAttack) actions.push('attack');
        } else if (unit.type === UnitType.MINESWEEPER) {
            actions.push('scan');
        } else if (unit.type === UnitType.MAKER) {
            actions.push('place_mine');
            ...
            if (!unit.carriedMine && mineInRange) actions.push('pickup_mine');

            // Stealth (Level 2 or 3-2)
            if (rngLevelB === 2 || (rngLevelB >= 3 && rngVariantB === 2)) {
                actions.push('stealth');
            }

            // Throw / Drop
            if (unit.carriedMine) {
                if (rngLevelB === 3 && rngVariantB === 2) actions.push('throw_mine');
                actions.push('drop_mine');
            }
            ...
            if (defLevelB >= 2) actions.push('move_mine_start');
            if (defLevelB === 3 && defVariantB === 1) actions.push('convert_mine');
        }

        // Flag (General or High-Level Gen-B)
        const genLevelB = player.evolutionLevels[UnitType.GENERAL].b;
        // Hotfix check: Restrict flag logic to General only to diagnose Ghost Button 3 on Minesweeper
        const canCarryFlag = unit.type === UnitType.GENERAL; // || genLevelB >= 3;
        if (canCarryFlag) {
            // Re-read player flag position in case of updates? It should be in gameState.players
            const isAtFlag = unit.r === player.flagPosition.r && unit.c === player.flagPosition.c;
            if (!unit.hasFlag && isAtFlag) actions.push('pickup_flag');
            if (unit.hasFlag) actions.push('drop_flag');
        }
        ...
        // Fix: Restrict logic to Ranger only to prevent "Ghost" action for other units
        const canTeleport = unit.type === UnitType.RANGER && (rangerLevelA >= 2 || (rangerLevelA === 3 && rangerVariantA === 2)) && hasHub;
        if (canTeleport) actions.push('teleport');

        // End Turn
        actions.push('end_turn');

        // DEBUG: Log the actions array to help diagnose hotkey issues
        if (actionType === 'end_turn') {
            console.log('[DEBUG] Actions list for', unit.type, ':', JSON.stringify(actions));
            console.warn('[DEBUG-WARN] Actions:', JSON.stringify(actions));
        }
    ...
    */
    ...
    }, [targetMode, addLog, handleUnitClick, playerActions, selectedMineType, selectedMineId, setSelectedMineId, setTargetMode, setGameState]); // Re-create when dependencies change
