...
export default function App() {
    ...
    const handleUnitClick = (unit: Unit) => {
        ...
        if (state.phase === 'thinking') {
            ...
            // Swap with the unit currently at the top of the squad panel (ready to act)
        ...
        if (unit.owner === state.currentPlayer) {
            ...
            // In action phase, before committing (activeUnitId is null), allow swapping unit order
            // Even if a unit is already selected, as long as no action is taken, allow switching
            if (state.phase === 'action' && !state.activeUnitId) {
                const player = state.players[state.currentPlayer];
                ...
                // If clicking a unit that is NOT the current first-in-line, swap it to first
                if (firstAliveId && firstAliveId !== unit.id) {
                    swapUnitDisplayOrder(firstAliveId, unit.id);
                    // Also update selection to the new unit
                    setGameState(prev => ({ ...prev, selectedUnitId: unit.id }));
                    setTargetMode('move');
                    return;
                }
