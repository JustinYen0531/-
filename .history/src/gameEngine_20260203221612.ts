...
export const calculateMineInteraction = (
    ...
    // "Whole team takes 1 less damage, 2 less if HP < 50%"
    if (defLevelA >= 1) {
        let reduction = 1;
        if (unit.hp < unit.maxHp * 0.5) reduction = 2;

        if (defLevelA >= 3 && defVariantA === 2) {
            // A3-2: Team reduction 3, 4 if HP < 50%
            reduction = 3;
            if (unit.hp < unit.maxHp * 0.5) reduction = 4;
        }
    ...
    if (unit.type === UnitType.DEFUSER) {
        ...
        if (defLevelA >= 3 && defVariantA === 2) {
            // Re-apply reduction logic: Base dmg was halved (50%), now set to 25% (75% reduction)
            // But we already applied 50% above.
            // Let's reset dmg to raw MINE_DAMAGE before team reduction?
            // No, the trait sets damage.
            // Logic: Base Dmg (e.g. 8) -> 50% (4).
            // A3-2 wants 75% reduction. So 8 -> 2.
            // So we apply another 50%? Or simpler:
            // "Defuser dmg reduction increased to 75%".
            // We need access to original raw damage to be precise, or just apply factor.
            // Let's assume factor 0.5 was applied at line 200.
            // Apply another 0.5? 0.5 * 0.5 = 0.25 (25% taken). Correct.
            dmg = Math.floor(dmg * 0.5);
        }

        // A2: Heal/MaxHP when triggering mine
        if (defLevelA >= 2) {
            result.newMaxHpBonus = 2;
            result.healAmount = 1;
            result.logKeys.push('log_evol_def_a_heal'); // Ensure this key exists or map to log_defuser_self_heal
        }
    }
    ...
    if (defLevelA >= 3 && defVariantA === 1) {
        const reflectAmount = (unit.type === UnitType.DEFUSER) ? 4 : 2;
        result.reflectedDamage = reflectAmount;
        result.logKeys.push('log_evol_def_reflect_hit');
    }
