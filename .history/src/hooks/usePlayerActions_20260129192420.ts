...
export const usePlayerActions = ({
    ...
    const handleActionComplete = useCallback((actedUnitId: string | null) => {
        ...
        if (p1Done && p2Done) {
            ...
            const currentPlayerHasMoves = nextState.players[currentPlayerId].units.some(u => !u.isDead && !u.hasActedThisRound);

            if (!currentPlayerHasMoves) {
                nextPlayer = currentPlayerId === PlayerID.P1 ? PlayerID.P2 : PlayerID.P1;
            }
    ...
    const handleAttack = useCallback((attackerId: string, targetUnit: Unit) => {
        ...
        setGameState(prev => {
            ...
            const updatedAttackerUnits = pStats.units.map(u => u.id === attacker.id ? { ...u, r: (genLevelA === 3 && genVariantA === 2) ? targetUnit.r : u.r, c: (genLevelA === 3 && genVariantA === 2) ? targetUnit.c : u.c, hp: Math.min(u.maxHp, u.hp + attackerHeal), energyUsedThisTurn: u.energyUsedThisTurn + cost, hasActedThisRound: true } : u);
