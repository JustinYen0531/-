...
import { GameState, PlayerID, Unit, UnitType, GameLog, Mine, MineType } from '../types';
...
export const useGameLoop = ({
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            // === CHRONAL OVERLOAD COOLDOWN CHECK (PRIORITY) ===
            // If in overload and cooldown has ended, reset the system
            if (state.isChronalOverload && state.chronalOverloadEndTime && now >= state.chronalOverloadEndTime) {
                setGameState(prev => ({
                    ...prev,
                    isChronalOverload: false,
                    chronalOverloadEndTime: undefined,
                    chronalCharges: 0 // Reset charges after cooldown
                }));
            }

            // Check if we're in slow-mo period (action happened within 500ms)
            const isInSlowMoPeriod = state.lastActionTime ? (now - state.lastActionTime < 500) : false;
            ...
            // Timer tick logic
            // Normal speed: 1.0 per sec -> 0.1 per 100ms
            // Slow speed: 0.2 per sec -> 0.02 per 100ms
            const step = isInSlowMoPeriod ? 0.02 : 0.1;

            timerAccumulator.current += step;

            if (timerAccumulator.current >= 1) {
                const amount = Math.floor(timerAccumulator.current);
                timerAccumulator.current -= amount;

                if (state.timeLeft > 0) {
                    setGameState(prev => ({
                        ...prev,
                        timeLeft: Math.max(0, prev.timeLeft - amount)
                        // NOTE: isTimeFrozen is NOT set here anymore - it's set by action handlers
                    }));
                } else {
