...
export const usePlayerActions = ({
    ...
    const handleActionComplete = useCallback((actedUnitId: string | null, timedOut: boolean = false) => {
        ...
        setGameState((prev: GameState) => {
            ...
            if (targetUnitId) {
                ...
                const updatedUnits = p.units.map((u: Unit) => u.id === targetUnitId ? { ...u, hasActedThisRound: true } : u);
                nextState.players = { ...nextState.players, [prev.currentPlayer]: { ...p, units: updatedUnits } };
            }

            const p1Done = nextState.players[PlayerID.P1].units.every((u: Unit) => u.isDead || u.hasActedThisRound);
            const p2Done = nextState.players[PlayerID.P2].units.every((u: Unit) => u.isDead || u.hasActedThisRound);
            ...
            if (p1Done && p2Done) {
                ...
                if (nextState.players[nextPlayer].units.every((u: Unit) => u.isDead || u.hasActedThisRound)) nextPlayer = prev.currentPlayer;
    ...
    const attemptMove = useCallback((unitId: string, r: number, c: number, cost: number) => {
        ...
        setGameState((prev: GameState) => {
            ...
            const newUnits = p.units.map((u: Unit) => u.id === unitId ? { ...u, r, c, energyUsedThisTurn: u.energyUsedThisTurn + cost, hasActedThisRound: true } : u);
    ...
    const executeAttack = useCallback((attackerId: string, targetUnit: Unit) => {
        ...
        setGameState((prev: GameState) => {
            ...
            const newTUnits = tp.units.map((u: Unit) => u.id === targetUnit.id ? { ...u, hp: Math.max(0, u.hp - dmg), isDead: u.hp - dmg <= 0 } : u);

            return {
                ...prev,
                players: {
                    ...prev.players,
                    [attacker.owner]: { ...p, energy: p.energy - cost, units: p.units.map((u: Unit) => u.id === attackerId ? { ...u, hasActedThisRound: true } : u) },
    ...
    const handleMinePlacement = useCallback((unit: Unit, targetR: number, targetC: number) => {
        ...
        setGameState((prev: GameState) => {
            ...
            return {
                ...
                players: { ...prev.players, [unit.owner]: { ...p, energy: p.energy - cost, units: p.units.map((u: Unit) => u.id === unit.id ? { ...u, hasActedThisRound: true } : u) } }
            };
        });
    ...
    const handleCellClick = useCallback((r: number, c: number) => {
        ...
        if (state.selectedUnitId) {
            ...
            setGameState((prev: GameState) => ({ ...prev, selectedUnitId: unitInCell.id }));
        }
