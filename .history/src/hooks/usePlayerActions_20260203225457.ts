                ...
                if (result.triggered || result.isNukeTriggered) {
                    ...
                    result.logKeys.forEach(k => {
                        if (k === 'log_smoke_deployed') {
                            ...
                            addLog(k, 'move', {}, unit.owner); // Blue color for P1
                        } else if (k !== 'log_evol_def_reflect_hit') { // Will handle reflect log separately to include target unit name
                            addLog(k, 'evolution');
                        }
                    });
        ...
        setGameState(prev => {
            ...
            const updatedUnits = prevPlayerState.units.map(u => {
                ...
                if (u.type !== UnitType.DEFUSER) {
                    ...
                    uHp = Math.min(uMaxHp, uHp + (result?.teamHealAmount || 0));
                }
                ...
                return {
                    ...u, r, c, hp: Math.min(uMaxHp, newHp + (u.type !== UnitType.DEFUSER ? (result?.teamHealAmount || 0) : 0)),
                    maxHp: uMaxHp, isDead,
            ...
            if (reflectDmg > 0) {
                ...
                if (enemyState.units.length > 0) {
                    ...
                    if (livingEnemies.length > 0) {
                        ...
                        addLog('log_evol_def_reflect_hit', 'move', { unit: getUnitName(target.type), dmg: reflectDmg }, unit.owner);
                    }
                }
            } else {
