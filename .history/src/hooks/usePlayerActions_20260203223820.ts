    ...
    const attemptMove = useCallback((unitId: string, r: number, c: number, cost: number) => {
        ...
        setGameState(prev => {
            ...
            const updatedUnits = prevPlayerState.units.map(u => {
                // Apply team buffs from Defuser A2 if applicable
                let uMaxHp = u.maxHp + (result?.teamMaxHpBonus || 0);
                let uHp = Math.min(uMaxHp, u.hp + (result?.teamHealAmount || 0));

                if (u.id !== unit.id) {
                    return { ...u, maxHp: uMaxHp, hp: uHp };
                }
                return {
                    ...u, r, c, hp: Math.min(uMaxHp, newHp + (result?.teamHealAmount || 0)), maxHp: uMaxHp, isDead,
