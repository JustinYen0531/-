...
export const usePlayerActions = ({
    ...
    const attemptMove = useCallback((unitId: string, r: number, c: number, cost: number) => {
        ...
        if (activeMineIndex !== -1 && mineToTrigger) {
            ...
            if (mine.type === MineType.CHAIN && mine.owner !== unit.owner && (!mine.immuneUnitIds || !mine.immuneUnitIds.includes(unit.id))) {
                ...
                let result: any = null;
                if (activeMineIndex !== -1 && mineToTrigger) {
                    // We need to re-call calculate if we are in this block mainly to get the result object for scope.
                    // Wait, result was defined at line 690?
                    // Line 690: `const result = calculateMineInteraction(...)`
                    // But that was inside `else` block of `if (mine.type === MineType.CHAIN)` ?
                    // Let's check line 689.
                    // Ah, previous code:
                    // 689: } else {
                    // 690:    const result = calculateMineInteraction(...)
                    // }
                    // This means `result` is scoped to the `else` block!
                    // So it is NOT available in `setGameState`.

                    // We need to define `result` at a higher scope.
                }
                mineTriggered = true;
    ...
    setGameState(prev => {
        ...
        const updatedUnits = prevPlayerState.units.map(u => {
            ...
            // Apply Team Buffs if any
            // Note: result is derived from calculation above. We need to access it here.
            // But result is outside this scope? No, it's in the closure.
            // Actually, check if 'result' variable is available. Yes, defined at line 690.
            if (activeMineIndex !== -1 && mineToTrigger && !shouldTriggerMine(unit, mineToTrigger, prevPlayerState) === false) {
                // We need to re-verify if result exists and applies.
                // The logic above sets 'dmg', 'newMaxHp', 'newHp' based on 'result'.
                // WE MUST access 'result' here.
            }

            // Wait, simply accessing 'result' is risky if it wasn't calculated (no mine).
            // But we are inside setGameState. 
            // We should calculate team buffs outside setGameState and pass them in?
            // Or just use the 'result' variable from closure.
            // The issue is 'result' is defined outside setGameState callback.
            // BUT variables like 'mineTriggered' and 'newHp' are used effectively.
            // However, 'result' is only defined inside the if block at 690?
            // No, const result = calculateMineInteraction(...) is at 690. It is always defined.

            if (result && result.teamMaxHpBonus) {
                uMaxHp += result.teamMaxHpBonus;
            }
            if (result && result.teamHealAmount) {
                uHp = Math.min(uMaxHp, uHp + result.teamHealAmount);
            }
            ...
            // For the triggering unit, 'newMaxHp' and 'newHp' were calculated based on 'result.newMaxHpBonus' (self).
            // They do NOT yet include team bonus if 'calculateMineInteraction' separates them.
            // In gameEngine, we set self bonus to 0 and team bonus to 2.
            // So 'newMaxHp' (line 696) added 0.
            // So we MUST add team bonus here for the triggering unit too.
            // But 'newHp' logic at 697 used 'result.healAmount' (0).
            // So we need to update the local 'newMaxHp' and 'newHp' variables before this map?
            // Or just update them here.

            return {
                ...u, r, c, hp: Math.min(uMaxHp, newHp + (result.teamHealAmount || 0)),
                maxHp: uMaxHp, // uMaxHp already has team bonus added to u.maxHp. 
                // Wait, u.maxHp is old max. newMaxHp variable tracked self bonus.
                // Since self bonus was 0, newMaxHp == u.maxHp.
                // So uMaxHp (u.maxHp + team) is correct new max.
                isDead,
