...
export const useGameLoop = ({
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            // Timer tick logic - SINGLE SOURCE OF TRUTH: use isTimeFrozen for speed
            // When frozen: extremely slow (0.01 per 100ms = 1sec/100sec)
            // When in slow-mo (just unfrozen): slow (0.02 per 100ms)
            // Normal: full speed (0.1 per 100ms)
            let step: number;
            if (state.isTimeFrozen) {
                step = 0.005; // Nearly stopped when frozen
            } else if (isInSlowMoPeriod) {
                step = 0.02; // Slow-mo recovery period
            } else {
                step = 0.1; // Normal speed
            }
