...
import {
    ...
    EVOLUTION_COSTS, UNIT_STATS, GRID_ROWS, GRID_COLS, MAX_MINES_ON_BOARD, EVOLUTION_CONFIG, PLACEMENT_MINE_LIMIT
} from '../constants';
import {
    getMineBaseCost, getUnitName,
    getEnemyTerritoryEnergyCost,
    getDisplayCost as getDisplayCostRaw,
    getUnitTypeAbbr
} from '../gameHelpers';
...
export const usePlayerActions = ({
    ...
    const getDisplayCost = useCallback((unit: Unit | null, baseCost: number, state: GameState = gameStateRef.current, actionType: string = 'move') => {
        return getDisplayCostRaw(unit, baseCost, state, actionType);
    }, [gameStateRef]);

    const checkEnergyCap = useCallback((unit: Unit, player: PlayerState, cost: number) => {
    ...
    const handlePlaceMineAction = useCallback((unit: Unit, targetR: number, targetC: number) => {
        const state = gameStateRef.current;
        const player = state.players[unit.owner];
        ...
        if (!isInFactoryRange && state.mines.filter(m => m.owner === unit.owner && !factories.some(f => Math.max(Math.abs(f.r - m.r), Math.abs(f.c - m.c)) <= (f.level >= 2 ? 2 : 1))).length >= 5) { addLog('log_maker_range', 'error'); return; }

        setGameState(prev => {
            const p = prev.players[unit.owner];
            return { ...prev, mines: [...prev.mines, { id: `m-${Date.now()}`, owner: unit.owner, type: effectiveMineType, r: targetR, c: targetC, revealedTo: [], immuneUnitIds: prev.players[PlayerID.P1].units.concat(prev.players[PlayerID.P2].units).filter(u => u.r === targetR && u.c === targetC).map(u => u.id) }], players: { ...prev.players, [unit.owner]: { ...p, energy: p.energy - cost, questStats: { ...p.questStats, makerMinesPlaced: p.questStats.makerMinesPlaced + 1 }, units: p.units.map(u => u.id === unit.id ? { ...u, energyUsedThisTurn: u.energyUsedThisTurn + cost } : u) } } };
        });
        addLog('log_mine_placed', 'mine', { r: targetR + 1, c: targetC + 1 }, unit.owner);
        setTargetMode(null);
        setSelectedMineType(MineType.NORMAL);
    }, [gameStateRef, selectedMineType, addLog, checkEnergyCap, setGameState, setTargetMode, setSelectedMineType]);
    ...
    return {
        handleCellClick: (r: number, c: number) => { }, // Placeholder for App.tsx's click logic if needed
        attemptMove,
        handleAttack,
