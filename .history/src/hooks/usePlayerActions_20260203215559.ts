...
export const usePlayerActions = ({
    ...
    const startNewRound = useCallback((prevState: GameState) => {
        ...
        setGameState({
            ...
            smokes: newSmokes,
            movements: [],
            sensorResults: prevState.sensorResults.filter(sr => nextTurn - sr.createdTurn < 3), // Keep sensor results for 3 rounds
            players: {
    ...
    const handleSensorScanAction = useCallback((unit: Unit, r: number, c: number) => {
        const state = gameStateRef.current;
        const swpLevelB = state.players[unit.owner].evolutionLevels[UnitType.MINESWEEPER].b;

        // Check if player has unlocked Sensor Scan (Path B Level 1+)
        if (swpLevelB < 1) {
            addLog('log_low_energy_evolve', 'error');
            return;
        }

        const cost = 5; // LV1 cost
        const player = state.players[unit.owner];
        ...
        // Range check (Manhattan distance <= 5)
        if (Math.abs(unit.r - r) + Math.abs(unit.c - c) > 5) {
            addLog('log_scan_range', 'error');
            return;
        }
        ...
        // Count mines in 3x3 area around target point
        let mineCount = 0;
        state.mines.forEach(m => {
            const distance = Math.max(Math.abs(m.r - r), Math.abs(m.c - c));
            if (distance <= 1) {
                mineCount++;
            }
        });

        // Add sensor result to display on grid
        const newSensorResult = {
            r,
            c,
            count: mineCount,
            owner: unit.owner,
        ...
        setGameState(prev => {
            const p = prev.players[unit.owner];
            const qStats = { ...p.questStats, sweeperScansPerformed: p.questStats.sweeperScansPerformed + 1 };

            // Remove old sensor results from same player and add new one
            const filteredSensorResults = prev.sensorResults.filter(sr => sr.owner !== unit.owner);

            return {
                ...prev,
                sensorResults: [...filteredSensorResults, newSensorResult],
                players: {
                    ...prev.players,
                    [unit.owner]: {
                        ...p,
        ...
        addLog('log_scan_count', 'move', { r: r + 1, c: c + 1, count: mineCount }, unit.owner);
    ...
    const handleSensorScan = useCallback((unitId: string, r: number, c: number) => {
        ...
        // Verify Range (Standard Sensor Range = 5)
        if (Math.abs(unit.r - r) + Math.abs(unit.c - c) > 5) {
            addLog('log_scan_range', 'error');
            return;
        }

        const baseCost = swpB >= 3 ? 3 : (swpB >= 2 ? 4 : 5);
        const isPathB3_2 = swpB === 3 && variantB === 2;
        const finalCost = isPathB3_2 ? 6 : baseCost;
        ...
        setGameState(prev => {
            ...
            // Deep copy of players to safely modify units/HP
            const p1 = { ...prev.players[PlayerID.P1], units: prev.players[PlayerID.P1].units.map(u => ({ ...u })) };
            const p2 = { ...prev.players[PlayerID.P2], units: prev.players[PlayerID.P2].units.map(u => ({ ...u })) };

            let explosionOccurred = false;

            if (isPathB3_2) {
                // Gravity Collapse (Nowhere to Hide): Pull mines in 3x3 to center
                const minesInArea = nextMines.filter(m =>
                    Math.abs(m.r - r) <= 1 && Math.abs(m.c - c) <= 1 && m.owner === enemyId
                );

                const unitsAtCenter = [...p1.units, ...p2.units].filter(u => !u.isDead && u.r === r && u.c === c);
                // Condition: Multiple mines pulled OR Unit present at center
                const shouldExplode = minesInArea.length > 1 || (minesInArea.length > 0 && unitsAtCenter.length > 0);

                if (shouldExplode) {
                    explosionOccurred = true;
                    let totalDamage = 0;
                    minesInArea.forEach(m => {
                        let dmg = (m.type === MineType.NUKE) ? 12 : 6;
                        totalDamage += dmg;
                    });

                    // Damage units at center
                    unitsAtCenter.forEach(u => {
                        u.hp = Math.max(0, u.hp - totalDamage);
                        if (u.hp <= 0) {
                            u.isDead = true;
                            u.respawnTimer = (u.type !== UnitType.GENERAL) ? (prev.turnCount <= 10 ? 2 : 3) : 0;
                        }
                    });

                    // Destroy collided mines
                    nextMines = nextMines.filter(m => !minesInArea.includes(m));
                } else if (minesInArea.length === 1) {
                    // Pull single mine to center without collision
                    const mine = minesInArea[0];
                    const idx = nextMines.findIndex(m => m.id === mine.id);
                    if (idx !== -1) {
                        nextMines[idx] = { ...mine, r: r, c: c, revealedTo: [...mine.revealedTo, unit.owner] }; // Reveal
                    }
                }
            } else {
                // Standard Sensor Scan Logic
                const executeAnalysis = (targetR: number, targetC: number) => {
                    let count = 0;
                    for (let dr = -1; dr <= 1; dr++) {
                        for (let dc = -1; dc <= 1; dc++) {
                            const rr = targetR + dr;
                    ...
                    nextSensorResults.push({ r: targetR, c: targetC, count, owner: unit.owner, createdTurn: prev.turnCount });
                };
                ...
                if (swpB >= 2) {
                    ...
                    if (centerMine && !centerMine.revealedTo.includes(unit.owner)) {
                        ...
                        if (idx !== -1) nextMines[idx] = { ...centerMine, revealedTo: [...centerMine.revealedTo, unit.owner] };
                    }
                }
                    ...
                    for (let dr = -1; dr <= 1; dr++) {
                        for (let dc = -1; dc <= 1; dc++) {
                            ...
                            if (rr >= 0 && rr < GRID_ROWS && cc >= 0 && cc < GRID_COLS) {
                                if (nextMines.some(m => m.r === rr && m.c === cc && m.owner === enemyId)) count++;
                            }
                        }
                    }
                    nextSensorResults.push({ r: unit.r, c: unit.c, count, owner: unit.owner, createdTurn: prev.turnCount });
                }
            }

            const activeP = (unit.owner === PlayerID.P1) ? p1 : p2;
            activeP.energy -= finalCost;
            const updatedUnit = activeP.units.find(u => u.id === unitId);
            if (updatedUnit) updatedUnit.energyUsedThisTurn += finalCost;
            ...
            return {
                ...
                vfx: explosionOccurred
                    ? [...prev.vfx, { id: `vfx-${Date.now()}`, type: 'explosion', r, c, size: 'large', startTime: Date.now() }]
                    : prev.vfx,
                lastActionTime: Date.now(),
                isTimeFrozen: true
            };
        ...
        addLog('log_action_sensor_scan', 'move', { unit: '掃雷者', r: r + 1, c: c + 1 }, unit.owner);
        setTargetMode(null);
    ...
    const handleEvolve = useCallback((unitType: UnitType, branch: 'a' | 'b', variant?: number) => {
        ...
        addLog('log_evolved', 'move', { unit: getUnitName(unitType), branch, level: currentLevel + 1 }, state.currentPlayer);
