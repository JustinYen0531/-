...
import {
    ...
    MineType, GameLog, Coordinates, PlayerState, VFXEffect
} from '../types';
import {
...
export const usePlayerActions = ({
        ...
        if (activeMineIndex !== -1 && mineToTrigger) {
            ...
            if (mine.type === MineType.CHAIN && mine.owner !== unit.owner && (!mine.immuneUnitIds || !mine.immuneUnitIds.includes(unit.id))) {
                ...
                normalMinesInRange.forEach(nm => {
                    addLog('log_evol_mkr_chain', 'mine', { r: nm.r + 1, c: nm.c + 1 });
    ...
    const handlePickupFlag = useCallback(() => {
        ...
        setGameState(prev => {
            ...
            return { ...prev, activeUnitId: unit.id, players: { ...prev.players, [unit.owner]: { ...p, units: p.units.map(u => u.id === unit.id ? { ...u, hasFlag: true } : u) } }, lastActionTime: Date.now() };
        });
    ...
    const handleDropFlag = useCallback(() => {
        ...
        setGameState(prev => {
            const p = prev.players[unit.owner];
            return { ...prev, activeUnitId: unit.id, players: { ...prev.players, [unit.owner]: { ...p, units: p.units.map(u => u.id === unit.id ? { ...u, hasFlag: false } : u), flagPosition: { r: unit.r, c: unit.c } } }, lastActionTime: Date.now() };
        });
        addLog('log_committed', 'info');
    ...
    const handleScanAction = useCallback((unit: Unit, r: number, c: number) => {
        ...
        setGameState(prev => {
            ...
            return { ...prev, mines: newMines, players: { ...prev.players, [unit.owner]: { ...p, energy: p.energy - cost, questStats: qStats, units: p.units.map(u => u.id === unit.id ? { ...u, energyUsedThisTurn: u.energyUsedThisTurn + cost } : u) } }, lastActionTime: Date.now() };
        });
    ...
    const handlePlaceMineAction = useCallback((unit: Unit, targetR: number, targetC: number, mineType: MineType) => {
        ...
        console.log('[DEBUG] handlePlaceMineAction - mineType received:', mineType);
