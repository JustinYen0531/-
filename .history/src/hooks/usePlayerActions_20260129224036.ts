...
export const usePlayerActions = ({
    ...
    const attemptMove = useCallback((unitId: string, r: number, c: number, cost: number) => {
        ...
        if (activeMineIndex !== -1 && mineToTrigger) {
            ...
            if (mine.type === MineType.CHAIN && mine.owner !== unit.owner && (!mine.immuneUnitIds || !mine.immuneUnitIds.includes(unit.id))) {
                ...
                dmg = 6; // Base damage from Chain Mine itself
                addLog('log_hit_mine', 'mine', { unit: getUnitName(unit.type), dmg: 6 }, unit.owner);
                    ...
                    state.players[enemyPlayer].units.forEach((u: Unit) => {
                        if (!u.isDead && Math.abs(u.r - nm.r) <= 1 && Math.abs(u.c - nm.c) <= 1) {
                            ...
                            if (u.id === unit.id) {
                                ...
                                u.hp = Math.max(0, u.hp - (6 + vuln));
                            }
                            addLog('log_chain_aoe', 'mine', { unit: getUnitName(u.type), dmg: 6 }, u.owner);
                        }
                    });
                });
    ...
    const handleRanger = useCallback((subAction: 'pickup' | 'drop') => {
        addLog('log_committed', 'info');
    }, [addLog]);
    ...
    const handleMoveEnemyMine = useCallback((unit: Unit, mineId: string, targetR: number, targetC: number) => {
        const state = gameStateRef.current;
        ...
        setGameState(prev => {
            const p = prev.players[unit.owner];
            const newMines = prev.mines.map(m => m.id === mineId ? { ...m, r: targetR, c: targetC } : m);
            return {
                ...prev,
                mines: newMines,
                players: { ...prev.players, [unit.owner]: { ...p, energy: p.energy - cost, units: p.units.map(u => u.id === unit.id ? { ...u, energyUsedThisTurn: u.energyUsedThisTurn + cost } : u) } },
        ...
        addLog('log_mine_moved', 'move', { r: targetR + 1, c: targetC + 1 }, unit.owner);
        handleActionComplete(unit.id);
    }, [gameStateRef, addLog, setGameState, handleActionComplete]);

    const handleConvertEnemyMine = useCallback((unit: Unit, r: number, c: number) => {
        const state = gameStateRef.current;
        const mine = state.mines.find(m => m.r === r && m.c === c);
        if (!mine || mine.owner === unit.owner) return;
        const cost = getEnemyTerritoryEnergyCost(unit, 15);
        ...
        setGameState(prev => {
            const p = prev.players[unit.owner];
            const newMines = prev.mines.map(m => m.id === mine.id ? { ...m, owner: unit.owner } : m);
            return {
                ...prev,
                mines: newMines,
                players: { ...prev.players, [unit.owner]: { ...p, energy: p.energy - cost, units: p.units.map(u => u.id === unit.id ? { ...u, energyUsedThisTurn: u.energyUsedThisTurn + cost } : u) } },
        ...
        addLog('log_mine_converted', 'evolution', { r: r + 1, c: c + 1 }, unit.owner);
        handleActionComplete(unit.id);
    }, [gameStateRef, addLog, setGameState, handleActionComplete]);

    const handlePickupMine = useCallback((unit: Unit, r: number, c: number) => {
        const state = gameStateRef.current;
        ...
        setGameState(prev => {
            const p = prev.players[unit.owner];
            // Remove mine from board, add to unit
            const newMines = prev.mines.filter(m => m.id !== mine.id);
            const newUnits = p.units.map(u => u.id === unit.id ? { ...u, carriedMine: mine, energyUsedThisTurn: u.energyUsedThisTurn + cost } : u);
            return {
                ...prev,
                mines: newMines,
                players: { ...prev.players, [unit.owner]: { ...p, energy: p.energy - cost, units: newUnits } },
                lastActionTime: Date.now(), isTimeFrozen: true
            };
        });
    ...
    const handleThrowMine = useCallback((unit: Unit, r: number, c: number) => {
        const state = gameStateRef.current;
        ...
        setGameState(prev => {
            const p = prev.players[unit.owner];
            const newMine: Mine = { ...unit.carriedMine!, r, c, id: `mine-${Date.now()}` }; // Reset ID and Pos
            const newUnits = p.units.map(u => u.id === unit.id ? { ...u, carriedMine: undefined, energyUsedThisTurn: u.energyUsedThisTurn + cost } : u);
        ...
        addVFX('smoke', r, c); // Visual feedback
        addLog('log_place_mine', 'move', { r: r + 1, c: c + 1 }, unit.owner);
        handleActionComplete(unit.id);
    }, [gameStateRef, addLog, setGameState, handleActionComplete, addVFX]);
