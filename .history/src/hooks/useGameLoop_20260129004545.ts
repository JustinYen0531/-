...
export const useGameLoop = ({
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            const now = Date.now();
            // Slow-mo if action happened within 500ms
            const isSlowMo = state.lastActionTime ? (now - state.lastActionTime < 500) : false;

            // Normal speed: 1.0 per sec -> 0.1 per 100ms
            // Slow speed: 0.2 per sec -> 0.02 per 100ms
            const step = isSlowMo ? 0.02 : 0.1;
            ...
            if (timerAccumulator.current >= 1) {
                ...
                // Update UI state for frozen icon even if time hasn't ticked
                if (!!state.isTimeFrozen !== isSlowMo) {
                    setGameState(prev => ({ ...prev, isTimeFrozen: isSlowMo }));
                }
            }
        }, 100);
