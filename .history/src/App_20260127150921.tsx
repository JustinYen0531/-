...
import {
    createInitialState, getStartingPositions
} from './gameInit';
import {
    getUnitTypeAbbr, getMineBaseCost, getUnitIcon, getUnitName
} from './gameHelpers';
import {
    ArrowRight, Zap, Swords, Info,
    Bomb, Eye, Shield, Crown, Flag, CheckCircle, Play, Pause, LogOut, Cpu, Users, ArrowDownToLine,
    Dna, Unlock, X, Globe, Hand, ChevronLeft, ChevronRight, DoorOpen, FlaskConical, Radio, ShieldAlert, RefreshCw, Ghost
} from './icons';
...
export default function App() {
    ...
    const handleAttack = (attackerId: string, targetUnit: Unit) => {
        ...
        const { damage: calculatedDmg, logKey } = calculateAttackDamage(
            attacker, targetUnit, state.players[attacker.owner], state.players[targetUnit.owner], state.isGodMode
        );
        let dmg = calculatedDmg;
    ...
    const handlePlaceHubAction = (unit: Unit, r: number, c: number) => {
        ...
        setGameState(prev => {
            ...
            return {
                ...
                logs: [
                    { turn: prev.turnCount, messageKey: 'log_placed_building', params: { type: '?喲?' }, owner: unit.owner, type: 'info' as const },
                    ...prev.logs
                ]
            };
        });
    ...
    const handlePlaceFactoryAction = (unit: Unit, r: number, c: number) => {
        ...
        setGameState(prev => {
            ...
            return {
                ...
                logs: [
                    { turn: prev.turnCount, messageKey: 'log_placed_building', params: { type: '?圈撌亙?' }, owner: unit.owner, type: 'info' as const },
                    ...prev.logs
                ]
            };
        });
    ...
    */
    ...
    const attemptMove = (unitId: string, r: number, c: number, cost: number) => {
        ...
        // Note: calculateMineInteraction returns pure logic results.
        // We still need to identify the active mine for Chain logic and specific removal.
        ...
        if (activeMineIndex !== -1 && mineToTrigger) {
            ...
            // Chain Logic (Handled here as it requires recursive state access)
            if (mine.type === MineType.CHAIN && mine.owner !== unit.owner && (!mine.immuneUnitIds || !mine.immuneUnitIds.includes(unit.id))) {
                ...
                subMines.forEach(sm => {
                    ...
                    if (sm.type === MineType.SMOKE) {
                        ...
                        // Note: duration not set here in original logic? Assuming intended.
                    } else if (sm.type === MineType.NUKE) {
                        // Chain-triggered Nuke
                ...
                // Use Game Engine for standard/Nuke/Slow/Smoke/Defuser Logic
                const result = calculateMineInteraction(unit, currentState.mines, r, c, currentState.players[unit.owner]);
                ...
                if (result.triggered || result.isNukeTriggered) {
                    ...
                    // Handle Destruction / AoE
                    if (result.isNukeTriggered) {
                        const nukeBlastDamage = 6;
                        // 1. Destroy ALL mines in 3x3
                        newMines = newMines.filter(m =>
                            !(Math.abs(m.r - mine.r) <= 1 && Math.abs(m.c - mine.c) <= 1)
                        );
                        // 2. Destroy ALL buildings in 3x3
                        currentBuildings = currentBuildings.filter(b =>
                            !(Math.abs(b.r - mine.r) <= 1 && Math.abs(b.c - mine.c) <= 1)
                        );
                        // 3. AoE Damage to Units
                        const allUnits = [...currentState.players[PlayerID.P1].units, ...currentState.players[PlayerID.P2].units];
                        allUnits.forEach(targetUnit => {
                            if (!targetUnit.isDead && Math.abs(targetUnit.r - mine.r) <= 1 && Math.abs(targetUnit.c - mine.c) <= 1) {
                                ...
                                if (targetUnit.id === unit.id) {
                                    // Triggering unit takes full dmg (already in dmg variable)
                                    return;
                                }
    ...
    const renderControlPanel = () => {
        ...
        return (
            <div className="h-48 bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 border-t-4 border-white flex shrink-0 z-20 shadow-2xl shadow-white/10">
                <div className="flex w-full">
                    <div className="flex flex-col justify-center p-2 border-r-2 border-white/30 bg-slate-800/50 shrink-0">
                        <button
                            onClick={() => setShowEvolutionTree(true)}
                            className="h-full px-3 bg-gradient-to-b from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 rounded font-black text-xs flex flex-col items-center justify-center gap-2 border-2 border-purple-400 shadow-lg shadow-purple-500/50 transition-all hover:scale-105 text-white"
                        >
                            <Dna size={24} />
                            <span style={{ writingMode: 'vertical-rl' }} className="tracking-widest filter drop-shadow-md">{t('evolution_tree')}</span>
                        </button>
                    </div>
                                                            ...
                                                            buttons.push(
                                                                <div key="custom_dismantle" className="flex flex-col items-center gap-1">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Unlock size={20} />
                                                                        <span className="text-[10px]">?撱箇?</span>
                                                                    </button>
                                                                    <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                        <span className="text-yellow-400">⚡</span>
                                                            ...
                                                            buttons.push(
                                                                <div key="place_tower" className="flex flex-col items-center gap-1">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Radio size={20} />
                                                                        <span className="text-[10px]">設置塔</span>
                                                                    </button>
                                                                    <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                        <span className="text-yellow-400">⚡</span>
                                                                        <span>{getDisplayCost(unit, 8)}</span>
                                                            ...
                                                            if (swpLevelA === 3 && variantA === 2) {
                                                                buttons.push(
                                                                    <div key="detonate_tower" className="flex flex-col items-center gap-1">
                                                                        ...
                                                                        >
                                                                            <Bomb size={20} />
                                                                            <span className="text-[10px]">撘?</span>
                                                                        </button>
                                                                        <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                            <span className="text-yellow-400">⚡</span>
                                                                            <span>2</span>
                                                            ...
                                                            buttons.push(
                                                                <div key="place_factory" className="flex flex-col items-center gap-1">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <span className="text-[10px]">閮剔蔭撌亙?</span>
                                                                    </button>
                                                                    <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                        <span className="text-yellow-400">⚡</span>
                                                                    ...
                                                                    {(() => {
                                                                        ...
                                                                        return (
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <Bomb size={20} />
                                                                                <span className="text-[10px]">?曄蔭?圈</span>
                                                                            </button>
                                                                        );
                                                                    })()}
                                                                        ...
                                                                        <div className="grid grid-cols-2 gap-1 px-2 bg-slate-900/90 rounded-r border-2 border-l-0 border-white h-full items-center shadow-xl backdrop-blur-md -ml-px">
                                                                            ...
                                                                            {player.evolutionLevels[UnitType.MAKER].a >= 3 && (
                                                                                <button
                                                                                    ...
                                                                                    title={player.evolutionLevels[UnitType.MAKER].aVariant === 1 ? "製造鏈鎖地雷" : "製造核彈地雷"}
                                                                                >
                                                                                    {player.evolutionLevels[UnitType.MAKER].aVariant === 1 ? '鏈' : '核'}
                                                                                </button>
                                                                            )}
                                                                        </div>
                                                                    </div>
                                                            ...
                                                            buttons.push(
                                                                <div key="hub_teleport" className="flex flex-col items-center gap-1">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Zap size={20} />
                                                                        <span className="text-[10px]">傳送</span>
                                                                    </button>
                                                                    <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                        <span>⚡</span>
                                                                        <span>5</span>
                                                            ...
                                                            buttons.push(
                                                                <div key="place_hub" className="flex flex-col items-center gap-1">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Cpu size={20} />
                                                                        <span className="text-[10px]">設置樞紐</span>
                                                                    </button>
                                                                    <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                        <span>⚡</span>
                                                                        <span>{getDisplayCost(unit, 8)}</span>
                                                                    ...
                                                                    buttons.push(
                                                                        <div key="stealth_action" className="flex flex-col items-center gap-1">
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <span className="text-[10px]">{unit.status.isStealthed ? '取消隱匿' : '隱匿行動'}</span>
                                                                            </button>
                                                                            <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white">
                                                                                <span>⚡</span>
                                                    ...
                                                    if (needsVariantA) {
                                                        ...
                                                        evolveButtons.push(
                                                            <div key="evolve_a_lv3_1" className="flex flex-col items-center gap-1">
                                                                ...
                                                                >
                                                                    ...
                                                                    <Dna size={20} />
                                                                    <span className="text-[9px] leading-none">進化 A</span>
                                                                ...
                                                                <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5">
                                                                    <span className="text-yellow-400">⚡</span>                                <span>{EVOLUTION_COSTS[2]}</span>
                                                                </div>
                                                            </div>
                                                        );
                                                        buttonIndex++;
                                                        ...
                                                        evolveButtons.push(
                                                            <div key="evolve_a" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5">
                                                                    <span className="text-yellow-400">⚡</span>                                <span>{costA}</span>
                                                                </div>
                                                            </div>
                                                        );
                                                    }
                                                    ...
                                                    if (needsVariantB) {
                                                        ...
                                                        evolveButtons.push(
                                                            <div key="evolve_b_lv3_1" className="flex flex-col items-center gap-1">
                                                                ...
                                                                >
                                                                    <div className="absolute top-0.5 left-1.5 text-xs font-black text-white/90">{buttonIndex}</div>
                                                                    <Dna size={20} />
                                                                    <span className="text-[9px] leading-none">進化 B</span>
                                                        ...
                                                        evolveButtons.push(
                                                            <div key="evolve_b" className="flex flex-col items-center gap-1">
                                                                ...
                                                                <div className="bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5">
                                                                    <span className="text-yellow-400">⚡</span>                                <span>{costB}</span>
                                                                </div>
                                                            </div>
                                                        );
                                                    }
