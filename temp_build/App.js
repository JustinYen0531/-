...
import { GRID_ROWS, GRID_COLS, UNIT_STATS, INITIAL_ENERGY, ENERGY_REGEN, MAX_INTEREST, ENERGY_CAP_RATIO, P1_FLAG_POS, P2_FLAG_POS, ORE_REWARDS, MINE_DAMAGE, TURN_TIMER, THINKING_TIMER, EVOLUTION_CONFIG, EVOLUTION_COSTS, PLACEMENT_TIMER, PLACEMENT_MINE_LIMIT, MAX_MINES_ON_BOARD } from './constants';
import GridCell from './components/GridCell';
import { ArrowRight, Skull, Zap, Swords, Info, Bomb, Eye, Shield, Footprints, Crown, Flag, CheckCircle, Play, Pause, LogOut, Cpu, Users, ArrowDownToLine, Dna, Unlock, X, Globe, Star, Hand, ChevronLeft, ChevronRight, DoorOpen, FlaskConical, Radio, ShieldAlert, RefreshCw, Ghost, Volume2, VolumeX } from 'lucide-react';
import { TRANSLATIONS } from './i18n';
// --- Helper Functions ---
const getUnitTypeAbbr = (type) => {
    switch (type) {
...
const getUnitIcon = (type, size = 18, tier = 0) => {
    if (type === UnitType.GENERAL && tier > 0) {
        return (_jsx(Crown, { size: size, className: "text-yellow-300 drop-shadow-lg animate-pulse", style: {
    ...
    switch (type) {
        case UnitType.GENERAL: return _jsx(Crown, { size: size, className: "text-yellow-300 drop-shadow-lg animate-pulse", style: { filter: 'drop-shadow(0 0 8px rgba(253, 224, 71, 0.6))', strokeWidth: '3px', stroke: 'currentColor' } });
        case UnitType.MINESWEEPER: return _jsx(Eye, { size: size, className: "text-cyan-300 drop-shadow-lg animate-pulse", style: { filter: 'drop-shadow(0 0 8px rgba(34, 211, 238, 0.6))', strokeWidth: '3px', stroke: 'currentColor' } });
        case UnitType.RANGER: return _jsx(Footprints, { size: size, className: "text-emerald-300 drop-shadow-lg animate-pulse", style: { filter: 'drop-shadow(0 0 8px rgba(52, 211, 153, 0.6))', strokeWidth: '3px', stroke: 'currentColor' } });
        case UnitType.MAKER: return _jsx(Bomb, { size: size, className: "text-red-300 drop-shadow-lg animate-pulse", style: { filter: 'drop-shadow(0 0 8px rgba(252, 165, 165, 0.6))', strokeWidth: '3px', stroke: 'currentColor' } });
        case UnitType.DEFUSER: return _jsx(Shield, { size: size, className: "text-blue-300 drop-shadow-lg animate-pulse", style: { filter: 'drop-shadow(0 0 8px rgba(147, 197, 253, 0.6))', strokeWidth: '3px', stroke: 'currentColor' } });
        default: return null;
    }
};
const getUnitName = (type) => {
    switch (type) {
        ...
        case UnitType.MINESWEEPER: return '掃雷者';
        case UnitType.RANGER: return '遊俠';
        case UnitType.MAKER: return '造雷者';
        case UnitType.DEFUSER: return '解雷者';
        default: return '未知';
    }
};
// --- Helper: Get Starting Positions for Units ---
const getStartingPositions = (pid) => {
    const positions = [];
...
const createInitialState = (mode) => {
    ...
    let p2Obstacles = 0;
    let attempts = 0;
    const hasNeighborObstacle = (r, c) => {
        for (let dr = -1; dr <= 1; dr++) {
            for (let dc = -1; dc <= 1; dc++) {
                const nr = r + dr;
                const nc = c + dc;
                if (nr >= 0 && nr < GRID_ROWS && nc >= 0 && nc < GRID_COLS) {
                    if (cells[nr][nc].isObstacle)
                        return true;
                }
            }
        }
    ...
    return {
        ...
        selectedUnitId: null,
        activeUnitId: null,
        logs: [{ turn: 1, messageKey: 'log_placement_phase', type: 'info' }],
        gameOver: false,
        winner: null,
...
export default function App() {
    ...
    const [gameState, setGameState] = useState(createInitialState('pvp'));
    ...
    const [selectedMineType, setSelectedMineType] = useState(MineType.NORMAL);
    ...
    useEffect(() => {
        const onMouseMove = (e) => {
            if (!sandboxDragRef.current.isDragging)
                return;
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            if (state.gameOver || state.isPaused || state.isSandboxTimerPaused)
                return;
            if (state.timeLeft > 0) {
    ...
    const addLog = (messageKey, type = 'info', params, owner) => {
        setGameState(prev => ({
            ...prev,
            logs: [{ turn: prev.turnCount, messageKey, params, type, owner }, ...prev.logs].slice(0, 100)
        }));
    };
    const getUnit = (id, state = gameState) => {
    ...
    const handleCellClick = (r, c) => {
        ...
        if (state.phase === 'thinking')
            return;
        if (state.gameMode === 'pve' && state.currentPlayer === PlayerID.P2)
            return;
    ...
    const getEnemyTerritoryEnergyCost = (unit, baseCost) => {
    ...
    const getDisplayCost = (unit, baseCost, state = gameState, actionType = 'move') => {
        if (!unit)
            return baseCost;
        const player = state.players[unit.owner];
        let cost = baseCost;
    ...
    const checkEnergyCap = (unit, _player, cost) => {
    ...
    const handleAttack = (attackerId, targetUnit) => {
        const state = gameStateRef.current;
        ...
        // Rule 1: Energy Cap
        if (!checkEnergyCap(attacker, player, cost))
            return;
        ...
        if (attacker.type === UnitType.GENERAL) {
            if (genLevelA >= 1)
                dmg = 6; // Level 1+: 6 damage (no further scaling)
        }
        ...
        if (attacker.type === UnitType.GENERAL && genLevelA >= 3) {
            ...
            if (isAttackerPosValid && isTargetPosValid) {
                setGameState(prev => {
                    ...
                    const tStats = prev.players[targetUnit.owner];
                    // Move attacker
                    const updatedAttackerUnits = pStats.units.map(u => u.id === attacker.id ? { ...u, r: newAttackerR, c: newAttackerC } : u);
                    // Knockback target
                    const updatedTargetUnits = tStats.units.map(u => u.id === targetUnit.id ? { ...u, r: newTargetR, c: newTargetC } : u);
                    return {
                        ...prev,
                        players: {
                            ...prev.players,
    ...
    const finishPlacementPhase = () => {
        setGameState(prev => {
            ...
            return {
                ...
                selectedUnitId: null,
                targetMode: null,
                logs: [{ turn: 1, messageKey: 'log_planning_phase', params: { round: 1 }, type: 'info' }, ...newLogs]
            };
        });
    };
    const applyRadarScans = (state) => {
    ...
    const startActionPhase = () => {
        setGameState(prev => {
            ...
            return {
                ...
                logs: [{ turn: prev.turnCount, messageKey: 'log_action_phase', type: 'info' }, ...prev.logs]
            };
        });
    ...
    const handleScanAction = (unit, r, c) => {
        ...
        if (swpLevelB >= 2)
            baseCost = 4; // Tier 2 Cost
        if (swpLevelB === 3 && variantB === 1)
            baseCost = 3; // Tier 3-1 Cost
        if (swpLevelB === 3 && variantB === 2)
            baseCost = 6; // Tier 3-2 Cost
        ...
        if (!checkEnergyCap(unit, gameState.players[unit.owner], cost))
            return;
    ...
    const handlePlaceTowerAction = (unit, r, c) => {
    ...
    const handleDetonateTowerAction = (unit) => {
        const swpLevelA = gameState.players[unit.owner].evolutionLevels[UnitType.MINESWEEPER].a;
        const variantA = gameState.players[unit.owner].evolutionLevels[UnitType.MINESWEEPER].aVariant;
        ...
        setGameState(prev => {
            ...
            const minesToRemove = prev.mines.filter(m => towers.some(t => Math.abs(m.r - t.r) <= 1 && Math.abs(m.c - t.c) <= 1)).map(m => m.id);
            const remainingBuildings = prev.buildings.filter(b => !(b.owner === unit.owner && b.type === 'tower'));
            return {
                ...prev,
    ...
    const handlePlaceMineAction = (unit, targetR, targetC) => {
        ...
        if (effectiveMineType === MineType.SMOKE && !(mkrLevelA >= 1 || mkrLevelB >= 2))
            effectiveMineType = MineType.NORMAL;
        if (effectiveMineType === MineType.SLOW && !(mkrLevelA >= 2 || mkrLevelB >= 2))
            effectiveMineType = MineType.NORMAL;
        // Level 3 Validation
        if ((effectiveMineType === MineType.CHAIN || effectiveMineType === MineType.NUKE) && mkrLevelA < 3) {
            ...
            if (effectiveMineType === MineType.CHAIN && mkrVariantA !== 1)
                effectiveMineType = MineType.NORMAL;
            if (effectiveMineType === MineType.NUKE && mkrVariantA !== 2)
                effectiveMineType = MineType.NORMAL;
        }
        ...
        const maxLimit = (() => {
            ...
            if (mkrLevelB === 3) {
                if (mkrVariantB === 2)
                    return 5 + (factories.length * 2); // 5 + 2 + 2 = 9
                return 5 + 3; // 5 + 3 = 8
            }
        ...
        const minesOutsideRange = state.mines.filter(m => m.owner === unit.owner &&
            !factories.some(f => Math.max(Math.abs(f.r - m.r), Math.abs(f.c - m.c)) <= (f.level >= 2 ? 2 : 1))).length;
    ...
    const lockUnit = (unitId) => {
        setGameState(prev => {
            const unit = getUnit(unitId, prev);
            if (!unit)
                return prev;
            const playerState = prev.players[unit.owner];
            const updatedUnits = playerState.units.map(u => u.id === unitId ? { ...u, startOfActionEnergy: playerState.energy } : u);
            return {
                ...prev,
                activeUnitId: unitId,
                players: {
    ...
    const handlePickupFlag = () => {
        ...
        if (!canPickUp)
            return;
        if (unit.r !== player.flagPosition.r || unit.c !== player.flagPosition.c)
            return;
        setGameState(prev => {
            const p = prev.players[unit.owner];
    ...
    const handleRangerAction = (subAction) => {
        const unitId = gameState.selectedUnitId;
        if (!unitId)
            return;
        const unit = getUnit(unitId);
        if (!unit || unit.type !== UnitType.RANGER)
            return;
        ...
        if (subAction === 'pickup') {
            ...
            const minesInRange = gameState.mines.filter(m => Math.abs(m.r - unit.r) <= pickupRadius &&
    ...
    const handlePickupMineAt = (unit, r, c) => {
    ...
    const handleStealthAction = (unitId) => {
        const unit = getUnit(unitId);
        if (!unit)
            return;
        const player = gameState.players[unit.owner];
        ...
        if (!checkEnergyCap(unit, player, cost))
            return;
        setGameState(prev => {
            const p = prev.players[unit.owner];
            const updatedUnits = p.units.map(u => u.id === unitId ? {
    ...
    const handlePlaceHubAction = (unit, r, c) => {
        const baseCost = 8;
        ...
        setGameState(prev => {
            ...
            return {
                ...
                logs: [
                    { turn: prev.turnCount, messageKey: 'log_placed_building', params: { type: '樞紐' }, owner: unit.owner, type: 'info' },
                    ...prev.logs
                ]
            };
        });
    ...
    const handleTeleportToHubAction = (unit) => {
        ...
        if (unit.type === UnitType.RANGER) {
            if (levelA >= 2) {
                if (levelA === 3 && variantA === 2)
                    cost = 4; // A3-2: Ranger Cost 4
                else
                    cost = 0; // A2: Ranger Cost 0 (Consumable)
            }
            else {
            ...
            // Non-Ranger
            if (!(levelA === 3 && variantA === 2))
                return; // Only A3-2 allows others
            cost = 6;
        }
        if (gameState.players[unit.owner].energy < cost) {
            addLog('log_low_energy', 'info', { cost });
    ...
    const handleThrowMineAction = (unit, r, c) => {
        if (!unit.carriedMine)
            return;
        const baseCost = 4;
        const cost = getEnemyTerritoryEnergyCost(unit, baseCost);
    ...
    const handlePlaceFactoryAction = (unit, r, c) => {
        const p = gameState.players[unit.owner];
        ...
        setGameState(prev => {
            ...
            return {
                ...
                logs: [
                    { turn: prev.turnCount, messageKey: 'log_placed_building', params: { type: '工廠' }, owner: unit.owner, type: 'info' },
                    ...prev.logs
                ]
            };
        });
    ...
    const handleDisarmAction = (unit, r, c) => {
        const state = gameStateRef.current;
        ...
        const revealedMineIndex = gameStateRef.current.mines.findIndex(m => m.r === r && m.c === c && m.owner !== unit.owner && (m.revealedTo.includes(unit.owner) || enemyUnitAtLocation));
        if (revealedMineIndex !== -1) {
            if (!checkEnergyCap(unit, gameStateRef.current.players[unit.owner], cost))
                return;
            spendEnergy(unit.owner, cost);
            lockUnit(unit.id);
            setGameState(prev => {
    ...
    const handleMoveEnemyMineAction = (unit, fromR, fromC, toR, toC) => {
    ...
    const handleConvertEnemyMineAction = (unit, r, c) => {
        const defLevelB = gameState.players[unit.owner].evolutionLevels[UnitType.DEFUSER].b;
        const variantB = gameState.players[unit.owner].evolutionLevels[UnitType.DEFUSER].bVariant;
    ...
    const handleEvolve = (unitType, branch) => {
        const player = gameState.players[gameState.currentPlayer];
        const currentLevel = player.evolutionLevels[unitType][branch];
        const cost = EVOLUTION_COSTS[currentLevel];
        if (player.energy < cost) {
    ...
    const getActionButtonIndex = (actionType, unit) => {
        if (!unit)
            return -1;
        let index = 0;
        ...
        if (canPlaceTower || canPlaceFactory || canPlaceHub) {
            if (actionType === 'place_tower' || actionType === 'place_factory' || actionType === 'place_hub')
                return index;
            index++;
        }
        ...
        if (isOnEnemyBuilding) {
            if (actionType === 'custom_dismantle')
                return index;
            index++;
        }
        ...
        if (unit.type === UnitType.GENERAL) {
            ...
            const rngLevelB = player.evolutionLevels[UnitType.RANGER].b;
            const pickupRadius = rngLevelB >= 1 ? 1 : 0;
            const mineInRange = gameState.mines.find(m => Math.abs(m.r - unit.r) <= pickupRadius &&
                Math.abs(m.c - unit.c) <= pickupRadius &&
                (m.owner === unit.owner || m.revealedTo.includes(unit.owner)));
            if (!unit.carriedMine && mineInRange) {
                if (actionType === 'pickup_mine')
                    return index;
                index++;
            }
            if (unit.carriedMine) {
                if (actionType === 'throw_mine')
                    return index;
                index++;
                if (actionType === 'drop_mine')
                    return index;
                index++;
            }
        }
        ...
        if (canCarryFlag) {
            if (!unit.hasFlag && isAtFlag) {
                if (actionType === 'pickup_flag')
                    return index;
                index++;
            }
            if (unit.hasFlag) {
                if (actionType === 'drop_flag')
                    return index;
                index++;
            }
        }
        ...
        if (actionType === 'end_turn') {
            ...
            const canEvolveA = levelA < 3 && player.energy >= EVOLUTION_COSTS[levelA] && conditionMetA;
            const canEvolveB = levelB < 3 && player.energy >= EVOLUTION_COSTS[levelB] && conditionMetB;
    ...
    const getEvolutionButtonStartIndex = (unit) => {
        if (!unit)
            return -1;
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e) => {
            ...
            if (unit.type === UnitType.GENERAL) {
                ...
                if (!unit.carriedMine && mineAtPosition)
                    buttons.push({ type: 'pickup_mine', action: 'pickup_mine' });
                if (unit.carriedMine) {
                ...
                if (player.evolutionLevels[UnitType.DEFUSER].b >= 2)
                    buttons.push({ type: 'move_mine_start', action: 'move_mine_start' });
                if (player.evolutionLevels[UnitType.DEFUSER].b === 3 && player.evolutionLevels[UnitType.DEFUSER].bVariant === 1)
                    buttons.push({ type: 'convert_mine', action: 'convert_mine' });
            }
            // Flag actions
            const genLevelB = player.evolutionLevels[UnitType.GENERAL].b;
            const canCarryFlag = unit.type === UnitType.GENERAL || genLevelB >= 3;
            ...
            if (canCarryFlag) {
                if (!unit.hasFlag && isAtFlag)
                    buttons.push({ type: 'pickup_flag', action: 'pickup_flag' });
                if (unit.hasFlag)
                    buttons.push({ type: 'drop_flag', action: 'drop_flag' });
            }
            // Evolution buttons
            const levelA = player.evolutionLevels[unit.type].a;
            const levelB = player.evolutionLevels[unit.type].b;
                    ...
                    case 'evolve_a_1':
                        setGameState(prev => {
                            ...
                            return {
                                ...
                                logs: [
                                    { turn: prev.turnCount, messageKey: 'log_committed', type: 'info' },
                                    { turn: prev.turnCount, messageKey: 'log_evolved', params: { unit: getUnitName(unit.type), branch: 'A', level: 3 }, type: 'evolution' },
                                    ...prev.logs
                                ]
                            };
                        });
                    ...
                    case 'evolve_b_1':
                        setGameState(prev => {
                            ...
                            return {
                                ...
                                logs: [
                                    { turn: prev.turnCount, messageKey: 'log_committed', type: 'info' },
                                    { turn: prev.turnCount, messageKey: 'log_evolved', params: { unit: getUnitName(unit.type), branch: 'B', level: 3 }, type: 'evolution' },
                                    ...prev.logs
                                ]
                            };
                        });
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e) => {
            ...
            if (!unit || unit.owner !== state.currentPlayer)
                return;
            if (unit.isDead || unit.hasActedThisRound)
                return;
            if (state.activeUnitId && state.activeUnitId !== unit.id)
                return;
            let dr = 0;
            ...
            if (targetR < 0 || targetR >= GRID_ROWS || targetC < 0 || targetC >= GRID_COLS)
                return;
            const isOccupied = state.players[PlayerID.P1].units.some(u => u.r === targetR && u.c === targetC && !u.isDead) ||
    ...
    const handleActionComplete = (actedUnitId, timedOut = false) => {
        const state = gameStateRef.current;
        if (state.phase === 'thinking' || state.phase === 'placement')
            return;
    ...
    const getNextUnitToAct = (state = gameState) => {
        const player = state.players[state.currentPlayer];
    ...
    const performAITurn = () => {
        const state = gameStateRef.current;
        if (state.gameMode !== 'pve' || state.currentPlayer !== PlayerID.P2 || state.gameOver)
            return;
    ...
    const startNewRound = (prevState) => {
        ...
        const calculateOreIncomeAndConsume = (units) => {
            let income = 0;
            units.forEach(u => {
                if (!u.isDead) {
        ...
        const resetUnits = (units, playerState, playerLogs) => {
            const resurrectedUnits = [];
            const updated = units.map((u, unitIndex) => {
                ...
                if (newU.isDead && newU.respawnTimer === 0 && newU.type !== UnitType.GENERAL) {
                    ...
                    const isOccupied = units.some((unit, idx) => idx !== unitIndex && unit.r === originalSpawnPos.r && unit.c === originalSpawnPos.c && !unit.isDead);
                    if (isOccupied) {
                        ...
                        for (let dr = -1; dr <= 1; dr++) {
                            for (let dc = -1; dc <= 1; dc++) {
                                ...
                                if (nr >= 0 && nr < GRID_ROWS && nc >= 0 && nc < GRID_COLS) {
                                    const cellOccupied = units.some((unit, idx) => idx !== unitIndex && unit.r === nr && unit.c === nc && !unit.isDead);
                        ...
                        if (candidates.length > 0) {
                            ...
                            const availableSpawns = spawnPositions.filter((pos, _) => !units.some((unit, uidx) => uidx !== unitIndex && unit.r === pos.r && unit.c === pos.c && !unit.isDead));
        ...
        setGameState({
            ...
            logs: [{ turn: nextTurn, messageKey: 'log_round_start', params: { round: nextTurn }, type: 'info' }, ...newLogs]
        });
    };
    /*
    ...
    */
    const attemptMove = (unitId, r, c, cost) => {
        ...
        // Rule 1: Energy Cap
        if (!checkEnergyCap(unit, player, totalCost))
            return;
        ...
        if (currentState.cells[r][c].isObstacle)
            return;
        ...
        const proximityNukeIndex = currentState.mines.findIndex(m => m.type === MineType.NUKE &&
        ...
        setGameState(prev => {
            ...
            if (reflectDmg > 0) {
                ...
                if (enemyState.units.length > 0) {
                    ...
                    if (livingEnemies.length > 0) {
                        ...
                        const newEnemyHp = Math.max(0, target.hp - reflectDmg);
                        const isEnemyDead = newEnemyHp === 0;
                        const enemyUpdatedUnits = enemyState.units.map(u => u.id === target.id ? {
                            ...u,
                            hp: newEnemyHp,
                            isDead: isEnemyDead,
            ...
            nukeAoeVictims.forEach(v => {
                ...
                if (v.owner === PlayerID.P1)
                    finalP1 = { ...finalP1, units: newUnits };
                else
                    finalP2 = { ...finalP2, units: newUnits };
            });
    ...
    const renderEvolutionTree = () => {
        ...
        const getProgress = (unitType, branch) => {
            switch (unitType) {
        ...
        return (_jsx("div", { className: "absolute inset-0 z-50 bg-black/80 flex items-center justify-center pointer-events-auto", children: _jsxs("div", { className: "w-[95%] max-w-6xl max-h-[85vh] bg-gray-900 border-2 border-indigo-500 rounded-lg shadow-2xl flex flex-col", children: [_jsxs("div", { className: "flex justify-between items-center p-4 border-b border-gray-700 bg-gray-800 shrink-0", children: [_jsxs("h2", { className: "text-2xl font-bold text-indigo-400 flex items-center gap-2", children: [_jsx(Dna, { size: 24 }), " ", t('evolution_tree')] }), _jsx("button", { onClick: () => setShowEvolutionTree(false), className: "p-2 hover:bg-gray-700 rounded text-gray-400 hover:text-white", children: _jsx(X, { size: 24 }) })] }), _jsx("div", { className: "flex-1 p-4 overflow-y-auto", children: _jsx("div", { className: "space-y-4", children: Object.entries(EVOLUTION_CONFIG).map(([typeStr, config]) => {
                                const type = typeStr;
                                return (_jsxs("div", { className: "bg-gray-800/50 rounded-lg p-4 border border-gray-700", children: [_jsxs("div", { className: "flex items-center gap-2 mb-4 text-indigo-300 font-bold border-b border-gray-700 pb-3", children: [getUnitIcon(type, 24), " ", _jsx("span", { className: "text-lg", children: type })] }), _jsx("div", { className: "grid grid-cols-2 gap-4", children: ['a', 'b'].map((branchKey) => {
                                                ...
                                                return (_jsxs("div", { className: "space-y-2", children: [_jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("div", { className: "text-sm text-gray-300 font-bold flex items-center gap-2", children: [_jsx("span", { className: `w-2.5 h-2.5 rounded-full ${branch === 'a' ? 'bg-blue-500' : 'bg-orange-500'}` }), _jsx("span", { children: t(branch === 'a' ? 'path_a' : 'path_b') }), _jsx("span", { className: "text-xs text-gray-400 font-normal", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_subtitle`) }), currentLevel < 3 && currentVal >= config[branch].thresholds[currentLevel] && (_jsx("span", { className: "text-[10px] text-green-400 font-bold animate-pulse", children: "??READY" }))] }), _jsxs("div", { className: "text-xs text-gray-400 font-mono", children: [currentVal, " / ", maxVal] })] }), _jsx("div", { className: "flex h-2", children: (() => {
                                                                ...
                                                                return (_jsxs(_Fragment, { children: [_jsx("div", { style: { width: `${seg1Width}%` }, className: "bg-gray-700/50 rounded-l-full overflow-hidden border-r border-gray-900/50", children: _jsx("div", { className: `h-full transition-all duration-500 ${barColor}`, style: { width: `${Math.min(100, (currentVal / t1) * 100)}%` } }) }), _jsx("div", { style: { width: `${seg2Width}%` }, className: "bg-gray-700/50 overflow-hidden border-r border-gray-900/50", children: _jsx("div", { className: `h-full transition-all duration-500 ${barColor}`, style: { width: currentVal > t1 ? `${Math.min(100, ((currentVal - t1) / (t2 - t1)) * 100)}%` : '0%' } }) }), _jsx("div", { style: { width: `${seg3Width}%` }, className: "bg-gray-700/50 rounded-r-full overflow-hidden", children: _jsx("div", { className: `h-full transition-all duration-500 ${barColor}`, style: { width: currentVal > t2 ? `${Math.min(100, ((currentVal - t2) / (maxVal - t2)) * 100)}%` : '0%' } }) })] }));
                                                            })() }), _jsxs("div", { className: "flex items-center h-24", children: [_jsx("div", { className: "flex-1 relative h-full", children: (() => {
                                                                        const cardId = `${type}-${branch}-1`;
                                                                        const isFlipped = flippedCardId === cardId;
                                                                        const isUnlocked = currentLevel >= 1;
                                                                        return (_jsx("div", { onClick: () => setFlippedCardId(isFlipped ? null : cardId), className: `absolute inset-0 z-10 p-2 rounded border flex flex-col justify-center cursor-pointer transition-all duration-300 ${isFlipped ? 'scale-110 !h-auto min-h-[140px] !z-50 bg-[#0a0f1a] border-indigo-400 shadow-2xl overflow-visible' : `h-full ${isUnlocked ? 'bg-blue-950/40 border-blue-600/80 text-blue-100' : 'bg-gray-800/50 border-gray-600 text-gray-500 opacity-60'}`}`, children: isFlipped ? (_jsx("div", { className: "text-[11px] leading-relaxed whitespace-pre-wrap py-1 text-blue-50", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r1_detail`) })) : (_jsxs("div", { className: "flex justify-between items-center w-full", children: [_jsxs("div", { className: "flex flex-col min-w-0", children: [_jsx("div", { className: "font-bold mb-0.5 opacity-70", children: "LV1" }), _jsx("div", { className: "text-[11px] leading-tight font-black truncate", children: t(config[branch].rewardText[0]) })] }), _jsxs("div", { className: "text-[10px] text-gray-300 font-bold flex items-center gap-1 shrink-0 ml-2", children: [isUnlocked && _jsx("div", { className: "w-2.5 h-2.5 bg-green-500 rounded-full flex items-center justify-center text-[6px] text-white shrink-0" }), _jsx("span", { className: "opacity-80", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r1_req`) })] })] })) }));
                                                                    })() }), _jsx("div", { className: "w-8 h-0.5 bg-gray-700/80 shrink-0" }), _jsx("div", { className: "flex-1 relative h-full", children: (() => {
                                                                        const cardId = `${type}-${branch}-2`;
                                                                        const isFlipped = flippedCardId === cardId;
                                                                        const isUnlocked = currentLevel >= 2;
                                                                        return (_jsx("div", { onClick: () => setFlippedCardId(isFlipped ? null : cardId), className: `absolute inset-0 z-10 p-2 rounded border flex flex-col justify-center cursor-pointer transition-all duration-300 ${isFlipped ? 'scale-110 !h-auto min-h-[140px] !z-50 bg-[#1a0a1f] border-indigo-400 shadow-2xl' : `h-full ${isUnlocked ? 'bg-purple-950/40 border-purple-600/80 text-purple-100' : 'bg-gray-800/50 border-gray-600 text-gray-500 opacity-60'}`}`, children: isFlipped ? (_jsx("div", { className: "text-[11px] leading-relaxed whitespace-pre-wrap py-1 text-purple-50", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r2_detail`) })) : (_jsxs("div", { className: "flex justify-between items-center w-full", children: [_jsxs("div", { className: "flex flex-col min-w-0", children: [_jsx("div", { className: "font-bold mb-0.5 opacity-70", children: "LV2" }), _jsx("div", { className: "text-[11px] leading-tight font-black truncate", children: t(config[branch].rewardText[1]) })] }), _jsxs("div", { className: "text-[10px] text-gray-300 font-bold flex items-center gap-1 shrink-0 ml-2", children: [isUnlocked && _jsx("div", { className: "w-2.5 h-2.5 bg-green-500 rounded-full flex items-center justify-center text-[6px] text-white shrink-0" }), _jsx("span", { className: "opacity-80", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r2_req`) })] })] })) }));
                                                                    })() }), currentLevel === 3 && selectedVariant ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "w-8 h-0.5 bg-gray-700/80 shrink-0" }), _jsx("div", { className: "flex-1 relative h-full", children: (() => {
                                                                                const subLevel = selectedVariant;
                                                                                const cardId = `${type}-${branch}-3-${subLevel}`;
                                                                                const isFlipped = flippedCardId === cardId;
                                                                                const cardIdx = (selectedVariant + 1);
                                                                                return (_jsx("div", { onClick: () => setFlippedCardId(isFlipped ? null : cardId), className: `absolute inset-0 z-10 p-2 rounded border flex flex-col justify-center cursor-pointer transition-all duration-300 ${isFlipped ? 'scale-110 !h-auto min-h-[140px] !z-50 bg-[#1f100a] border-indigo-400 shadow-2xl' : 'h-full bg-orange-950/40 border-orange-500 text-orange-100 shadow-[0_0_12px_rgba(249,115,22,0.15)]'}`, children: isFlipped ? (_jsx("div", { className: "text-[11px] leading-relaxed whitespace-pre-wrap py-1 text-orange-50", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r3_${subLevel}_detail`) })) : (_jsxs("div", { className: "flex justify-between items-center w-full", children: [_jsxs("div", { className: "flex flex-col min-w-0", children: [_jsxs("div", { className: "font-bold mb-0.5 opacity-70", children: ["LV3-", subLevel] }), _jsx("div", { className: "text-[11px] leading-tight font-black truncate", children: t(config[branch].rewardText[cardIdx]) })] }), _jsxs("div", { className: "text-[10px] text-gray-300 font-bold flex items-center gap-1 shrink-0 ml-2", children: [_jsx("div", { className: "w-2.5 h-2.5 bg-green-500 rounded-full flex items-center justify-center text-[6px] text-white shrink-0" }), _jsx("span", { className: "opacity-80", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r3_req`) })] })] })) }));
                                                                            })() })] })) : (_jsxs(_Fragment, { children: [_jsx("div", { className: "flex flex-col items-center justify-center text-gray-700/80 w-8 shrink-0", children: _jsxs("svg", { width: "32", height: "80", viewBox: "0 0 32 80", fill: "none", xmlns: "http://www.w3.org/2000/svg", children: [_jsx("path", { d: "M0 40H8C12 40 14 39 14 36V18C14 15 16 14 24 14H32", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round" }), _jsx("path", { d: "M14 44V62C14 65 16 66 24 66H32", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round" }), _jsx("circle", { cx: "14", cy: "40", r: "2.5", fill: "currentColor", className: "animate-pulse" })] }) }), _jsx("div", { className: "flex flex-col flex-1 gap-1.5 h-full", children: [2, 3].map((idx) => {
                                                                                ...
                                                                                const isFlipped = flippedCardId === cardId;
                                                                                return (_jsx("div", { className: "relative h-[46px]", children: _jsx("div", { onClick: () => {
                                                                                            if (isOtherVariantSelected)
                                                                                                return;
                                                                                            setFlippedCardId(isFlipped ? null : cardId);
                                                                                        }, className: `absolute inset-0 z-10 p-2 rounded border flex flex-col justify-center cursor-pointer transition-all duration-300 ${isFlipped ? 'scale-110 !h-auto min-h-[140px] !z-50 bg-[#1a1a1a] border-indigo-400 shadow-2xl' : `h-full ${isUnlocked ? 'bg-orange-950/40 border-orange-500 text-orange-100 shadow-[0_0_12px_rgba(249,115,22,0.2)]' : isOtherVariantSelected ? 'bg-transparent border-gray-800 text-gray-700 opacity-20 grayscale pointer-events-none' : 'bg-gray-800/40 border-gray-600/50 text-gray-500 hover:border-gray-500'}`}`, children: isFlipped ? (_jsx("div", { className: "text-[11px] leading-relaxed whitespace-pre-wrap py-1 text-gray-100", children: t(`evol_${getUnitTypeAbbr(type)}_${branch}_r3_${subLevel}_detail`) })) : (_jsxs("div", { className: "flex justify-between items-center h-full", children: [_jsxs("div", { className: "flex flex-col justify-center min-w-0", children: [_jsxs("div", { className: "flex items-center gap-1.5 mb-0.5", children: [_jsxs("span", { className: `text-[9px] ${isUnlocked ? 'text-orange-400' : 'text-gray-500'} font-bold`, children: ["LV3-", subLevel] }), isUnlocked && _jsx("span", { className: "text-[7px] px-1 bg-orange-600 text-white rounded-sm font-black animate-pulse", children: "ACTIVED" })] }), _jsx("div", { className: "text-[10px] leading-tight truncate font-bold", children: t(config[branch].rewardText[idx]) })] }), _jsxs("div", { className: "text-[10px] text-gray-400 font-bold italic shrink-0 ml-2 text-right leading-tight", children: [t(`evol_${getUnitTypeAbbr(type)}_${branch}_r3_req`), !selectedVariant && currentLevel === 2 && currentVal >= config[branch].thresholds[2] && _jsx("div", { className: "text-green-400 font-black animate-bounce mt-0.5 text-[8px]", children: "SELECT" })] })] })) }) }, idx));
                                                                            }) })] }))] })] }, branch));
                                            }) })] }, type));
                            }) }) })] }) }));
    };
    const renderControlPanel = () => {
        ...
        return (_jsx("div", { className: "h-56 bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 border-t-4 border-white flex shrink-0 z-20 shadow-2xl shadow-white/10", children: _jsxs("div", { className: "flex w-full", children: [_jsxs("div", { className: "flex-[3] flex flex-col p-3 border-r-2 border-white/30 min-w-[200px] bg-slate-800/50 gap-2", children: [_jsxs("div", { className: "flex flex-col gap-2", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Zap, { size: 20, className: "text-yellow-400 drop-shadow-lg" }), _jsx("span", { className: "text-4xl font-black text-yellow-400 drop-shadow-lg", children: player.energy })] }), _jsxs("div", { className: "text-xs text-white space-y-1 font-semibold", children: [_jsxs("div", { className: "flex justify-between items-center bg-slate-900/50 p-1.5 rounded border border-emerald-500/30", children: [_jsx("span", { className: "text-white/80 text-xs", children: "\u4E0B\u56DE\u5408\u9810\u8A08\u6536\u76CA" }), _jsxs("span", { className: "text-emerald-400 font-black text-lg", children: ["+", totalIncome] })] }), _jsxs("div", { className: "grid grid-cols-2 gap-1.5 text-[11px] bg-slate-900/30 p-1.5 rounded border border-white/20", children: [_jsxs("div", { className: "flex justify-between", children: [_jsxs("span", { className: "flex items-center gap-1", children: [_jsx("span", { className: "w-2 h-2 bg-blue-400 rounded-full" }), "\u57FA\u790E"] }), _jsxs("span", { className: "text-blue-300 font-bold", children: ["+", currentRegen] })] }), _jsxs("div", { className: "flex justify-between", children: [_jsxs("span", { className: "flex items-center gap-1", children: [_jsx("span", { className: "w-2 h-2 bg-emerald-400 rounded-full" }), "\u5229\u606F"] }), _jsxs("span", { className: "text-emerald-300 font-bold", children: ["+", interest] })] }), _jsxs("div", { className: "flex justify-between", children: [_jsxs("span", { className: "flex items-center gap-1", children: [_jsx("span", { className: "w-2 h-2 bg-yellow-400 rounded-full" }), "\u9818\u5730/\u7926\u77F3"] }), _jsxs("span", { className: "text-yellow-300 font-bold", children: ["+", currentOreIncome] })] }), _jsxs("div", { className: "flex justify-between", children: [_jsxs("span", { className: "flex items-center gap-1", children: [_jsx("span", { className: "w-2 h-2 bg-red-400 rounded-full" }), "\u64CA\u6BBA"] }), _jsxs("span", { className: "text-red-300 font-bold", children: ["+", player.energyFromKills] })] })] })] })] }), _jsxs("button", { onClick: () => setShowEvolutionTree(true), className: "mt-auto py-1 px-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 rounded font-black text-[11px] flex items-center justify-center gap-1 border-2 border-purple-400 shadow-lg shadow-purple-500/50 transition-all hover:scale-105 text-white", children: [_jsx(Dna, { size: 14 }), " \u9032\u5316\u6A39"] })] }), _jsxs("div", { className: "flex-[7] flex flex-col border-l-2 border-white/30 px-4 items-center h-full justify-center bg-slate-800/30", children: [_jsxs("div", { className: "text-sm text-white mb-2 uppercase tracking-widest text-center w-full flex justify-between px-4 font-bold", children: [_jsx("span", { className: "text-base", children: "\u5C0F\u968A\u9078\u55AE" }), _jsx("span", { className: "text-yellow-400 font-black text-base", children: isPlacement ? `${t('round')} ${gameState.turnCount}` : `${t('round')} ${gameState.turnCount}-${player.units.filter(u => u.hasActedThisRound).length + 1}` })] }), _jsx("div", { className: "flex gap-4 justify-center w-full", children: player.units.map((u) => {
                                    ...
                                    return (_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { disabled: u.isDead || u.hasActedThisRound, onClick: () => handleUnitClick(u), className: `
                                    ...
                                    `, children: [_jsxs("div", { className: "absolute top-0.5 right-0.5 flex flex-col gap-0.5", children: [levelA > 0 && (_jsx("div", { className: "flex gap-0.5", children: Array.from({ length: levelA }).map((_, i) => {
                                                                    ...
                                                                    if (levelA === 3 && variantA) {
                                                                        if (variantA === 1)
                                                                            colorClass = "text-cyan-400 fill-cyan-400"; // 3-1 Light Blue
                                                                        else if (variantA === 2)
                                                                            colorClass = "text-purple-400 fill-purple-400"; // 3-2 Purple
                                                                    }
                                                                    else if (i === 2 && variantA) {
                                                                        // Fallback for partial data if level is 3 but variant only applies to last
                                                                        if (variantA === 1)
                                                                            colorClass = "text-cyan-400 fill-cyan-400";
                                                                        else if (variantA === 2)
                                                                            colorClass = "text-purple-400 fill-purple-400";
                                                                    }
                                                                    return _jsx(Star, { size: 8, className: `${colorClass} drop-shadow-sm` }, `a-${i}`);
                                                                }) })), levelB > 0 && (_jsx("div", { className: "flex gap-0.5", children: Array.from({ length: levelB }).map((_, i) => {
                                                                    const variantB = player.evolutionLevels[u.type].bVariant;
                                                                    let colorClass = "text-orange-400 fill-orange-400";
                                                                    // If Level 3 is reached with a variant, make ALL stars that color
                                                                    if (levelB === 3 && variantB) {
                                                                        if (variantB === 1)
                                                                            colorClass = "text-yellow-400 fill-yellow-400"; // 3-1 Light Yellow
                                                                        else if (variantB === 2)
                                                                            colorClass = "text-rose-500 fill-rose-500"; // 3-2 Red
                                                                    }
                                                                    ...
                                                                    return _jsx(Star, { size: 8, className: `${colorClass} drop-shadow-sm` }, `b-${i}`);
                                                                }) }))] }), _jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: u.type === UnitType.GENERAL ? 'Q' : u.type === UnitType.MINESWEEPER ? 'W' : u.type === UnitType.RANGER ? 'E' : u.type === UnitType.MAKER ? 'R' : 'T' }), u.isDead && _jsx(Skull, { size: 40, className: "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white z-20 drop-shadow-lg" }), canSwap && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center pointer-events-none", children: _jsxs("svg", { className: "w-10 h-10 text-emerald-300 opacity-40", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round", strokeLinejoin: "round", children: [_jsx("path", { d: "M7 16V4m0 0L3 8m4-4l4 4" }), _jsx("path", { d: "M17 8v12m0 0l4-4m-4 4l-4-4" })] }) })), _jsx("div", { className: "flex-1 flex flex-col items-center justify-center w-full pt-1", children: _jsx("div", { className: `${u.owner === PlayerID.P1 ? 'text-cyan-400 drop-shadow-lg' : 'text-red-400 drop-shadow-lg'} flex items-center justify-center`, children: getUnitIcon(u.type, 30, tier) }) }), _jsxs("div", { className: "flex flex-col items-center gap-0.5 pb-0", children: [_jsx("div", { className: "w-12 h-1.5 bg-slate-950 rounded-full overflow-hidden border border-slate-600", children: _jsx("div", { className: `h-full transition-all ${u.hp < u.maxHp * 0.3 ? 'bg-red-500 shadow-lg shadow-red-500/50' : 'bg-emerald-500 shadow-lg shadow-emerald-500/50'}`, style: { width: `${(u.hp / u.maxHp) * 100}%` } }) }), _jsxs("div", { className: "text-[8px] font-black text-white font-mono", children: ["HP:", u.hp] })] }), _jsx("div", { className: "text-[9px] font-black text-slate-300 pb-0.5", children: getUnitName(u.type) }), u.isDead && u.respawnTimer > 0 && (_jsxs("div", { className: "text-[10px] font-black text-red-500 font-mono", children: ["\u5FA9\u6D3B:", u.respawnTimer] }))] }), _jsxs("div", { className: "text-[10px] font-black text-cyan-300 font-mono bg-slate-900/50 px-2 py-1 rounded border border-slate-700", children: [u.energyUsedThisTurn, "/", Math.floor(u.startOfActionEnergy * ENERGY_CAP_RATIO)] })] }, u.id));
                                }) })] }), _jsx("div", { className: "flex-[4] flex flex-col border-l-2 border-white/30 px-4 items-center justify-between h-full py-1 bg-slate-800/30", children: isPlacement ? (_jsxs("div", { className: "w-full h-full flex flex-col items-center justify-center gap-3", children: [_jsx("div", { className: "text-sm font-black text-white uppercase tracking-widest animate-pulse drop-shadow-lg", children: t('placement_phase') }), _jsx("div", { className: "text-[10px] text-white text-center font-semibold", children: t('placement_guide') }), _jsxs("div", { className: "flex gap-2 w-full", children: [_jsxs("button", { onClick: () => setTargetMode(targetMode === 'place_setup_mine' ? null : 'place_setup_mine'), className: `flex-1 py-2 px-1 rounded font-black text-xs flex items-center justify-center gap-1 border-2 transition-all ${targetMode === 'place_setup_mine' ? 'bg-purple-600 border-purple-400 text-white shadow-lg shadow-purple-500/50' : 'bg-slate-700 border-slate-600 hover:bg-slate-600 hover:border-purple-500 text-slate-300'}`, children: [_jsx(Bomb, { size: 14 }), " ", t('place_setup_mine'), " (", player.placementMinesPlaced, "/", PLACEMENT_MINE_LIMIT, ")"] }), _jsxs("button", { onClick: finishPlacementPhase, className: "flex-1 py-2 px-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-black text-xs flex items-center justify-center gap-1 border-2 border-emerald-400 shadow-lg shadow-emerald-500/50 transition-all", children: [_jsx(CheckCircle, { size: 14 }), " ", t('confirm_placement')] })] })] })) : (_jsxs(_Fragment, { children: [_jsx("div", { className: "text-base text-white uppercase tracking-widest text-center w-full font-bold mt-6", children: gameState.selectedUnitId ? t('select_action') : t('select_unit') }), _jsx("div", { className: "flex-1 flex flex-col justify-center w-full", children: _jsx("div", { className: "w-full flex flex-col justify-center gap-2 items-center relative min-w-[150px]", children: isThinking ? (_jsxs("div", { className: "flex flex-col items-center gap-2", children: [_jsx("span", { className: "text-white font-black animate-pulse uppercase tracking-widest text-xs drop-shadow-lg", children: t('planning_phase') }), _jsxs("button", { onClick: () => startActionPhase(), className: "px-6 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 rounded font-black shadow-lg shadow-cyan-500/50 flex items-center gap-2 border-2 border-cyan-400 transition-all", children: [_jsx(Play, { size: 20, fill: "currentColor" }), " ", t('ready')] })] })) : gameState.selectedUnitId ? (_jsxs("div", { className: "flex gap-2 justify-center flex-wrap", children: [_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode('move'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'move' ? 'bg-emerald-600 shadow-lg shadow-emerald-500/50 scale-105 border-emerald-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-emerald-500 text-slate-300'}`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('move', getUnit(gameState.selectedUnitId)) }), _jsx(ArrowRight, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('move') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: (() => {
                                                    ...
                                                    if (unit.type === UnitType.GENERAL) {
                                                        ...
                                                        if (canAttack || gameState.isGodMode) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode('attack'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'attack' ? 'bg-red-600 shadow-lg shadow-red-500/50 scale-105 border-red-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-red-500 text-slate-300'}`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('attack', unit) }), _jsx(Swords, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('attack') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 8) })] })] }, "attack"));
                                                        }
                                                    }
                                                    ...
                                                    if (unit.type !== UnitType.DEFUSER) {
                                                        const isOnEnemyBuilding = gameState.buildings.some(b => b.r === unit.r && b.c === unit.c && b.owner !== unit.owner);
                                                        if (isOnEnemyBuilding) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleDisarmAction(unit, unit.r, unit.c), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-indigo-500 text-slate-300", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('custom_dismantle', unit) }), _jsx(Unlock, { size: 20 }), _jsx("span", { className: "text-[10px]", children: language === 'zh_tw' ? '拆除敵方建築' : 'Dismantle' })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 3) })] })] }, "custom_dismantle"));
                                                        }
                                                    }
                                                    ...
                                                    if (unit.type === UnitType.GENERAL) {
                                                        const isAtFlag = unit.r === player.flagPosition.r && unit.c === player.flagPosition.c;
                                                        if (!unit.hasFlag && isAtFlag) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: handlePickupFlag, className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative bg-yellow-600/70 hover:bg-yellow-600 border-yellow-500 cursor-pointer`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('pickup_flag', unit) }), _jsx(Flag, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('take') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "0" })] })] }, "pickup"));
                                                        }
                                                        if (unit.hasFlag) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: handleDropFlag, className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative bg-yellow-600/70 hover:bg-yellow-600 font-bold border-2 border-yellow-500", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('drop_flag', unit) }), _jsx(ArrowDownToLine, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('drop') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "0" })] })] }, "drop"));
                                                        }
                                                    }
                                                    // Minesweeper Actions
                                                    if (unit.type === UnitType.MINESWEEPER) {
                                                        ...
                                                        if (swpLevelA >= 1) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handlePlaceTowerAction(unit, unit.r, unit.c), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-indigo-500 text-slate-300", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('place_tower', getUnit(gameState.selectedUnitId)) }), _jsx(Radio, { size: 20 }), _jsx("span", { className: "text-[10px]", children: language === 'zh_tw' ? '部署掃雷塔' : 'Deploy Tower' })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 8) })] })] }, "place_tower"));
                                                            // Path A Lvl 3-2: Detonate Tower
                                                            if (swpLevelA === 3 && variantA === 2) {
                                                                buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleDetonateTowerAction(unit), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold bg-red-600 hover:bg-red-500 border-2 border-red-400 text-white shadow-lg", children: [_jsx(Bomb, { size: 20 }), _jsx("span", { className: "text-[10px]", children: language === 'zh_tw' ? '引爆塔' : 'Detonate' })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "2" })] })] }, "detonate_tower"));
                                                            }
                                                        }
                                                        // Default Scan
                                                        const baseScanCost = (swpLevelB >= 3) ? 3 : (swpLevelB >= 2 ? 4 : UNIT_STATS[UnitType.MINESWEEPER].scanCost);
                                                        buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode('scan'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'scan' ? 'bg-cyan-600 shadow-lg shadow-cyan-500/50 scale-105 border-cyan-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-cyan-500 text-slate-300'}`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('scan', getUnit(gameState.selectedUnitId)) }), _jsx(Eye, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('scan') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, baseScanCost) })] })] }, "scan"));
                                                    }
                                                    // Maker place mine + Factory
                                                    if (unit.type === UnitType.MAKER) {
                                                        const mkrLevelB = player.evolutionLevels[UnitType.MAKER].b;
                                                        if (mkrLevelB >= 1) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handlePlaceFactoryAction(unit, unit.r, unit.c), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-orange-500 text-slate-300", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('place_factory', getUnit(gameState.selectedUnitId)) }), _jsx(FlaskConical, { size: 20 }), _jsx("span", { className: "text-[10px]", children: language === 'zh_tw' ? '部署自動工廠' : 'Deploy Factory' })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, (player.evolutionLevels[UnitType.MAKER].b === 3 && player.evolutionLevels[UnitType.MAKER].bVariant === 2) ? 4 : 6) })] })] }, "place_factory"));
                                                        }
                                                        buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("div", { className: "flex items-stretch gap-0", children: [(() => {
                                                                            ...
                                                                            return (_jsxs("button", { onClick: () => setTargetMode(targetMode === 'place_mine' ? null : 'place_mine'), className: `px-3 py-2 flex flex-col items-center justify-center gap-1 min-w-[72px] transition-all relative font-bold border-2 ${isSelected ? `bg-purple-600 border-white z-20 ${hasMenu ? 'rounded-l border-r-0' : 'rounded'} shadow-[0_0_15px_rgba(168,85,247,0.4)]` : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-purple-500 text-slate-300 rounded'}`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('place_mine', getUnit(gameState.selectedUnitId)) }), _jsx(Bomb, { size: 20 }), _jsx("span", { className: "text-[10px]", children: "\u653E\u7F6E\u5730\u96F7" })] }));
                                                                        })(), _jsx("div", { className: `overflow-hidden transition-all duration-300 flex items-center ${targetMode === 'place_mine' && (player.evolutionLevels[UnitType.MAKER].a >= 1 || player.evolutionLevels[UnitType.MAKER].b >= 2) ? 'max-w-xs opacity-100 scale-100' : 'max-w-0 opacity-0 scale-50 pointer-events-none'}`, children: _jsxs("div", { className: "grid grid-cols-2 gap-1 px-2 bg-slate-900/90 rounded-r border-2 border-l-0 border-white h-full items-center shadow-xl backdrop-blur-md -ml-px", children: [_jsx("button", { onClick: () => setSelectedMineType(MineType.NORMAL), className: `w-9 h-6 rounded border ${selectedMineType === MineType.NORMAL ? 'bg-purple-600 border-white shadow-[0_0_10px_rgba(168,85,247,0.6)]' : 'bg-slate-800 border-slate-700 hover:border-purple-400'} text-[10px] font-black flex items-center justify-center transition-all hover:scale-110 active:scale-90`, title: "\u4E00\u822C\u5730\u96F7", children: "\u666E" }), (player.evolutionLevels[UnitType.MAKER].a >= 1 || player.evolutionLevels[UnitType.MAKER].b >= 2) && (_jsx("button", { onClick: () => setSelectedMineType(MineType.SMOKE), className: `w-9 h-6 rounded border ${selectedMineType === MineType.SMOKE ? 'bg-blue-600 border-white shadow-[0_0_10px_rgba(59,130,246,0.6)]' : 'bg-slate-800 border-slate-700 hover:border-blue-400'} text-[10px] font-black flex items-center justify-center transition-all hover:scale-110 active:scale-90`, title: "\u7159\u9727\u5730\u96F7", children: "\u7159" })), (player.evolutionLevels[UnitType.MAKER].a >= 2 || player.evolutionLevels[UnitType.MAKER].b >= 2) && (_jsx("button", { onClick: () => setSelectedMineType(MineType.SLOW), className: `w-9 h-6 rounded border ${selectedMineType === MineType.SLOW ? 'bg-amber-500 border-white shadow-[0_0_10px_rgba(245,158,11,0.6)]' : 'bg-slate-800 border-slate-700 hover:border-amber-400'} text-[10px] font-black flex items-center justify-center transition-all hover:scale-110 active:scale-90`, title: "\u7DE9\u901F\u5730\u96F7", children: "\u7DE9" })), player.evolutionLevels[UnitType.MAKER].a >= 3 && (_jsx("button", { onClick: () => setSelectedMineType(player.evolutionLevels[UnitType.MAKER].aVariant === 1 ? MineType.CHAIN : MineType.NUKE), className: `w-9 h-6 rounded border ${selectedMineType === MineType.NUKE || selectedMineType === MineType.CHAIN ? 'bg-red-600 border-white shadow-[0_0_15px_rgba(220,38,38,0.7)]' : 'bg-slate-800 border-slate-700 hover:border-red-400'} text-[10px] font-black flex items-center justify-center animate-pulse transition-all hover:scale-110 active:scale-90`, title: player.evolutionLevels[UnitType.MAKER].aVariant === 1 ? "連鎖反應地雷" : "核子爆炸地雷", children: player.evolutionLevels[UnitType.MAKER].aVariant === 1 ? '連' : '核' }))] }) })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white shadow-inner", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { className: "min-w-[10px] text-center", children: (() => {
                                                                                const mkrLevelB = player.evolutionLevels[UnitType.MAKER].b;
                                                                                const mkrVariantB = player.evolutionLevels[UnitType.MAKER].bVariant;
                                                    ...
                                                    if (unit.type === UnitType.DEFUSER) {
                                                        ...
                                                        buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode(targetMode === 'disarm' ? null : 'disarm'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'disarm' ? 'bg-teal-600 shadow-lg shadow-teal-500/50 scale-105 border-teal-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-teal-500 text-slate-300'}`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('disarm', getUnit(gameState.selectedUnitId)) }), _jsx(Unlock, { size: 20 }), _jsx("span", { className: "text-[10px]", children: (() => {
                                                                                const u = getUnit(gameState.selectedUnitId);
                                                                                if (u) {
                                                                                    const hasBuilding = gameState.buildings.some(b => b.r === u.r && b.c === u.c && b.owner !== u.owner);
                                                                                    if (hasBuilding)
                                                                                        return language === 'zh_tw' ? '拆除建築' : 'Dismantle';
                                                                                }
                                                                                return t('disarm');
                                                                            })() })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 3) })] })] }, "disarm"));
                                                        // Path B LV2+: Move Enemy Mine
                                                        if (defLevelB >= 2) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode(targetMode === 'move_mine_start' ? null : 'move_mine_start'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'move_mine_start' || targetMode === 'move_mine_end' ? 'bg-amber-600 shadow-lg shadow-amber-500/50 scale-105 border-amber-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-stone-500 text-slate-300'}`, children: [_jsx(ChevronRight, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('move_mine') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, (defLevelB === 3 && variantB === 2) ? 5 : 2) })] })] }, "move_mine"));
                                                        }
                                                        // Path B LV3-1: Convert Mine
                                                        if (defLevelB === 3 && variantB === 1) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode(targetMode === 'convert_mine' ? null : 'convert_mine'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'convert_mine' ? 'bg-indigo-600 shadow-lg shadow-indigo-500/50 scale-105 border-indigo-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-indigo-500 text-slate-300'}`, children: [_jsx(Users, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('convert_mine') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 5) })] })] }, "convert_mine"));
                                                        }
                                                    }
                                                    ...
                                                    if (unit.type === UnitType.RANGER) {
                                                        ...
                                                        const canTeleport = ((unit.type === UnitType.RANGER && rangerLevelA >= 2) || (rangerLevelA === 3 && rangerVariantA === 2)) && hasHub;
                                                        if (canTeleport) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleTeleportToHubAction(unit), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 bg-purple-600 hover:bg-purple-500 shadow-lg shadow-purple-500/50 border-purple-400`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('teleport', getUnit(gameState.selectedUnitId)) }), _jsx(Zap, { size: 20 }), _jsx("span", { className: "text-[10px]", children: language === 'zh_tw' ? '傳送' : 'Teleport' })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "5" })] })] }, "hub_teleport"));
                                                        }
                                                        ...
                                                        if (rngLevelA >= 1) {
                                                            buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handlePlaceHubAction(unit, unit.r, unit.c), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-blue-500 text-slate-300", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('place_hub', getUnit(gameState.selectedUnitId)) }), _jsx(Cpu, { size: 20 }), _jsx("span", { className: "text-[10px]", children: language === 'zh_tw' ? '部署樞紐' : 'Deploy Hub' })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 8) })] })] }, "place_hub"));
                                                            if (!unit.carriedMine) {
                                                                const rngLevelB = player.evolutionLevels[UnitType.RANGER].b;
                                                                const pickupRadius = rngLevelB >= 1 ? 1 : 0;
                                                                const mineInRange = gameState.mines.find(m => Math.abs(m.r - unit.r) <= pickupRadius &&
                                                                    ...
                                                                    (m.owner === unit.owner || m.revealedTo.includes(unit.owner)));
                                                                if (mineInRange) {
                                                                    buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleRangerAction('pickup'), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative bg-yellow-600 hover:bg-yellow-500 animate-pulse shadow-lg shadow-yellow-500/50 border-yellow-400 cursor-pointer", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('pickup_mine', getUnit(gameState.selectedUnitId)) }), _jsx(Hand, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('take_mine') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "0" })] })] }, "pickup_mine"));
                                                                }
                                                                // Stealth Action (B2)
                                                                if (rngLevelB === 2) {
                                                                    buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleStealthAction(unit.id), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${unit.status.isStealthed ? 'bg-indigo-900 border-indigo-600' : 'bg-indigo-700 hover:bg-indigo-600 border-indigo-400'} shadow-lg shadow-indigo-500/50`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('stealth', getUnit(gameState.selectedUnitId)) }), _jsx(Ghost, { size: 20, className: unit.status.isStealthed ? 'opacity-50' : '' }), _jsx("span", { className: "text-[10px]", children: unit.status.isStealthed ? (language === 'zh_tw' ? '取消隱形' : 'Cancel Stealth') : (language === 'zh_tw' ? '隱形行動' : 'Stealth Action') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: unit.status.isStealthed ? 0 : 3 })] })] }, "stealth_action"));
                                                                }
                                                            }
                                                            else {
                                                                // Path B Tier 3-2: Throw Mine
                                                                if (player.evolutionLevels[UnitType.RANGER].b === 3 && player.evolutionLevels[UnitType.RANGER].bVariant === 2) {
                                                                    buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => setTargetMode(targetMode === 'throw_mine' ? null : 'throw_mine'), className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${targetMode === 'throw_mine' ? 'bg-orange-600 shadow-lg shadow-orange-500/50 scale-105 border-orange-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600 hover:border-orange-500 text-slate-300'}`, children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('throw_mine', getUnit(gameState.selectedUnitId)) }), _jsx(Bomb, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('throw_mine') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: getDisplayCost(unit, 4) })] })] }, "throw_mine"));
                                                                }
                                                                buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleRangerAction('drop'), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative bg-yellow-600 hover:bg-yellow-500 animate-pulse shadow-lg shadow-yellow-500/50 font-bold border-2 border-yellow-400", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('drop_mine', getUnit(gameState.selectedUnitId)) }), _jsx(ArrowDownToLine, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('drop_mine') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "0" })] })] }, "drop_mine"));
                                                            }
                                                        }
                                                    }
                                                    ...
                                                    if (unit.type !== UnitType.GENERAL) {
                                                        ...
                                                        if (canCarry) {
                                                            if (!unit.hasFlag && isAtFlag) {
                                                                buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: handlePickupFlag, disabled: !isAtFlag, className: `px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative font-bold border-2 ${isAtFlag ? 'bg-yellow-600/70 hover:bg-yellow-600 border-yellow-500 cursor-pointer' : 'bg-slate-600 border-slate-500 text-slate-400 cursor-not-allowed opacity-50'}`, children: [isAtFlag && _jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('pickup_flag', getUnit(gameState.selectedUnitId)) }), _jsx(Flag, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('take') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "0" })] })] }, "pickup"));
                                                            }
                                                            if (unit.hasFlag) {
                                                                buttons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: handleDropFlag, className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative bg-yellow-600/70 hover:bg-yellow-600 font-bold border-2 border-yellow-500", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('drop_flag', getUnit(gameState.selectedUnitId)) }), _jsx(ArrowDownToLine, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('drop') })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-center gap-0.5 text-[10px] font-bold text-white", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), _jsx("span", { children: "0" })] })] }, "drop"));
                                                            }
                                                        }
                                                    }
                                                    return buttons;
                                                    ...
                                                    if (needsVariantA) {
                                                        ...
                                                        evolveButtons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => {
                                                                        setGameState(prev => {
                                                                            const p = prev.players[prev.currentPlayer];
                                                                            const newLevels = { ...p.evolutionLevels };
                                                                            newLevels[unit.type] = { ...newLevels[unit.type], a: 3, aVariant: 1 };
                                                                    ...
                                                                    }, className: "px-3 py-2 rounded flex flex-col items-center gap-0.5 min-w-[60px] transition-all relative bg-cyan-600 hover:bg-cyan-500 font-bold border-2 border-cyan-400 shadow-lg shadow-cyan-500/50 text-white animate-pulse", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: buttonIndex }), _jsx(Dna, { size: 20 }), _jsx("span", { className: "text-[9px] leading-none", children: language === 'zh_tw' ? '進化 A' : 'Evolve A' }), _jsx("span", { className: "text-[8px] leading-none text-cyan-200", children: "LV2 \u2192 3-1" })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), "                                ", _jsx("span", { children: EVOLUTION_COSTS[2] })] })] }, "evolve_a_lv3_1"));
                                                        buttonIndex++;
                                                        evolveButtons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => {
                                                                        setGameState(prev => {
                                                                            const p = prev.players[prev.currentPlayer];
                                                                    ...
                                                                    }, className: "px-3 py-2 rounded flex flex-col items-center gap-0.5 min-w-[60px] transition-all relative bg-purple-600 hover:bg-purple-500 font-bold border-2 border-purple-400 shadow-lg shadow-purple-500/50 text-white animate-pulse", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: buttonIndex }), _jsx(Dna, { size: 20 }), _jsx("span", { className: "text-[9px] leading-none", children: language === 'zh_tw' ? '進化 A' : 'Evolve A' }), _jsx("span", { className: "text-[8px] leading-none text-purple-200", children: "LV2 \u2192 3-2" })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), "                                ", _jsx("span", { children: EVOLUTION_COSTS[2] })] })] }, "evolve_a_lv3_2"));
                                                    }
                                                    else if (canEvolveA && !needsVariantA) {
                                                        const costA = EVOLUTION_COSTS[levelA];
                                                        const startIndex = getEvolutionButtonStartIndex(unit);
                                                        evolveButtons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleEvolve(unit.type, 'a'), className: "px-3 py-2 rounded flex flex-col items-center gap-0.5 min-w-[60px] transition-all relative bg-blue-600 hover:bg-blue-500 font-bold border-2 border-blue-400 shadow-lg shadow-blue-500/50 text-white animate-pulse", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: startIndex }), _jsx(Dna, { size: 20 }), _jsx("span", { className: "text-[9px] leading-none", children: language === 'zh_tw' ? '進化 A' : 'Evolve A' }), _jsxs("span", { className: "text-[8px] leading-none text-blue-200", children: ["LV", levelA, " \u2192 ", levelA + 1] })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), "                                ", _jsx("span", { children: costA })] })] }, "evolve_a"));
                                                    }
                                                    if (needsVariantB) {
                                                        const startIndex = getEvolutionButtonStartIndex(unit);
                                                        ...
                                                        evolveButtons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => {
                                                                    ...
                                                                    }, className: "px-3 py-2 rounded flex flex-col items-center gap-0.5 min-w-[60px] transition-all relative bg-yellow-600 hover:bg-yellow-500 font-bold border-2 border-yellow-400 shadow-lg shadow-yellow-500/50 text-white animate-pulse", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: buttonIndex }), _jsx(Dna, { size: 20 }), _jsx("span", { className: "text-[9px] leading-none", children: language === 'zh_tw' ? '進化 B' : 'Evolve B' }), _jsx("span", { className: "text-[8px] leading-none text-yellow-200", children: "LV2 \u2192 3-1" })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), "                                ", _jsx("span", { children: EVOLUTION_COSTS[2] })] })] }, "evolve_b_lv3_1"));
                                                        buttonIndex++;
                                                        evolveButtons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => {
                                                                        setGameState(prev => {
                                                                            const p = prev.players[prev.currentPlayer];
                                                                    ...
                                                                    }, className: "px-3 py-2 rounded flex flex-col items-center gap-0.5 min-w-[60px] transition-all relative bg-rose-600 hover:bg-rose-500 font-bold border-2 border-rose-400 shadow-lg shadow-rose-500/50 text-white animate-pulse", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: buttonIndex }), _jsx(Dna, { size: 20 }), _jsx("span", { className: "text-[9px] leading-none", children: language === 'zh_tw' ? '進化 B' : 'Evolve B' }), _jsx("span", { className: "text-[8px] leading-none text-rose-200", children: "LV2 \u2192 3-2" })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), "                                ", _jsx("span", { children: EVOLUTION_COSTS[2] })] })] }, "evolve_b_lv3_2"));
                                                    }
                                                    else if (canEvolveB && !needsVariantB) {
                                                        const costB = EVOLUTION_COSTS[levelB];
                                                        const startIndex = getEvolutionButtonStartIndex(unit);
                                                        const buttonIndex = startIndex + (needsVariantA ? 2 : canEvolveA ? 1 : 0);
                                                        evolveButtons.push(_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsxs("button", { onClick: () => handleEvolve(unit.type, 'b'), className: "px-3 py-2 rounded flex flex-col items-center gap-0.5 min-w-[60px] transition-all relative bg-orange-600 hover:bg-orange-500 font-bold border-2 border-orange-400 shadow-lg shadow-orange-500/50 text-white animate-pulse", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: buttonIndex }), _jsx(Dna, { size: 20 }), _jsx("span", { className: "text-[9px] leading-none", children: language === 'zh_tw' ? '進化 B' : 'Evolve B' }), _jsxs("span", { className: "text-[8px] leading-none text-orange-200", children: ["LV", levelB, " \u2192 ", levelB + 1] })] }), _jsxs("div", { className: "bg-slate-800 rounded px-1.5 py-0.5 flex items-start gap-0.5 text-[10px] font-bold text-white -mt-0.5", children: [_jsx("span", { className: "text-yellow-400", children: _jsx(Zap, { size: 10 }) }), "                                ", _jsx("span", { children: costB })] })] }, "evolve_b"));
                                                    }
                                                    return evolveButtons;
                                                })(), _jsx("div", { className: "flex flex-col items-center gap-1", children: _jsxs("button", { onClick: () => handleActionComplete(gameState.selectedUnitId), className: "px-3 py-2 rounded flex flex-col items-center gap-1 min-w-[60px] transition-all relative bg-slate-600 hover:bg-slate-500 font-bold border-2 border-slate-500 text-slate-200", children: [_jsx("div", { className: "absolute top-0.5 left-1.5 text-xs font-black text-white/90", children: getActionButtonIndex('end_turn', getUnit(gameState.selectedUnitId)) }), _jsx(CheckCircle, { size: 20 }), _jsx("span", { className: "text-[10px]", children: t('end_turn') })] }) })] })) : (_jsx("div", { className: "w-full flex flex-col items-center justify-center gap-2", children: (() => {
                                                const nextUnit = getNextUnitToAct();
                                                if (!nextUnit)
                                                    return _jsx("span", { className: "text-white font-semibold", children: t('select_unit') });
                                                return (_jsxs("button", { onClick: () => {
                                                                ...
                                                                players: {
                                                                    ...
                                                                    [PlayerID.P1]: {
                                                                        ...
                                                                        questStats: {
                                                                            ...
                                                                            triggeredMineThisRound: false,
                                                                            rangerMinesMovedThisRound: new Set()
                                                                        }
                                                                    },
                                                                    [PlayerID.P2]: {
                                                                        ...prev.players[PlayerID.P2],
                                                        ...
                                                        addLog('log_skip', 'move', { unit: getUnitName(nextUnit.type) }, nextUnit.owner);
                                                        handleActionComplete(nextUnit.id);
                                                    }, className: "w-full px-6 py-3 rounded flex items-center justify-center gap-2 transition-all bg-indigo-600 hover:bg-indigo-500 font-bold border-2 border-indigo-500 text-indigo-200 shadow-lg shadow-indigo-500/50", children: [_jsx(ArrowRight, { size: 20 }), _jsx("span", { className: "text-sm", children: t('skip_turn') })] }));
                                            })() })) }) })] })) })] }) }));
    };
    const renderSandboxPanel = () => {
        ...
        const evolveCurrentUnit = (branch, variant) => {
            ...
            setGameState(prev => {
                ...
                if (curLevel < 3) {
                    ...
                    if (curLevel + 1 === 3 && variant) {
                        if (branch === 'a')
                            newLevels[unit.type].aVariant = variant;
                        else
                            newLevels[unit.type].bVariant = variant;
                    }
                }
                return {
                    ...prev,
                    ...
                    logs: [{ turn: prev.turnCount, messageKey: 'log_evolved', params: { unit: getUnitName(unit.type), branch: branch.toUpperCase() + (variant ? `-${variant}` : ''), level: newLevels[unit.type][branch] }, type: 'evolution' }, ...prev.logs]
                };
            });
        };
        const toggleGodMode = () => {
            setGameState(prev => ({
                ...
                logs: [{ turn: prev.turnCount, messageKey: !prev.isGodMode ? 'God Mode Enabled' : 'God Mode Disabled', params: {}, type: 'info' }, ...prev.logs]
            }));
        };
        ...
        const updateUnitStat = (stat, change) => {
            if (!gameState.selectedUnitId)
                return;
            setGameState(prev => {
                const currentPlayer = prev.players[prev.currentPlayer];
                ...
                const newUnits = p.units.map(u => {
                    if (u.id === unit.id) {
                        const newVal = Math.max(1, u[stat] + change);
        ...
        return (_jsxs("div", { className: `fixed top-20 left-4 z-[60] bg-slate-900/95 border-2 border-yellow-500 rounded-xl shadow-2xl flex flex-col backdrop-blur-md animate-fade-in ${isSandboxCollapsed ? 'w-12 h-12 p-0 cursor-pointer hover:bg-slate-800' : 'p-4 min-w-[200px]'} transition-[width,height,padding,background-color] duration-300`, style: {
            ...
            }, children: [_jsxs("div", { onMouseDown: (e) => {
                        onSandboxDragStart(e);
                        // Stop propagation to prevent clicking through when dragging
                        if (isSandboxCollapsed)
                            e.stopPropagation();
                    }, className: `flex items-center select-none ${isSandboxCollapsed ? 'w-full h-full justify-center cursor-move' : 'justify-between gap-3 cursor-move border-b border-yellow-500/30 pb-2 mb-3'}`, children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(FlaskConical, { size: isSandboxCollapsed ? 24 : 18, className: "text-yellow-400" }), !isSandboxCollapsed && (_jsx("span", { className: "text-yellow-400 font-black text-sm tracking-wider", children: language === 'zh_tw' ? '沙盒模式' : 'SANDBOX TOOLS' }))] }), !isSandboxCollapsed && (_jsx("button", { onClick: (e) => {
                                e.stopPropagation();
                                setIsSandboxCollapsed(true);
                            }, className: "p-1 hover:bg-white/10 rounded transition-colors text-yellow-400", children: _jsx(X, { size: 16 }) }))] }), !isSandboxCollapsed && (_jsxs("div", { className: "grid grid-cols-1 gap-2 animate-in fade-in slide-in-from-top-2 duration-300", children: [_jsxs("button", { onClick: addEnergy, className: "flex items-center justify-center gap-2 px-3 py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded-lg font-black text-xs transition-all transform active:scale-95 shadow-lg shadow-yellow-500/20", children: [_jsx(Zap, { size: 14 }), language === 'zh_tw' ? '增加 100 能量' : '+100 Energy'] }), _jsxs("button", { onClick: healAll, className: "flex items-center justify-center gap-2 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-black text-xs transition-all transform active:scale-95 shadow-lg shadow-emerald-500/20", children: [_jsx(Shield, { size: 14 }), language === 'zh_tw' ? '治療全員' : 'Heal All'] }), _jsxs("button", { onClick: toggleGodMode, className: `flex items-center justify-center gap-2 px-3 py-2 rounded-lg font-black text-xs transition-all transform active:scale-95 shadow-lg ${gameState.isGodMode ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-500/20' : 'bg-slate-700 hover:bg-slate-600 text-slate-300 shadow-slate-900/20'}`, children: [_jsx(ShieldAlert, { size: 14 }), language === 'zh_tw' ? (gameState.isGodMode ? '關閉無敵模式' : '開啟無敵模式') : 'God Mode'] }), _jsxs("button", { onClick: skipToNextRound, className: "flex items-center justify-center gap-2 px-3 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-lg font-black text-xs transition-all transform active:scale-95 shadow-lg shadow-rose-500/20", children: [_jsx(RefreshCw, { size: 14 }), language === 'zh_tw' ? '跳過回合' : 'New Round'] }), _jsxs("button", { onClick: () => setGameState(prev => ({ ...prev, isSandboxTimerPaused: !prev.isSandboxTimerPaused })), className: `flex items-center justify-center gap-2 px-3 py-2 rounded-lg font-black text-xs transition-all transform active:scale-95 shadow-lg ${gameState.isSandboxTimerPaused ? 'bg-green-600 hover:bg-green-500 text-white shadow-green-500/20' : 'bg-orange-600 hover:bg-orange-500 text-white shadow-orange-500/20'}`, children: [gameState.isSandboxTimerPaused ? _jsx(Play, { size: 14 }) : _jsx(Pause, { size: 14 }), gameState.isSandboxTimerPaused ? (language === 'zh_tw' ? '恢復計時' : 'Resume Timer') : (language === 'zh_tw' ? '暫停計時' : 'Pause Timer')] }), _jsxs("button", { onClick: () => setTargetMode(targetMode === 'teleport' ? null : 'teleport'), className: `flex items-center justify-center gap-2 px-3 py-2 rounded-lg font-black text-xs transition-all transform active:scale-95 shadow-lg ${targetMode === 'teleport' ? 'bg-purple-500 text-white ring-2 ring-purple-300 shadow-purple-500/40' : 'bg-slate-700/50 hover:bg-slate-700 text-slate-300 shadow-slate-900/20'}`, children: [_jsx(Hand, { size: 14 }), language === 'zh_tw' ? '拖曳單位' : 'Drag Unit'] }), _jsx("div", { className: "border-t border-slate-700 my-1 opacity-50" }), _jsx("div", { className: "text-[10px] text-slate-400 font-black uppercase tracking-widest", children: language === 'zh_tw' ? '進化單位 (選中單位)' : 'Evolve Selected' }), _jsx("div", { className: "flex flex-col gap-1.5", children: (() => {
                                const unit = gameState.selectedUnitId ? getUnit(gameState.selectedUnitId) : null;
                                if (!unit)
                                    return _jsx("div", { className: "text-[10px] text-slate-500 italic text-center py-1 bg-slate-800/30 rounded", children: language === 'zh_tw' ? '未選擇單位' : 'No unit selected' });
                                const levels = gameState.players[unit.owner].evolutionLevels[unit.type];
                                return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex gap-1 items-center", children: [_jsx("span", { className: "text-[9px] font-black w-4 text-blue-400", children: "A" }), levels.a < 2 ? (_jsx("button", { onClick: () => evolveCurrentUnit('a'), className: "flex-1 px-1 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-black text-[10px] transition-all transform active:scale-95 shadow-lg shadow-blue-500/10", children: language === 'zh_tw' ? `進化 A (LV${levels.a} → ${levels.a + 1})` : `LV${levels.a} → ${levels.a + 1}` })) : levels.a === 2 ? (_jsxs("div", { className: "flex-1 flex gap-1", children: [_jsx("button", { onClick: () => evolveCurrentUnit('a', 1), className: "flex-1 px-1 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-black text-[10px] transition-all transform active:scale-95 border border-blue-400/50", children: "3-1" }), _jsx("button", { onClick: () => evolveCurrentUnit('a', 2), className: "flex-1 px-1 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-black text-[10px] transition-all transform active:scale-95 border border-blue-400/50", children: "3-2" })] })) : (_jsx("div", { className: "flex-1 py-1 bg-blue-900/30 text-blue-400/50 rounded-lg font-black text-[10px] text-center border border-blue-500/20", children: language === 'zh_tw' ? 'A 已滿級' : 'A MAX' }))] }), _jsxs("div", { className: "flex gap-1 items-center", children: [_jsx("span", { className: "text-[9px] font-black w-4 text-orange-400", children: "B" }), levels.b < 2 ? (_jsx("button", { onClick: () => evolveCurrentUnit('b'), className: "flex-1 px-1 py-1.5 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-black text-[10px] transition-all transform active:scale-95 shadow-lg shadow-orange-500/10", children: language === 'zh_tw' ? `進化 B (LV${levels.b} → ${levels.b + 1})` : `LV${levels.b} → ${levels.b + 1}` })) : levels.b === 2 ? (_jsxs("div", { className: "flex-1 flex gap-1", children: [_jsx("button", { onClick: () => evolveCurrentUnit('b', 1), className: "flex-1 px-1 py-1.5 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-black text-[10px] transition-all transform active:scale-95 border border-orange-400/50", children: "3-1" }), _jsx("button", { onClick: () => evolveCurrentUnit('b', 2), className: "flex-1 px-1 py-1.5 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-black text-[10px] transition-all transform active:scale-95 border border-orange-400/50", children: "3-2" })] })) : (_jsx("div", { className: "flex-1 py-1 bg-orange-900/30 text-orange-400/50 rounded-lg font-black text-[10px] text-center border border-orange-500/20", children: language === 'zh_tw' ? 'B 已滿級' : 'B MAX' }))] }), _jsxs("div", { className: "flex flex-col gap-1 border-t border-slate-700/50 pt-1.5 mt-0.5", children: [_jsx("div", { className: "text-[9px] text-slate-400 font-bold uppercase tracking-wider mb-1", children: language === 'zh_tw' ? '數值調整' : 'STATS ADJUST' }), _jsxs("div", { className: "flex items-center justify-between gap-2", children: [_jsx("span", { className: "text-[10px] font-black text-emerald-400", children: "HP" }), _jsxs("div", { className: "flex gap-1", children: [_jsx("button", { onClick: () => updateUnitStat('hp', -1), className: "w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-white rounded-md font-black text-xs border border-slate-600", children: "-" }), _jsx("div", { className: "min-w-[30px] text-center font-mono text-xs text-emerald-300 bg-emerald-950/30 rounded px-1 flex items-center justify-center", children: unit.hp }), _jsx("button", { onClick: () => updateUnitStat('hp', 1), className: "w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-white rounded-md font-black text-xs border border-slate-600", children: "+" })] })] }), _jsxs("div", { className: "flex items-center justify-between gap-2", children: [_jsx("span", { className: "text-[10px] font-black text-rose-400", children: "MAX" }), _jsxs("div", { className: "flex gap-1", children: [_jsx("button", { onClick: () => updateUnitStat('maxHp', -1), className: "w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-white rounded-md font-black text-xs border border-slate-600", children: "-" }), _jsx("div", { className: "min-w-[30px] text-center font-mono text-xs text-rose-300 bg-rose-950/30 rounded px-1 flex items-center justify-center", children: unit.maxHp }), _jsx("button", { onClick: () => updateUnitStat('maxHp', 1), className: "w-6 h-6 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-white rounded-md font-black text-xs border border-slate-600", children: "+" })] })] })] })] }));
                            })() })] }))] }));
    };
    return (_jsxs("div", { className: "w-full h-screen bg-slate-950 text-white flex flex-col overflow-hidden font-sans select-none", children: [_jsx("audio", { ref: audioRef, src: view === 'lobby' ? '/純音樂 - bgmdaily - 聽.mp3' : '/the-final-boss-battle-158700.mp3', loop: true }), view === 'lobby' ? (_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-8 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 p-4 relative", children: [_jsxs("div", { className: "text-center space-y-2 z-10", children: [_jsx("h1", { className: "text-7xl font-black bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 drop-shadow-2xl animate-pulse", children: t('app_title') }), _jsx("p", { className: "text-cyan-300 max-w-md mx-auto font-semibold", children: t('lobby_desc') })] }), _jsx("div", { className: "flex gap-6 z-10", children: !roomId ? (_jsxs(_Fragment, { children: [_jsxs("button", { onClick: () => handleStartGame('sandbox'), className: "px-8 py-4 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 rounded-xl font-black text-lg shadow-2xl shadow-yellow-500/50 transform hover:scale-110 transition-all duration-200 flex items-center gap-3 border-2 border-amber-300", children: [_jsx(FlaskConical, { size: 24 }), t('sandbox_mode')] }), _jsxs("button", { onClick: () => handleStartGame('pve'), className: "px-8 py-4 bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:from-violet-400 hover:to-fuchsia-400 rounded-xl font-black text-lg shadow-2xl shadow-violet-500/50 transform hover:scale-110 transition-all duration-200 flex items-center gap-3 border-2 border-violet-300", children: [_jsx(Cpu, { size: 24 }), t('pve_mode')] }), _jsxs("button", { onClick: () => setShowJoinModal(true), className: "px-8 py-4 bg-gradient-to-r from-slate-600 to-slate-500 hover:from-slate-500 hover:to-slate-400 rounded-xl font-black text-lg shadow-2xl shadow-slate-500/50 transform hover:scale-110 transition-all duration-200 flex items-center gap-3 border-2 border-slate-400 text-white", children: [_jsx(DoorOpen, { size: 24 }), language === 'zh_tw' ? '加入大廳' : 'Join Lobby'] })] })) : (_jsx("div", { className: "flex flex-col items-center gap-4 animate-fade-in", children: isHost ? (_jsxs("div", { className: "flex items-center gap-4", children: [_jsx("button", { onClick: () => {
                                            setRoomId(null);
                                            setIsHost(false);
                                        }, className: "px-8 py-6 bg-slate-700 hover:bg-slate-600 rounded-xl font-bold text-white text-xl transition-all border-2 border-slate-500", children: language === 'zh_tw' ? '離開大廳' : 'LEAVE' }), _jsxs("button", { onClick: () => handleStartGame('pvp'), className: "px-12 py-6 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 rounded-xl font-black text-2xl shadow-2xl shadow-orange-500/50 transform hover:scale-105 transition-all duration-200 flex items-center gap-4 border-2 border-orange-400 animate-pulse", children: [_jsx(Swords, { size: 32 }), language === 'zh_tw' ? '開始對戰' : 'START GAME'] })] })) : (_jsxs("div", { className: "flex flex-col items-center gap-4", children: [_jsxs("div", { className: "text-2xl font-bold text-cyan-300 animate-pulse", children: [language === 'zh_tw' ? '等待房主開始對戰' : 'Waiting for host to start game', _jsx("span", { className: "dot-loading" })] }), _jsx("button", { onClick: () => {
                                            setRoomId(null);
                                            setIsHost(false);
                                        }, className: "px-6 py-2 bg-slate-700/50 hover:bg-slate-600/50 rounded-lg text-slate-300 text-sm border border-slate-600 transition-all hover:text-white", children: language === 'zh_tw' ? '離開大廳' : 'Leave Lobby' }), _jsx("style", { children: `
                                            .dot-loading::after {
                                                content: ' .';
                                        ...
                                        ` })] })) })) }), roomId && (_jsxs("div", { className: "absolute top-4 left-4 z-20 bg-cyan-900/80 border border-cyan-500 px-4 py-2 rounded-full text-cyan-200 font-bold backdrop-blur-sm shadow-lg shadow-cyan-500/20 flex items-center gap-2", children: [_jsx("span", { className: "w-2 h-2 bg-green-500 rounded-full animate-pulse" }), language === 'zh_tw' ? '已加入房間: ' : 'Joined Room: ', " ", roomId] })), _jsx("div", { className: "absolute top-4 right-4 flex gap-2", children: _jsxs("button", { onClick: () => setLanguage(language === 'zh_tw' ? 'en' : 'zh_tw'), className: "p-2 bg-gray-800/50 hover:bg-gray-700/50 rounded-lg border border-gray-600 transition-colors flex items-center gap-2", children: [_jsx(Globe, { size: 16 }), language === 'zh_tw' ? 'EN' : '中文'] }) }), showJoinModal && (_jsx("div", { className: "fixed inset-0 bg-black/90 z-50 flex items-center justify-center backdrop-blur-md p-4", children: _jsxs("div", { className: "bg-slate-900 border-2 border-cyan-500 rounded-xl shadow-2xl shadow-cyan-500/20 max-w-5xl w-full h-[600px] flex overflow-hidden relative", children: [_jsx("button", { onClick: () => setShowJoinModal(false), className: "absolute top-4 right-4 z-10 p-2 text-slate-400 hover:text-white bg-slate-800/50 rounded-full hover:bg-slate-700 transition-colors", children: _jsx(X, { size: 24 }) }), _jsxs("div", { className: "w-2/3 border-r border-slate-700 bg-slate-900/50 flex flex-col", children: [_jsx("div", { className: "p-6 border-b border-slate-700 bg-slate-800/30", children: _jsxs("h2", { className: "text-2xl font-black text-white flex items-center gap-2", children: [_jsx(Users, { size: 24, className: "text-cyan-400" }), language === 'zh_tw' ? '大廳列表' : 'Lobby List'] }) }), _jsx("div", { className: "flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar", children: MOCK_ROOMS.map(room => (_jsxs("div", { onClick: () => room.status === 'waiting' ? setRoomCode(room.id) : null, className: `p-4 rounded-lg border-2 transition-all cursor-pointer flex items-center justify-between group
                                                    ${room.status === 'waiting'
                                                    ? 'border-slate-700 hover:border-cyan-500 bg-slate-800/50 hover:bg-slate-800'
                                                    : 'border-slate-800 bg-slate-900/50 opacity-60 cursor-not-allowed'}`, children: [_jsxs("div", { className: "flex flex-col", children: [_jsx("span", { className: `font-bold text-lg ${room.status === 'waiting' ? 'text-white group-hover:text-cyan-300' : 'text-slate-500'}`, children: room.name }), _jsxs("span", { className: "text-xs text-slate-400 font-mono", children: ["ID: ", room.id] })] }), _jsxs("div", { className: "flex items-center gap-4", children: [_jsx("div", { className: `px-3 py-1 rounded-full text-xs font-bold uppercase
                                                        ${room.status === 'waiting' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`, children: room.status === 'waiting'
                                                                    ? (language === 'zh_tw' ? '等待中' : 'WAITING')
                                                                    : (language === 'zh_tw' ? '遊戲中' : 'PLAYING') }), _jsxs("div", { className: "flex items-center gap-1 text-slate-400", children: [_jsx(Users, { size: 14 }), _jsxs("span", { className: "text-sm font-mono", children: [room.players, "/", room.maxPlayers] })] })] })] }, room.id))) })] }), _jsxs("div", { className: "w-1/3 bg-slate-950/80 flex flex-col items-center justify-center p-8 border-l-2 border-cyan-500/10 shadow-xl relative overflow-hidden", children: [_jsx("div", { className: "absolute inset-0 opacity-10 pointer-events-none", style: { backgroundImage: 'radial-gradient(circle at center, cyan 1px, transparent 1px)', backgroundSize: '20px 20px' } }), joinMode === 'join' ? (_jsxs("div", { className: "w-full max-w-xs space-y-6 relative z-10 animate-fade-in", children: [_jsxs("div", { className: "text-center", children: [_jsx("h3", { className: "text-xl font-bold text-white mb-2", children: language === 'zh_tw' ? '輸入房間代碼' : 'Enter Room Code' }), _jsx("p", { className: "text-xs text-slate-400", children: language === 'zh_tw' ? '輸入私人房間ID加入' : 'Input private room ID to join' })] }), _jsx("input", { type: "text", value: roomCode, onChange: (e) => setRoomCode(e.target.value), className: "w-full bg-slate-900 border-2 border-slate-700 rounded-lg p-4 text-white font-black text-center focus:border-cyan-500 focus:shadow-[0_0_20px_rgba(34,211,238,0.3)] outline-none text-3xl tracking-[0.2em] transition-all uppercase placeholder:opacity-20", placeholder: "____", maxLength: 6 }), _jsxs("button", { onClick: () => {
                                                        if (roomCode.trim()) {
                                                            setRoomId(roomCode);
                                                            setShowJoinModal(false);
                                                        }
                                                    }, className: "w-full py-4 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 rounded-lg font-black text-white text-lg shadow-lg shadow-cyan-500/30 transform hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2", disabled: !roomCode.trim(), children: [language === 'zh_tw' ? '加入房間' : 'JOIN ROOM', _jsx(ArrowRight, { size: 20 })] }), _jsxs("div", { className: "flex items-center justify-center gap-2 pt-4 border-t border-slate-800/50", children: [_jsx("span", { className: "text-xs text-slate-500", children: language === 'zh_tw' ? '沒有房間？' : 'No room?' }), _jsx("button", { onClick: () => setJoinMode('create'), className: "text-xs font-bold text-cyan-400 hover:text-cyan-300 underline", children: language === 'zh_tw' ? '創建大廳' : 'Create Room' })] })] })) : (_jsxs("div", { className: "w-full max-w-xs space-y-4 relative z-10 animate-fade-in", children: [_jsx("div", { className: "text-center mb-2", children: _jsx("h3", { className: "text-xl font-bold text-white mb-1", children: language === 'zh_tw' ? '創建大廳' : 'Create Room' }) }), _jsxs("div", { className: "space-y-3", children: [_jsxs("div", { children: [_jsx("label", { className: "text-xs font-bold text-slate-400 mb-1 block", children: language === 'zh_tw' ? '房間ID' : 'Room ID' }), _jsx("input", { type: "text", value: createRoomId, onChange: (e) => setCreateRoomId(e.target.value), className: "w-full bg-slate-900 border border-slate-700 rounded p-2 text-white font-mono text-center focus:border-cyan-500 outline-none uppercase", placeholder: "ID", maxLength: 6 })] }), _jsxs("div", { children: [_jsx("label", { className: "text-xs font-bold text-slate-400 mb-1 block", children: language === 'zh_tw' ? '房間名稱' : 'Room Name' }), _jsx("input", { type: "text", value: createRoomName, onChange: (e) => setCreateRoomName(e.target.value), className: "w-full bg-slate-900 border border-slate-700 rounded p-2 text-white focus:border-cyan-500 outline-none", placeholder: language === 'zh_tw' ? '輸入名稱...' : 'Enter name...', maxLength: 12 })] }), _jsxs("div", { className: "flex items-center justify-between", children: [_jsx("label", { className: "text-xs font-bold text-slate-400", children: language === 'zh_tw' ? '私人房間？' : 'Private?' }), _jsx("input", { type: "checkbox", checked: isPrivate, onChange: (e) => setIsPrivate(e.target.checked), className: "w-4 h-4 rounded border-slate-600 bg-slate-800 text-cyan-500 focus:ring-offset-slate-900" })] }), isPrivate && (_jsx("input", { type: "password", value: createRoomPassword, onChange: (e) => setCreateRoomPassword(e.target.value), className: "w-full bg-slate-900 border border-slate-700 rounded p-2 text-white focus:border-cyan-500 outline-none", placeholder: language === 'zh_tw' ? '密碼...' : 'Password...' }))] }), _jsxs("div", { className: "flex gap-2 pt-2", children: [_jsx("button", { onClick: () => setJoinMode('join'), className: "flex-1 py-3 bg-slate-800 hover:bg-slate-700 rounded font-bold text-slate-400 text-sm transition-colors", children: language === 'zh_tw' ? '返回' : 'Back' }), _jsx("button", { onClick: () => {
                                                                if (createRoomId.trim() && createRoomName.trim()) {
                                                                    setRoomId(createRoomId);
                                                                    setIsHost(true); // Host
                                                                    setShowJoinModal(false);
                                                                }
                                                            }, className: "flex-[2] py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 rounded font-black text-white text-sm shadow-lg shadow-emerald-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed", disabled: !createRoomId.trim() || !createRoomName.trim(), children: language === 'zh_tw' ? '創建' : 'Create' })] })] }))] })] }) })), _jsxs("div", { className: "absolute inset-0 overflow-hidden pointer-events-none", children: [_jsxs("div", { className: "absolute top-20 left-20", children: [_jsx("div", { className: "absolute inset-0 bg-cyan-500/20 blur-[50px] rounded-full scale-150 animate-pulse", style: { animation: 'cloud-flow 6s ease-in-out infinite' } }), _jsx("div", { style: { animation: 'fadeInOut 4s ease-in-out infinite' }, children: _jsx(Crown, { size: 200, className: "text-cyan-400 drop-shadow-2xl relative z-10" }) })] }), _jsxs("div", { className: "absolute bottom-20 right-20", children: [_jsx("div", { className: "absolute inset-0 bg-red-500/20 blur-[50px] rounded-full scale-150 animate-pulse", style: { animation: 'cloud-flow 6s ease-in-out infinite reverse' } }), _jsx("div", { style: { animation: 'fadeInOut 4s ease-in-out infinite' }, children: _jsx(Bomb, { size: 200, className: "text-red-400 drop-shadow-2xl relative z-10" }) })] }), _jsx("div", { className: "absolute inset-0 bg-gradient-to-br from-cyan-500/5 via-transparent to-red-500/5" })] })] })) : (_jsxs(_Fragment, { children: [showGameStartAnimation && (_jsxs("div", { className: "fixed inset-0 flex items-center justify-center bg-black/80 z-50 animate-fade-out", style: {
              ...
              ` }), _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "text-6xl font-black text-white mb-4 animate-pulse", children: "\u958B\u59CB" }), _jsx("div", { className: "text-4xl font-black bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500", children: "BATTLE START" })] })] })), _jsxs("div", { className: "h-12 bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 border-b-2 border-cyan-500 flex items-center justify-between px-4 shrink-0 z-30 shadow-2xl shadow-cyan-500/20", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("button", { onClick: handleExitGame, className: "p-2 hover:bg-cyan-500/20 rounded text-cyan-400 hover:text-cyan-300 transition-colors border border-cyan-500/50", children: _jsx(LogOut, { size: 20 }) }), _jsx("span", { className: "text-lg font-black bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500", children: t('app_title') }), _jsx("span", { className: "text-xs font-bold text-white bg-cyan-500/10 px-3 py-1 rounded-full border border-cyan-500/50 ml-2", children: gameState.gameMode === 'pvp' ? t('pvp_mode') : t('pve_mode') })] }), _jsxs("div", { className: "flex items-center gap-4", children: [_jsxs("div", { className: "text-sm text-white font-semibold", children: [t('current'), ": ", _jsx("span", { className: `font-black ${gameState.currentPlayer === PlayerID.P1 ? 'text-blue-400' : 'text-red-400'}`, children: gameState.currentPlayer === PlayerID.P1 ? 'P1' : 'P2' })] }), _jsx("button", { onClick: handlePause, className: "p-2 bg-cyan-500/10 hover:bg-cyan-500/20 rounded border border-cyan-500/50 transition-colors text-xs font-bold flex items-center gap-1 text-white hover:text-white", children: gameState.isPaused ? t('resume') : t('pause') }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: "relative w-5 h-5 flex items-center justify-center text-white", children: musicVolume === 0 ? (_jsxs(_Fragment, { children: [_jsx(VolumeX, { size: 20 }), _jsx("div", { className: "absolute w-6 h-1 bg-red-500 transform -rotate-45" })] })) : (_jsx(Volume2, { size: 20 })) }), _jsx("input", { type: "range", min: "0", max: "100", value: musicVolume * 100, onChange: (e) => setMusicVolume(Number(e.target.value) / 100), className: "w-24 h-2 bg-cyan-500/20 rounded-lg appearance-none cursor-pointer accent-cyan-400", style: {
                                                ...
                                                } }), _jsxs("span", { className: "text-xs text-cyan-300 font-bold w-8", children: [Math.round(musicVolume * 100), "%"] })] }), _jsxs("button", { onClick: () => setLanguage(language === 'zh_tw' ? 'en' : 'zh_tw'), className: "p-2 bg-cyan-500/10 hover:bg-cyan-500/20 rounded border border-cyan-500/50 transition-colors text-xs font-bold flex items-center gap-1 text-white hover:text-white", children: [_jsx(Globe, { size: 12 }), language === 'zh_tw' ? 'EN' : '中文'] })] })] }), _jsxs("div", { className: "flex-1 flex flex-col overflow-hidden relative", style: {
                            background: 'linear-gradient(135deg, #0a0e27 0%, #1a0033 25%, #0a0e27 50%, #1a0033 75%, #0a0e27 100%)',
                            backgroundSize: '400% 400%',
                            animation: 'gradientShift 15s ease infinite'
                        }, children: [_jsx("div", { className: "absolute inset-0 opacity-10", style: {
                                    backgroundImage: `
                     ...
                     `,
                                    ...
                                    pointerEvents: 'none',
                                    zIndex: 1
                                } }), _jsxs("div", { className: "absolute inset-0 pointer-events-none overflow-hidden", style: { zIndex: 9 }, children: [_jsx("style", { children: `
                            ...
                            ` }), Array.from({ length: 25 }).map((_, i) => (_jsx("div", { className: "absolute", style: {
                                            left: `${(i * 13 + 7) % 100}%`,
                                            top: '-200px',
                                            ...
                                            opacity: 0
                                            ...
                                            } }) }, `meteor-${i}`)))] }), _jsxs("div", { className: "absolute inset-0 pointer-events-none overflow-hidden", style: { zIndex: 5 }, children: [_jsxs("div", { className: "absolute inset-0 opacity-40 mix-blend-screen", style: { zIndex: 0 }, children: [_jsx("div", { className: "absolute inset-[-50%]", style: {
                                                ...
                                                } }), _jsx("div", { className: "absolute inset-[-50%]", style: {
                                            ...
                                            animationDelay: '1s',
                                            zIndex: 1
                                        } })] }), _jsxs("div", { className: "flex-1 flex flex-row overflow-hidden relative", style: { zIndex: 10 }, children: [_jsxs("div", { className: "flex-1 flex flex-col overflow-visible relative", children: [_jsx("div", { className: "flex-1 flex items-center justify-center p-4 relative overflow-visible", style: { zIndex: 10 }, onMouseMove: handleBoardMouseMove, onMouseLeave: handleBoardMouseLeave, children: _jsxs("div", { ref: boardRef, className: "grid gap-0 border-4 border-white bg-slate-900 rounded-lg overflow-visible relative z-10 shadow-[0_0_10px_rgba(255,255,255,0.3)]", style: {
                                                        gridTemplateColumns: 'repeat(24, 48px)',
                                                        gridTemplateRows: 'repeat(7, 48px)',
                                                        animation: 'gentleBreathe 4s ease-in-out infinite',
                                                        transformStyle: 'preserve-3d'
                                                    }, children: [_jsx("div", { className: "absolute top-0 bottom-0 left-1/2 w-0.5 bg-white shadow-[0_0_10px_rgba(255,255,255,0.8),0_0_20px_rgba(255,255,255,0.4)] z-20 transform -translate-x-1/2", style: { animation: 'gentleBreathe 4s ease-in-out infinite' } }), gameState.cells.map((row, r) => row.map((cell, c) => {
                                                            ...
                                                            const isVisible = realUnit && (realUnit.owner === gameState.currentPlayer ||
                                                            ...
                                                            return (_jsx(GridCell, { cell: cell, unit: unit, mine: mine, building: building, isSelected: gameState.selectedUnitId === unit?.id, isValidMove: false, isAttackTarget: false, isSkillTarget: false, currentPlayer: gameState.currentPlayer, isUnitStealthed: isUnitStealthed && realUnit?.owner === gameState.currentPlayer, onClick: () => {
                                                                ...
                                                                }, p1FlagLoc: gameState.players[PlayerID.P1].flagPosition, p2FlagLoc: gameState.players[PlayerID.P2].flagPosition, targetMode: targetMode, selectedUnit: selectedUnit, selectedGeneralLevelA: selectedGeneralLevelA, evolutionLevelA: cellUnitLevelA, evolutionLevelB: unit ? gameState.players[unit.owner].evolutionLevels[unit.type].b : 0, evolutionVariantA: unit ? gameState.players[unit.owner].evolutionLevels[unit.type].aVariant : undefined, evolutionVariantB: unit ? gameState.players[unit.owner].evolutionLevels[unit.type].bVariant : undefined, p1GeneralLevelB: gameState.players[PlayerID.P1].evolutionLevels[UnitType.GENERAL].b, p2GeneralLevelB: gameState.players[PlayerID.P2].evolutionLevels[UnitType.GENERAL].b, p1GeneralVariantB: gameState.players[PlayerID.P1].evolutionLevels[UnitType.GENERAL].bVariant, p2GeneralVariantB: gameState.players[PlayerID.P2].evolutionLevels[UnitType.GENERAL].bVariant, buildings: gameState.buildings, isSmoked: isSmoked, smokeOwner: smokesAtCell[0]?.owner, forceShowMines: gameState.sandboxShowAllMines }, `${r}-${c}`));
                                                        }))] }) }), _jsxs("div", { className: "w-full px-8 py-3 flex items-center justify-center gap-6 relative z-20", children: [_jsx("span", { className: "text-white font-bold text-lg min-w-[80px]", children: t(`${gameState.phase}_phase`) }), _jsx("div", { className: "flex-1 max-w-2xl", children: _jsx("div", { className: "w-full h-5 bg-slate-900 rounded-full overflow-hidden border-2 border-white/50 shadow-lg", children: (() => {
                                                                ...
                                                                return (_jsx("div", { className: `h-full ${color} ${transitionClass}`, style: { width: `${Math.min(percentage, 100)}%` } }));
                                                            })() }) }), _jsxs("span", { className: "text-white font-bold text-lg min-w-[50px] text-right", children: [gameState.timeLeft, "s"] })] })] }), _jsxs("div", { className: `shrink-0 z-20 relative transition-all duration-500 ease-in-out flex flex-row ${showLog ? 'w-80' : 'w-0'}`, children: [_jsxs("button", { onClick: () => setShowLog(!showLog), className: "absolute left-0 top-1/2 -translate-y-1/2 -translate-x-full w-8 h-24 bg-slate-900 border-2 border-white border-r-0 rounded-l-2xl flex flex-col items-center justify-center text-white hover:bg-slate-800 transition-all shadow-[0_0_15px_rgba(255,255,255,0.2)] z-30 group", title: showLog ? (language === 'zh_tw' ? "隱藏日誌" : "Hide Log") : (language === 'zh_tw' ? "顯示日誌" : "Show Log"), children: [showLog ? _jsx(ChevronRight, { size: 20, className: "group-hover:scale-125 transition-transform" }) : _jsx(ChevronLeft, { size: 20, className: "group-hover:scale-125 transition-transform" }), _jsx("div", { className: "text-[10px] font-black uppercase vertical-text mt-1 opacity-50 group-hover:opacity-100 transition-opacity", children: "LOG" })] }), _jsxs("div", { className: `w-80 h-full bg-slate-900 border-l-4 border-white flex flex-col text-sm transition-all duration-500 ${!showLog ? 'opacity-0 pointer-events-none' : 'opacity-100'}`, children: [_jsxs("div", { className: "shrink-0 p-3 font-black border-b-2 border-white bg-slate-800 flex items-center gap-2 text-white", children: [_jsx(Info, { size: 18 }), _jsx("span", { className: "text-lg whitespace-nowrap overflow-hidden", children: language === 'zh_tw' ? '遊戲日誌' : 'GAME LOG' })] }), _jsx("div", { className: "flex-1 overflow-y-auto p-2 space-y-2 text-sm scrollbar-thin scrollbar-thumb-white/30", children: gameState.logs.map((log, i) => {
                                                            ...
                                                            return (_jsxs("div", { className: `p-2 rounded border-l-4 leading-tight font-bold text-xs ${bgColor}`, children: [_jsxs("span", { className: "opacity-60 text-[10px] mr-2 font-bold text-gray-400", children: ["[", log.turn, "]"] }), t(log.messageKey, log.params)] }, i));
                                                        }) })] })] })] })] }), renderControlPanel(), renderSandboxPanel(), showEvolutionTree && renderEvolutionTree(), gameState.gameOver && (_jsx("div", { className: "absolute inset-0 z-50 bg-black/95 flex items-center justify-center pointer-events-auto backdrop-blur-sm", children: _jsxs("div", { className: "text-center space-y-6", children: [_jsx("h1", { className: "text-7xl font-black text-yellow-400 drop-shadow-2xl animate-pulse", children: t('game_over') }), _jsx("div", { className: "text-5xl font-black", children: gameState.winner === PlayerID.P1 ? (_jsxs("span", { className: "bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500", children: ["Player 1 ", t('wins')] })) : (_jsxs("span", { className: "bg-clip-text text-transparent bg-gradient-to-r from-red-400 to-pink-500", children: ["Player 2 ", t('wins')] })) }), _jsxs("div", { className: "flex gap-4 justify-center", children: [_jsx("button", { onClick: () => {
                                                setGameState(createInitialState(gameState.gameMode));
                                                setTargetMode(null);
                                            }, className: "px-8 py-4 bg-gradient-to-r from-emerald-600 to-cyan-600 hover:from-emerald-500 hover:to-cyan-500 rounded-lg font-black text-lg shadow-2xl shadow-emerald-500/50 transform hover:scale-110 transition-all border-2 border-emerald-400", children: t('play_again') }), _jsx("button", { onClick: handleExitGame, className: "px-8 py-4 bg-gradient-to-r from-slate-700 to-slate-600 hover:from-slate-600 hover:to-slate-500 rounded-lg font-black text-lg shadow-2xl transform hover:scale-110 transition-all border-2 border-slate-500", children: t('exit_lobby') })] })] }) })), gameState.isPaused && (_jsx("div", { className: "absolute inset-0 z-50 bg-black/95 flex items-center justify-center pointer-events-auto backdrop-blur-sm", children: _jsxs("div", { className: "text-center space-y-6", children: [_jsx("h1", { className: "text-7xl font-black text-cyan-400 drop-shadow-2xl animate-pulse", children: t('paused') }), _jsxs("div", { className: "flex gap-4 justify-center", children: [_jsx("button", { onClick: handlePause, className: "px-8 py-4 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 rounded-lg font-black text-lg shadow-2xl shadow-cyan-500/50 transform hover:scale-110 transition-all border-2 border-cyan-400", children: t('resume') }), _jsx("button", { onClick: handleExitGame, className: "px-8 py-4 bg-gradient-to-r from-slate-700 to-slate-600 hover:from-slate-600 hover:to-slate-500 rounded-lg font-black text-lg shadow-2xl transform hover:scale-110 transition-all border-2 border-slate-500", children: t('exit_lobby') })] })] }) }))] })), renderSandboxPanel()] }));
}
